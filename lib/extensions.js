...
module.exports = function(realmConstructor, context) {
    ...
    Object.defineProperties(realmConstructor, getOwnPropertyDescriptors({
        open: function (config) {
            ...
            let openLocalRealmImmediately = config.sync[behavior] !== undefined && config.sync[behavior].type === 'openImmediately'; // Defaults to 'syncBeforeOpen'
            let throwOnTimeOut = config.sync[behavior] !== undefined && config.sync[behavior]["timeOutBehavior"] === 'throwException';
            let timeOut = 0x7FFFFFFF; // Max value allowed by setTimeOut()
    ...
    if (realmConstructor.Sync) {
        ...
        realmConstructor.Sync.syncBeforeOpenBehavior = {
            type: 'syncBeforeOpen',
            timeOut: 30 * 1000,
