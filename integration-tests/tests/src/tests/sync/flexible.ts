...
let syncError: Realm.SyncError | Realm.ClientResetError | undefined;
...
describe("Flexible sync", function () {
  ...
  async function expectClientResetError(
    config: Realm.Configuration,
    user: Realm.User,
    action: (realm: Realm) => void,
  ): Promise<void> {
  ...
  importAppBefore("with-db-flx", {}, DEBUG ? "all" : undefined);
  authenticateUserBefore();
  ...
  describe("setup", function () {
    ...
    describe("Subscriptions", function () {
      ...
      describe("#error", function () {
        ...
        it("still has the subscription in the set if there was an error synchronising subscriptions", async function (this: RealmContext) {
          const { subs } = addSubscription(
            this.realm,
            this.realm.objects(PersonSchema.name).filtered("nonQueryable == 'test'"),
          );
      ...
      describe("#update", function () {
        // These tests are a hangover from before MutableSubscriptionSet was introduced in C++
        // but maybe it still makes sense to keep them to be explicit about the divide
        ...
        it("still applies updates in a batch if one errors", async function (this: RealmContext) {
          const { subs } = addPersonSubscription(this.realm);
          ...
          expect(subs.snapshot()).to.have.length(4);
        });
    ...
    // TODO verify correct behaviour here
    describe("when realm is closed", function () {
      xit("waitForSynchronization throws an error if the Realm has been closed", async function (this: RealmContext) {
        const { subs } = addPersonSubscription(this.realm);
        this.realm.close();
        await subs.waitForSynchronization();
      });

      xit("update throws an error if the Realm has been closed", async function (this: RealmContext) {
        const { subs } = addPersonSubscription(this.realm);
        this.realm.close();
        subs.update(() => {});
      });
  ...
  describe("synchronising data", function () {
    ...
    // Waiting on https://github.com/realm/realm-core/pull/5134 and https://github.com/10gen/baas/pull/5471
