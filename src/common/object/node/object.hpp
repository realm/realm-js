...
template <typename GetterSetter> class JavascriptObject {
private:
  Napi::Env context;
  ...
  template <class T> using ObjectAPI = realm::js::Object<T>;
...
public:
  ...
  void finalize(RemovalCallback &&callback, Self *self) {
    auto object = get();
    object.AddFinalizer([callback](auto, void *data_ref) { callback(); }, self);
  }
  ...
  bool is_alive() { return !get().IsUndefined(); }
