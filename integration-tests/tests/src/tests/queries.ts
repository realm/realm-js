...
describe("Queries", () => {
  [false, true].forEach((useFlexibleSync) => {
    describe.skipIf(
      environment.missingServer && useFlexibleSync,
      `GeoSpatial ${useFlexibleSync ? "(with QBS)" : ""}`,
      () => {
        useFlexibleSync && importAppBefore(buildAppConfig("with-flx").anonAuth().flexibleSync());
        ...
        async function geoTest(
          realm: Realm,
          geo: GeoCircle | GeoBox | GeoPolygon,
          pois: IPointOfInterest[],
          useFlexibleSync = false,
        ) {
          const poiIds = pois.map((p) => p._id);
          return expectQueryResultValues(
            realm,
            PointOfInterest,
            "_id",
        ...
        beforeEach(async function (this: RealmContext | UserContext) {
          const objects = this.realm.objects(PointOfInterest);
          if (useFlexibleSync) {
            await objects.subscribe();
          }
          if (objects.length === 0) {
            this.realm.write(() => {
        ...
        describe("Base cases", () => {
          it("GeoCircle basic", async function (this: RealmContext) {
            ...
            await expectQueryLength(
              this.realm,
              PointOfInterest,
              [[0, "location geoWithin $0 SORT(_id ASC)", circle]],
              useFlexibleSync,
            );
            await expectQueryLength(
              this.realm,
          ...
          it("GeoBox basic", async function (this: RealmContext) {
            ...
            await expectQueryLength(
              this.realm,
              PointOfInterest,
              [[0, "location geoWithin $0 SORT(_id ASC)", box]],
              useFlexibleSync,
            );
            await expectQueryLength(
              this.realm,
          ...
          it("GeoPolygon basic", async function (this: RealmContext) {
            ...
            await expectQueryLength(
              this.realm,
              PointOfInterest,
              [[0, "location geoWithin $0 SORT(_id ASC)", polygon]],
              useFlexibleSync,
            );
            await expectQueryLength(
              this.realm,
              ...
              [
                [
                  0,
                  "location geoWithin geoPolygon({[-20.0, -20.0], [-10.0, 10.0], [-50.0, 0], [-20.0, -20.0]}) SORT(_id ASC)",
                ],
              ],
              useFlexibleSync,
            );
          ...
          it("Alternative GeoPoint", async function (this: RealmContext) {
            ...
            await geoTest(this.realm, circle, queryResults, useFlexibleSync);
            ...
            await geoTest(this.realm, box, queryResults, useFlexibleSync);
            ...
            await geoTest(this.realm, polygon, queryResults, useFlexibleSync);
          });
          ...
          it.skipIf(useFlexibleSync, "Altitude is supported but ignored", async function (this: RealmContext) {
            let circle: GeoCircle = {
              center: [-32.34, -25, 23],
              distance: 0.5,
            };
          ...
          it("Coordinate Substitution", async function (this: RealmContext) {
            ...
            await expectQueryResultValues(
              ...
              [
                [
                  queryResultsIds,
                  "location geoWithin geoBox([$0, $1], [$2, $3]) SORT(_id ASC)",
                  -90.23,
                  -80.25,
                  85.24,
                  88.0,
          ...
          it.skipIf(useFlexibleSync, "Distance conversions", function (this: RealmContext) {
        ...
        describe("Complex cases", () => {
          it("Intersection queries", async function (this: RealmContext) {
            ...
            await geoTest(this.realm, polygon, [poiA, poiB], useFlexibleSync);
          ...
          it("Circle around north pole", async function (this: RealmContext) {
            ...
            await geoTest(this.realm, circle, [northPole], useFlexibleSync);
          });
          ...
          it("Small distances", async function (this: RealmContext) {
            ...
            await geoTest(this.realm, circle, [slurpRamen], useFlexibleSync);
            ...
            await geoTest(this.realm, circle, [magasasaKodbyen, slurpRamen], useFlexibleSync);
          });
          ...
          it("Polygon with multiple holes", async function (this: RealmContext) {
            ...
            await geoTest(this.realm, polygon, [poiB, poiD], useFlexibleSync);
          });
        });

        // Not sync relevant, so we can skip testing for these errors twice
        describe.skipIf(useFlexibleSync, "Error cases", () => {
