...
public:
  ...
  Value unwrap(Context context, Mixed mixed) {
    return Utils::from_string(context, mixed.template get<StringData>().data());
  }
};
...
class MixedNullable : public MixedWrapper<Context, Value> {
  Mixed wrap(Context context, Value const &value) { return Mixed(); }
...
template <typename JavascriptEngine> class TypeMixed {
private:
...
public:
  ...
  void unregister(types::Type type) { strategies.erase(type); }
  ...
  Value wrap(Context context, Mixed mixed) {
    ...
    if (strategy == nullptr) {
      throw std::runtime_error("The " +
                               type_deduction.javascript_type(rjs_type) +
  ...
  Mixed unwrap(Context context, Value const &js_value) {
    ...
    if (strategy == nullptr) {
      throw std::runtime_error("Mixed conversion not possible for type: " +
