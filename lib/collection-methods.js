...
const { parseFilter } = require("./filter-query-parser");
...
["entries", "keys", "values"].forEach(function (methodName) {
  ...
  function filterQuery(filterFn, deps = undefined) {
    const rql = parseFilter(filterFn.toString(), deps.toString(), deps);
    console.log(`Using RQL: ${rql}`);

    return this.filtered(rql);
  }
