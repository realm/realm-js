...
### Enhancements
* Added configuration option `SyncConfiguration.cancelWaitOnNonFatalError`. Set to true, all async operations (such as opening the Realm with `Realm.open`) will fail when a non-fatal error, such as a timeout, occurs.  
