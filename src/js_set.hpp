 ...
 * @param prop (mutable) Property object that will be changed to be correct for
 * \ref Set
 * @exception std::logic_error Thrown if the the prop argument contains an
 * invalid property configuration
 */
inline static void derive_property_type(StringData const &object_name,
                                        Property &prop) {
  ...
  if (prop.object_type == "bool") {
    ...
    if (is_nullable(prop.type)) {
      throw std::logic_error(util::format(
          "Set property '%1.%2' cannot be optional", object_name, prop.name));
    }
    if (is_array(prop.type)) {
      throw std::logic_error(
          util::format("Set property '%1.%2' must have a non-list value type",
...
/**
 * @brief Glue class that provides an interface between \ref SetClass and \ref
 * realm::object_store::Set
 *
 *  The Set class itself is an internal glue that delegates operations from \ref
 * SetClass to \ref realm::object_store::Set.  It is used by Realm-JS's object
 * management system, and it not meant to be instantiated directly.
 *
 * @tparam T The type of the elements that the Set will hold.  Inherited from
 * \ref SetClass
 */
template <typename T> class Set : public realm::object_store::Set {
public:
  Set(const realm::object_store::Set &set) : realm::object_store::Set(set) {}
  void derive_property_type(StringData const &object_name,
                            Property &prop) const;

  std::vector<std::pair<Protected<typename T::Function>, NotificationToken>>
      m_notification_tokens;
};
...
struct SetClass : ClassDefinition<T, realm::js::Set<T>, CollectionClass<T>> {
  ...
  static void delete_element(ContextType, ObjectType, Arguments &,
                             ReturnValue &);
  ...
  MethodMap<T> const methods = {
      ...
      {"min",
       wrap<compute_aggregate_on_collection<SetClass<T>, AggregateFunc::Min>>},
      {"max",
       wrap<compute_aggregate_on_collection<SetClass<T>, AggregateFunc::Max>>},
      {"sum",
       wrap<compute_aggregate_on_collection<SetClass<T>, AggregateFunc::Sum>>},
      {"avg",
       wrap<compute_aggregate_on_collection<SetClass<T>, AggregateFunc::Avg>>},
...
private:
  static void validate_value(ContextType, realm::object_store::Set &,
                             ValueType);
};

template <typename T>
typename T::Object SetClass<T>::create_instance(ContextType ctx,
                                                realm::object_store::Set set) {
  return create_object<T, SetClass<T>>(ctx,
                                       new realm::js::Set<T>(std::move(set)));
}
 ...
 * @param return_value \ref ReturnValue wrapping an integer that gives the
 * number of elements in the set to return to the JS context
 */
template <typename T>
void SetClass<T>::get_size(ContextType ctx, ObjectType object,
 ...
 */
template <typename T>
void SetClass<T>::get_indexed(ContextType ctx, ObjectType object,
                              uint32_t index, ReturnValue &return_value) {
  auto set = get_internal<T, SetClass<T>>(ctx, object);
...
/**
 * @brief Check whether the Set's element type is marked as optional
 * (`nullable`)
 *
 * @param ctx JS context
 * @param object \ref ObjectType wrapping the SetClass itself
 * @param return_value \ref ReturnValue wrapping a boolean that is true if the
 * Set's element type is nullable, false otherwise
 */
template <typename T>
void SetClass<T>::get_optional(ContextType ctx, ObjectType object,
                               ReturnValue &return_value) {
  auto set = get_internal<T, SetClass<T>>(ctx, object);
 ...
 *
 *  Adds a single element, `A`, of type `T` to the set.  `A` will not be added
 * if it already exists within the SetClass. See [MDN's reference
 ...
 * @param args \ref Arguments structure containing a single new element of type
 * `T` to add to the Set
 * @param return_value \ref ReturnValue wrapping the Set itself, including the
 * newly added element to return to the JS context
 */
template <typename T>
void SetClass<T>::add(ContextType ctx, ObjectType this_object, Arguments &args,
                      ReturnValue &return_value) {
 ...
 * @param return_value \ref ReturnValue wrapping a `Mixed<T>` object, wrapping
 * the found element to return to the JS context
 */
template <typename T>
void SetClass<T>::get(ContextType ctx, ObjectType this_object, Arguments &args,
                      ReturnValue &return_value) {
  args.validate_maximum(1);
  ...
  switch_on_type(value_type, [&](auto const type_indicator) -> void {
    ...
    realm::Mixed element_value =
        set->template get<std::remove_pointer_t<decltype(type_indicator)>>(
            requested_index);
...
/**
 * @brief Implements JavaScript Set's `clear()` method.  Removes all elements
 * from the set.
 *
 ...
 * @param return_value \ref ReturnValue wrapping `undefined` to return to the JS
 * context
 */
template <typename T>
void SetClass<T>::clear(ContextType ctx, ObjectType this_object,
...
/**
 * @brief Implements JavaScript Set's `delete()` method.  Removes a single
 ...
 * @param return_value \ref ReturnValue wrapping the value to return to the JS
 * context
 */
template <typename T>
void SetClass<T>::delete_element(ContextType ctx, ObjectType this_object,
                                 Arguments &args, ReturnValue &return_value) {
  args.validate_maximum(1);
 ...
 * @param ctx JS context
 * @param this_object \ref ObjectType wrapping the SetClass itself
 * @param args \ref Arguments structure containing a single element of type `T`
 * to search for
 * @param return_value \ref ReturnValue wrapping the value to return to the JS
 * context
 */
template <typename T>
void SetClass<T>::has(ContextType ctx, ObjectType this_object, Arguments &args,
                      ReturnValue &return_value) {
  args.validate_maximum(1);
 ...
 *
 *  Applies a filter to the elements in the SetClass and returns the elements
 * that match the filter. Filters are only supported for sets of objects. Will
 * throw `std::runtime_error` if the Set's element type is not objects.
 *
 * @param this_object \ref ObjectType wrapping the SetClass itself
 * @param args \ref Arguments structure containing the filter that will be
 * applied to the SetClass
 * @param return_value \ref ReturnValue wrapping a \ref ResultClass containing
 * matching objects to return to the JS context
 * @exception std::runtime_error Thrown if the \ref SetClass does not contain
 * objects
 */
template <typename T>
void SetClass<T>::filtered(ContextType ctx, ObjectType this_object,
                           Arguments &args, ReturnValue &return_value) {
  auto const set = get_internal<T, SetClass<T>>(ctx, this_object);
...
/**
 * @brief Return a textual description of the value element type for the given
 * set
 *
 * @param ctx JS context
 * @param object \ref ObjectType wrapping the SetClass itself
 * @param return_value \ref ReturnValue wrapping a static `const char *`
 * descriping the set's element type
 */
template <typename T>
void SetClass<T>::get_type(ContextType ctx, ObjectType object,
                           ReturnValue &return_value) {
  auto const set = get_internal<T, ListClass<T>>(ctx, object);
  return_value.set(local_string_for_property_type(set->get_type() &
                                                  ~realm::PropertyType::Flags));
}

/**
 * @brief Utility function that validates that elements of a given type is
 * eligible for insertion into the set
 *
 ...
 * @param value \ref ValueType that is to be checked whether it is valid for the
 * set
 * @exception TypeErrorException Thrown if `value` is not valid for the set
 */
template <typename T>
void SetClass<T>::validate_value(ContextType ctx, realm::object_store::Set &set,
                                 ValueType value) {
  ...
  if (!Value::is_valid_for_property_type(ctx, value, type, object_type)) {
    throw TypeErrorException("Property",
                             object_type ? object_type
                                         : local_string_for_property_type(type),
 ...
 * @param return_value \ref ReturnValue wrapping a a new Set instance created by
 * the snapshot
 */
template <typename T>
void SetClass<T>::snapshot(ContextType ctx, ObjectType this_object,
                           Arguments &args, ReturnValue &return_value) {
  args.validate_maximum(0);
  auto set = get_internal<T, SetClass<T>>(ctx, this_object);
 ...
 */
template <typename T>
void SetClass<T>::add_listener(ContextType ctx, ObjectType this_object,
                               Arguments &args, ReturnValue &return_value) {
 ...
 * @param ctx JS context
 * @param set \ref realm::object_store::Set that contains the valid value type
 * @param args A single argument containing the callback function of the
 * previously-registered listener
 * @param return_value None
 */
template <typename T>
void SetClass<T>::remove_listener(ContextType ctx, ObjectType this_object,
                                  Arguments &args, ReturnValue &return_value) {
  // args is validated by ResultClass
  auto set = get_internal<T, SetClass<T>>(ctx, this_object);
 ...
 */
template <typename T>
void SetClass<T>::remove_all_listeners(ContextType ctx, ObjectType this_object,
                                       Arguments &args,
                                       ReturnValue &return_value) {
  args.validate_maximum(0);
  auto set = get_internal<T, SetClass<T>>(ctx, this_object);
