...
- Proof-read and mildly edit `CHANGELOG.md`.
    ...
    - If the release upgrades `realm-core`, review the `realm-core` release notes and copy over any relevant notes (edited to be JS-specific if appropriate) into our `CHANGELOG.md`.
        - Format the link as such: `([realm/realm-core#XXXX](https://github.com/realm/realm-core/issues/XXXX), since vX.Y.Z)` where the `since` version refers to the Realm JS version.
...
- Click on the "Actions" tab, scroll down to "[Prepare Release](https://github.com/realm/realm-js/actions/workflows/prepare-release.yml)" (or first click on "Show more workflows..." if not visible), and click on that. If you have access to the Realm group, you will see a note "This workflow has a `workflow_dispatch` event trigger", with a "Run workflow" drop-down button.
...
- If the "Publish Docs" action has not started automatically, click on that action and then click on "Run workflow".
    ...
    - Click "Run workflow".
    - Once the workflow is done, verify that the new docs have been published by checking our [API Reference page](https://www.mongodb.com/docs/realm-sdks/js/latest/).
