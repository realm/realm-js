...
describe("User", () => {
  ...
  it("has context property", () => {
    ...
    user.removeContext("X-Custom-Header");
    {
      expect(user.context).not.has.property("X-Custom-Header");
  ...
  it("has dynamic context property", () => {
    ...
    let ctxValue = 42;
    user.setContext("X-Custom-Header", () => String(ctxValue));
    expect(user.context["X-Custom-Header"]).equals("42");
    ctxValue *= 2;
    expect(user.context["X-Custom-Header"]).equals("84");
  });
