...
module.exports = function(realmConstructor, context) {
    ...
    if (realmConstructor.Sync) {
        ...
        ResultSets.schema = Object.freeze({
            name: '__ResultSets',
            properties: {
                name: { type: 'string', indexed: true, alias: '_name' },
                query: {type: 'string', alias: '_query'},
                matches_property: {type: 'string', alias: '_matchesProperty'},
                status: {type: 'int', alias: '_state'},
                error_message: { type: 'string', alias: '_errorMessage'},
                query_parse_counter: {type: 'int', alias: '_queryParseCounter'},
                created_at: { type: 'date', alias: '_createdAt'},
                updated_at: { type: 'date', alias: '_updatedAt'},
                expires_at: { type: 'date', optional: true, alias: '_expiresAt'},
                time_to_live: { type: 'int', optional: true, alias: '_timeToLive'},
            }
        });
        Realm.prototype._subscriptionUpdated = function(sub) {
            this._updatedAt = new Date();
            this._expiresAt = new Date(this._updatedAt.getTime() + this._timeToLive);
        };
        Object.defineProperties(ResultSets.prototype, {
            objectType: {
                enumerable: true,
                get: function() {
                    return this['matches_property'].replace(subscriptionObjectNameRegex, '$2');
                }
            },
            name: {
                enumerable: true,
