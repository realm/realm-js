...
template<>
    inline bool hermes::Value::is_array_buffer_view(JsiEnv env, const JsiVal& value) {
        ...
        return arrayBuffer.getPropertyAsFunction(env, "isView").call(env, value).getBool();
    }

template<>
    inline bool hermes::Value::is_date(JsiEnv env, const JsiVal& value) {
        return value->isObject() && value->getObject(env).instanceOf(env, env->global().getPropertyAsFunction(env, "Date"));
    }
...
template<>
    inline bool hermes::Value::is_error(JsiEnv env, const JsiVal& value) {
        return value->isObject() && value->getObject(env).instanceOf(env, env->global().getPropertyAsFunction(env, "Error"));
    }
