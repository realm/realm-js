...
const syncLevels = ["all", "trace", "debug", "detail", "info", "warn", "error", "fatal", "off"];
...
class SyncServerProxy {
  ...
  register(serverPath) {
    ...
    this.adminRealm.write(() =>
      this.adminRealm.create("RealmFile", {
...
global.TestObjectServer = module.exports = class TestObjectServer extends EventEmitter {
  constructor() {
    ...
    this.httpPort = 9090;
    this.adminUser = Realm.Sync.User.login(
  ...
  deleteRealm(serverPath) {
    ...
    realm.write(() => {
      ...
      if (objects.length != 1) {
        throw new Error(
          `Realm ${serverPath} not found. Existing Realms: ${realm.objects("RealmFile").map((r) => r.path)}`,
        );
      }
  ...
  _writeConfiguration() {
    fs.writeFileSync(
      path.join(this._temp.name, "config.yaml"),
      `
        storage:
