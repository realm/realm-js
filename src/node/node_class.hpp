...
namespace realm {
namespace js {
template <typename T> struct RealmObjectClass;
...
static void node_class_init(Napi::Env env) {
  auto setPrototypeOf = env.Global()
                            .Get("Object")
                            .As<Napi::Object>()
                            .Get("setPrototypeOf")
                            .As<Napi::Function>();
  ...
  auto getOwnPropertyDescriptor = env.Global()
                                      .Get("Object")
                                      .As<Napi::Object>()
...
template <typename T> using ClassDefinition = js::ClassDefinition<Types, T>;
...
public:
  ...
  static Napi::Function init(
      ...
      const std::vector<Napi::ClassPropertyDescriptor<WrappedObject<ClassType>>>
          &properties,
  ...
  void setter_callback(const Napi::CallbackInfo &info,
                       const Napi::Value &value);
  void readonly_setter_callback(const Napi::CallbackInfo &info,
                                const Napi::Value &value);
  static void readonly_static_setter_callback(const Napi::CallbackInfo &info,
                                              const Napi::Value &value);
...
private:
  ...
  private:
    static Napi::Value bind_native_function(Napi::Env env,
    ...
    static Napi::Value
    get_own_property_descriptor_trap(const Napi::CallbackInfo &info);
    static Napi::Value
    get_prototype_of_proxy_trap(const Napi::CallbackInfo &info);
    static Napi::Value
    set_prototype_of_proxy_trap(const Napi::CallbackInfo &info);
  };
};
...
template <typename ClassType> std::string WrappedObject<ClassType>::m_name;

template <typename ClassType>
std::function<bool(const std::string &)>
    WrappedObject<ClassType>::m_has_native_methodFunc;

template <typename ClassType>
Napi::Reference<Napi::External<typename ClassType::Internal>>
    WrappedObject<ClassType>::m_nullExternal;
...
public:
  ...
  static Napi::Object
  create_instance_by_schema(Napi::Env env, Napi::Function &constructor,
                            const realm::ObjectSchema &schema,
                            Internal *internal = nullptr);
  static Napi::Object
  create_instance_by_schema(Napi::Env env, const realm::ObjectSchema &schema,
                            Internal *internal = nullptr) {
    Napi::Function constructor;
    return create_instance_by_schema(env, constructor, schema, internal);
  }
  static void internal_finalizer(Napi::Env,
  ...
  static void set_internal(Napi::Env env, const Napi::Object &object,
                           Internal *data);
...
public:
  ...
  static Napi::Function init_class(Napi::Env env) { return Napi::Function(); }

  static bool has_native_method(const std::string &name) { return false; }
};

// Gives access to ObjectWrap<ClassType> init_class private static member. See
// https://stackoverflow.com/a/40937193
template <typename T> struct ObjectWrapAccessor {
  inline static Napi::Function init_class(Napi::Env env) {
    return ObjectWrap<T>::init_class(env);
  }
};

static inline std::vector<Napi::Value>
get_arguments(const Napi::CallbackInfo &info) {
...
static inline std::vector<napi_value>
napi_get_arguments(const Napi::CallbackInfo &info) {
  size_t count = info.Length();
...
template <typename ClassType> inline auto &ObjectWrap<ClassType>::get_class() {
...
inline auto &ObjectWrap<ClassType>::get_schemaObjectTypes() {
  static std::unordered_map<
      std::string, std::unordered_map<std::string, SchemaObjectType *> *>
      s_schemaObjectTypes;
  return s_schemaObjectTypes;
}

// A cache for property names. The pair is property name and a node::String* to
// the same string representation. The cache is persisted throughout the process
// life time to preseve property names between constructor cache invalidations
// (on_destory_context is called) Since RealmObjectClass instances may be used
// after context is destroyed, their property names should be valid
...
inline static void copy_object(Napi::Env env, const Napi::Value &source,
                               const Napi::Error &target) {
...
WrappedObject<ClassType>::WrappedObject(const Napi::CallbackInfo &info)
  ...
  // skip the constructor_callback if create_instance is creating a JS instance
  // only
  ...
  try {
    node::Types::FunctionCallback constructor_callback =
        (node::Types::FunctionCallback)info.Data();
...
Napi::Function WrappedObject<ClassType>::init(
  ...
  WrappedObject<ClassType>::m_nullExternal =
      Napi::Persistent(Napi::External<Internal>::New(env, nullptr));
  WrappedObject<ClassType>::m_nullExternal.SuppressDestruct();

  Napi::Function ctor = Napi::ObjectWrap<WrappedObject<ClassType>>::DefineClass(
      env, name.c_str(), properties, (void *)constructor_callback);

  constructor = Napi::Persistent(ctor);
  constructor.SuppressDestruct();

  m_indexPropertyHandlers =
      const_cast<IndexPropertyType *>(indexPropertyHandlers);
  return ctor;
}

// This creates the required JS instance with a Proxy parent to support get and
// set handlers and returns a proxy created on the JS instance to support
// property enumeration handler the returned JS Proxy has only ownKeys trap setup
// so that all other member accesses skip the Proxy and go directly to the JS
// instance
template <typename ClassType>
Napi::Value WrappedObject<ClassType>::create_instance_with_proxy(
  ...
  if (constructor.IsEmpty()) {
    ...
    std::string errorMessage =
        util::format("create_instance_with_proxy: Class %s not initialized. "
                     "Call init() first",
                     typeName);
  ...
  if (!info.IsConstructCall()) {
    throw Napi::Error::New(env,
                           "This function should be called as a constructor");
  }
  ...
  try {
    ...
    // using DefineProperty to make it non enumerable and non configurable and
    // non writable
    instance.DefineProperty(
        Napi::PropertyDescriptor::Value("_instance", instance, napi_default));

    info.This().As<Napi::Object>().DefineProperty(
        Napi::PropertyDescriptor::Value(
            "isRealmCtor", Napi::Boolean::New(env, true), napi_configurable));

    ObjectSetPrototypeOf.Call({info.This(), instance});
    Napi::Object instanceProxy = GlobalProxy.New(
        {info.This(), ProxyHandler::get_instance_proxy_handler(env)});

    instance.DefineProperty(Napi::PropertyDescriptor::Value(
        "_instanceProxy", instanceProxy, napi_default));
...
Napi::Object WrappedObject<ClassType>::create_instance(Napi::Env env,
  ...
  if (constructor.IsEmpty() || factory_constructor.IsEmpty()) {
    std::string typeName(typeid(ClassType).name());
    std::string errorMessage = util::format(
        "create_instance: Class %s not initialized. Call init() first",
        typeName);
    throw Napi::Error::New(env, errorMessage);
  }
  Napi::EscapableHandleScope scope(env);
...
template <typename ClassType>
WrappedObject<ClassType> *
WrappedObject<ClassType>::try_unwrap(const Napi::Object &object) {
  Napi::Env env = object.Env();

  WrappedObject<ClassType> *unwrapped;
  napi_status status =
      napi_unwrap(env, object, reinterpret_cast<void **>(&unwrapped));
  if ((status) != napi_ok) {
...
template <typename ClassType>
void WrappedObject<ClassType>::set_factory_constructor(
    const Napi::Function &factoryConstructor) {
...
template <typename ClassType>
inline bool WrappedObject<ClassType>::is_instance(Napi::Env env,
                                                  const Napi::Object &object) {
  if (constructor.IsEmpty()) {
    std::string typeName(typeid(ClassType).name());
    std::string errorMessage = util::format(
        "is_instance: Class %s not initialized. Call init() first", typeName);
    throw Napi::Error::New(env, errorMessage);
  }

  Napi::HandleScope scope(env);

  // Check the object is instance of the constructor. This will be true when the
  // object have it's prototype set with setPrototypeOf. This is true for objects
  // configured in the schema with a function type.
  ...
  // Object store needs is_instance to return true when called with RealmObject
  // instance even if the prototype was changed with setPrototypeOf
  Napi::Object instance = object.Get("_instance").As<Napi::Object>();
  ...
  // In this case we just check for existing internal value to identify
  // RealmObject instances
  if (!isInstanceOf) {
    bool isRealmObjectClass =
        std::is_same<ClassType,
                     realm::js::RealmObjectClass<realm::node::Types>>::value;
    if (isRealmObjectClass) {
      Napi::External<typename ClassType::Internal> external =
          object.Get(ExternalSymbol)
              .As<Napi::External<typename ClassType::Internal>>();
...
static inline Napi::Value method_callback(const Napi::CallbackInfo &info) {
  node::Types::FunctionCallback method =
      (node::Types::FunctionCallback)info.Data();
  return method(info);
}
...
template <typename ClassType>
Napi::Value
WrappedObject<ClassType>::method_callback(const Napi::CallbackInfo &info) {
  node::Types::FunctionCallback method =
      (node::Types::FunctionCallback)info.Data();
  return method(info);
}
...
template <typename ClassType>
Napi::Value
WrappedObject<ClassType>::getter_callback(const Napi::CallbackInfo &info) {
  PropertyType *propertyType = (PropertyType *)info.Data();
  return propertyType->getter(info);
}

template <typename ClassType>
void WrappedObject<ClassType>::setter_callback(const Napi::CallbackInfo &info,
                                               const Napi::Value &value) {
  PropertyType *propertyType = (PropertyType *)info.Data();
...
template <typename ClassType>
void WrappedObject<ClassType>::readonly_setter_callback(
    const Napi::CallbackInfo &info, const Napi::Value &value) {
  Napi::Error error =
      Napi::Error::New(info.Env(), "Cannot assign to read only property");
  error.Set("readOnly", true);
  throw error;
}

template <typename ClassType>
void WrappedObject<ClassType>::readonly_static_setter_callback(
    const Napi::CallbackInfo &info, const Napi::Value &value) {
  Napi::Error error = Napi::Error::New(
      info.Env(), "Cannot assign to read only static property");
  error.Set("readOnly", true);
  throw error;
}
...
template <typename ClassType>
Napi::Value WrappedObject<ClassType>::ProxyHandler::get_instance_proxy_handler(
    Napi::Env env) {
  ...
  Napi::PropertyDescriptor instanceGetTrapFunc =
      Napi::PropertyDescriptor::Function(
          "get", &WrappedObject<ClassType>::ProxyHandler::get_proxy_trap);
  Napi::PropertyDescriptor instanceSetTrapFunc =
      Napi::PropertyDescriptor::Function(
          "set", &WrappedObject<ClassType>::ProxyHandler::set_proxy_trap);
  Napi::PropertyDescriptor ownKeysTrapFunc = Napi::PropertyDescriptor::Function(
      "ownKeys", &WrappedObject<ClassType>::ProxyHandler::own_keys_proxy_trap);
  Napi::PropertyDescriptor hasTrapFunc = Napi::PropertyDescriptor::Function(
      "has", &WrappedObject<ClassType>::ProxyHandler::has_proxy_trap);
  Napi::PropertyDescriptor getOwnPropertyDescriptorTrapFunc =
      Napi::PropertyDescriptor::Function(
          "getOwnPropertyDescriptor",
          &WrappedObject<
              ClassType>::ProxyHandler::get_own_property_descriptor_trap);
  Napi::PropertyDescriptor getPrototypeOfFunc =
      Napi::PropertyDescriptor::Function(
          "getPrototypeOf",
          &WrappedObject<ClassType>::ProxyHandler::get_prototype_of_proxy_trap);
  Napi::PropertyDescriptor setPrototypeOfFunc =
      Napi::PropertyDescriptor::Function(
          "setPrototypeOf",
          &WrappedObject<ClassType>::ProxyHandler::set_prototype_of_proxy_trap);

  proxyObject.DefineProperties({instanceGetTrapFunc, instanceSetTrapFunc,
                                ownKeysTrapFunc, hasTrapFunc,
                                getOwnPropertyDescriptorTrapFunc,
                                getPrototypeOfFunc, setPrototypeOfFunc});
...
static Napi::Value bind_function(Napi::Env env, const std::string &functionName,
                                 const Napi::Function &function,
                                 const Napi::Object &thisObject) {
  Napi::Function boundFunc =
      FunctionBind.Call(function, {thisObject}).As<Napi::Function>();
  return boundFunc;
}

template <typename ClassType>
Napi::Value WrappedObject<ClassType>::ProxyHandler::bind_native_function(
    Napi::Env env, const std::string &functionName,
    const Napi::Function &function, const Napi::Object &thisObject) {
  Napi::EscapableHandleScope scope(env);

  // do not bind the non native functions. These are attached from extensions.js
  // and should be called on the instanceProxy.
...
static inline Napi::Object get_prototype(Napi::Env env,
                                         const Napi::Object &object) {
  napi_value napi_proto;
  napi_status status = napi_get_prototype(env, object, &napi_proto);
  if (status != napi_ok) {
    throw Napi::Error::New(env,
                           "Invalid object. Couldn't get prototype of object");
  }
...
template <typename ClassType>
Napi::Value WrappedObject<ClassType>::ProxyHandler::get_proxy_trap(
    const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  Napi::EscapableHandleScope scope(env);
...
#if DEBUG
  std::string _debugproperty =
      property.IsString() ? (std::string)property.As<Napi::String>() : "";
  ...
  if (isNumber) {
    ...
    WrappedObject<ClassType> *wrappedObject =
        WrappedObject<ClassType>::Unwrap(instance);
    Napi::Value result =
        wrappedObject->m_indexPropertyHandlers->getter(info, instance, index);
    return scope.Escape(result);
  }
  ...
  if (m_has_native_methodFunc(propertyText)) {
    ...
    Napi::Value propertyValue = instance.Get(property);
    Napi::Value result = bind_function(
        env, propertyText, propertyValue.As<Napi::Function>(), instance);
    return scope.Escape(result);
  }
...
template <typename ClassType>
Napi::Value WrappedObject<ClassType>::ProxyHandler::set_proxy_trap(
    const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  Napi::EscapableHandleScope scope(env);
  ...
  try {
    ...
    if (!readOnly.IsUndefined() && readOnly) {
      std::string message =
          "Cannot assign to read only property '" + propertyText + "'";
      throw Napi::Error::New(env, message);
    }
...
template <typename ClassType>
Napi::Value WrappedObject<ClassType>::ProxyHandler::own_keys_proxy_trap(
    const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  Napi::EscapableHandleScope scope(env);
...
template <typename ClassType>
Napi::Value WrappedObject<ClassType>::ProxyHandler::has_proxy_trap(
    const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  Napi::EscapableHandleScope scope(env);
...
template <typename ClassType>
Napi::Value
WrappedObject<ClassType>::ProxyHandler::get_own_property_descriptor_trap(
    const Napi::CallbackInfo &info) {
  // This exists only for ownKeysTrap to work properly with Object.keys(). It
  // does not check if the property is from the named handler or it is an
  // existing property on the instance. This implementation can be extended to
  // return the true property descriptor if the property is an existing one
...
template <typename ClassType>
Napi::Value WrappedObject<ClassType>::ProxyHandler::get_prototype_of_proxy_trap(
    const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  Napi::EscapableHandleScope scope(env);
...
template <typename ClassType>
Napi::Value WrappedObject<ClassType>::ProxyHandler::set_prototype_of_proxy_trap(
    const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  throw Napi::Error::New(
      env, "Setting the prototype on this type of object is not supported");
}
...
template <typename ClassType>
Napi::Value
ObjectWrap<ClassType>::property_getter(const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  auto &s_class = get_class();
  try {
    auto propertyName = (node::String *)info.Data();
    return s_class.string_accessor.getter(info, info.This().As<Napi::Object>(),
                                          propertyName->ToString(env));
  } catch (const Napi::Error &e) {
    e.ThrowAsJavaScriptException();
    return env.Undefined();
  }
...
void ObjectWrap<ClassType>::property_setter(const Napi::CallbackInfo &info) {
  ...
  try {
    auto propertyName = (node::String *)info.Data();
    auto value = info[0];
    s_class.string_accessor.setter(info, info.This().As<Napi::Object>(),
                                   propertyName->ToString(env), value);
  } catch (const Napi::Error &e) {
    e.ThrowAsJavaScriptException();
  }
}
...
Napi::Function ObjectWrap<ClassType>::create_constructor(Napi::Env env) {
  ...
  // If the class has no index accessor we can create an instance of the class
  // itself and can skip proxy objects
  bool has_index_accessor =
      (s_class.index_accessor.getter || s_class.index_accessor.setter);
  bool isRealmObjectClass =
      std::is_same<ClassType,
                   realm::js::RealmObjectClass<realm::node::Types>>::value;
  ...
  // since NAPI ctors can't change the returned type we need to return a factory
  // func which will be called when 'new ctor()' is called from JS. This will
  // create a JS Proxy and return it to the caller. The proxy is needed to
  // support named and index handlers
  Napi::Function factory = Napi::Function::New(
      env, WrappedObject<ClassType>::create_instance_with_proxy, s_class.name);
  auto ctorPrototypeProperty = ctor.Get("prototype");

  // the factory function should have the same prototype property as the
  // constructor.prototype so `instanceOf` works
...
Napi::Function ObjectWrap<ClassType>::init_class(Napi::Env env) {
  auto &s_class = get_class();
  // check if the constructor is already created. It means this class and it's
  // parent are already initialized.
  ...
  std::vector<Napi::ClassPropertyDescriptor<WrappedObject<ClassType>>>
      properties;
  ...
  bool has_index_accessor =
      (s_class.index_accessor.getter || s_class.index_accessor.setter);
  const IndexPropertyType *index_accessor =
      has_index_accessor ? &s_class.index_accessor : nullptr;

  ctor = WrappedObject<ClassType>::init(
      env, s_class.name, &ObjectWrap<ClassType>::constructor_callback,
  ...
  Napi::Function parentCtor =
      ObjectWrapAccessor<ParentClassType>::init_class(env);
  if (!parentCtor.IsEmpty()) {
    ...
    ObjectSetPrototypeOf.Call(
        {ctorPrototypeProperty, parentCtorPrototypeProperty});
    ObjectSetPrototypeOf.Call({ctor, parentCtor});
  }

  // use PropertyDescriptors instead of ClassPropertyDescriptors, since
  // ClassPropertyDescriptors requires the JS instance members callbacks to be
  // instance members of the WrappedObject<ClassType> class
  if (isRealmObjectClass) {
    ...
    for (auto &pair : s_class.methods) {
      auto descriptor = Napi::PropertyDescriptor::Function(
          env, ctorPrototype, Napi::String::New(env, pair.first) /*name*/,
          &method_callback,
          napi_default | realm::js::PropertyAttributes::DontEnum,
    ...
    for (auto &pair : s_class.properties) {
      napi_property_attributes napi_attributes =
          napi_default | (realm::js::PropertyAttributes::DontEnum |
                          realm::js::PropertyAttributes::DontDelete);
      auto descriptor =
          Napi::PropertyDescriptor::Accessor<property_getter_callback>(
  ...
  bool isRealmClass =
      std::is_same<ClassType, realm::js::RealmClass<realm::node::Types>>::value;
  if (isRealmClass) {
...
template <typename ClassType>
Napi::Object ObjectWrap<ClassType>::create_instance(Napi::Env env,
                                                    Internal *internal) {
  Napi::EscapableHandleScope scope(env);
  ...
  if (isRealmObjectClass && !internal) {
    throw Napi::Error::New(env, "RealmObjectClass requires an internal realm "
                                "object when creating instances");
  }

  Napi::Object factory =
      WrappedObject<ClassType>::create_instance(env, internal);
  return scope.Escape(factory).As<Napi::Object>();
}

inline static void
schema_object_type_constructor(const Napi::CallbackInfo &info) {}

template <typename ClassType>
void ObjectWrap<ClassType>::internal_finalizer(
    Napi::Env, typename ClassType::Internal *internal) {
  if (internal) {
    delete internal;
    internal = nullptr;
  }
...
static inline void remove_schema_object(
    std::unordered_map<std::string, SchemaObjectType *> *schemaObjects,
...
ObjectWrap<ClassType>::create_napi_property_descriptors(
    Napi::Env env, const Napi::Object &constructorPrototype,
  ...
  for (auto &property : schema.persisted_properties) {
    std::string propName =
        property.public_name.empty() ? property.name : property.public_name;
    if (redefine || !constructorPrototype.HasOwnProperty(propName)) {
      node::String *name = get_cached_property_name(propName);
      auto descriptor =
          Napi::PropertyDescriptor::Accessor<property_getter, property_setter>(
...
template <typename ClassType>
Napi::Object ObjectWrap<ClassType>::create_instance_by_schema(
    Napi::Env env, Napi::Function &constructor,
  ...
  if (isRealmObjectClass && !internal) {
    throw Napi::Error::New(env, "RealmObjectClass requires an internal realm "
                                "object when creating instances by schema");
  }
  ...
  // if we are creating a RealmObject from schema with no user defined
  // constructor
  if (constructor.IsEmpty()) {
    // 1.Check by name if the constructor is already created for this
    // RealmObject
    if (!schemaObjects->count(schemaName)) {
      ...
      schemaObjectConstructor =
          Napi::Function::New(env, schema_object_type_constructor, schema.name);

      Napi::Function realmObjectClassConstructor =
          ObjectWrap<ClassType>::create_constructor(env);
      auto parentCtorPrototypeProperty =
          realmObjectClassConstructor.Get("prototype");
      auto childPrototypeProperty =
          schemaObjectConstructor.Get("prototype").As<Napi::Object>();
      ObjectSetPrototypeOf.Call(
          {childPrototypeProperty, parentCtorPrototypeProperty});
      ObjectSetPrototypeOf.Call(
          {schemaObjectConstructor, realmObjectClassConstructor});

      // get all properties from the schema
      std::vector<Napi::PropertyDescriptor> properties =
          create_napi_property_descriptors(env, childPrototypeProperty, schema,
                                           true /*redefine*/);
      ...
      // hot path. The constructor for this schema object is already cached. use
      // it and return a new instance
      SchemaObjectType *schemaObjectType = schemaObjects->at(schemaName);
      schemaObjectConstructor = schemaObjectType->constructor.Value();
    }

    Napi::External<Internal> externalValue =
        Napi::External<Internal>::New(env, internal, internal_finalizer);
    ...
    Napi::Object constructorPrototype =
        constructor.Get("prototype").As<Napi::Object>();

    // get all properties from the schema
    std::vector<Napi::PropertyDescriptor> properties =
        create_napi_property_descriptors(env, constructorPrototype, schema,
                                         false /*redefine*/);

    Napi::Function realmObjectClassConstructor =
        ObjectWrap<ClassType>::create_constructor(env);
    bool isInstanceOfRealmObjectClass =
        constructorPrototype.InstanceOf(realmObjectClassConstructor);

    // Skip if the user defined constructor inherited the RealmObjectClass. All
    // RealmObjectClass members are available already.
    if (!isInstanceOfRealmObjectClass) {
      ...
      for (auto &pair : s_class.methods) {
        ...
        if (!constructorPrototype.HasOwnProperty(pair.first)) {
          auto descriptor = Napi::PropertyDescriptor::Function(
              env, constructorPrototype,
              Napi::String::New(env, pair.first) /*name*/, &method_callback,
              napi_default | realm::js::PropertyAttributes::DontEnum,
              (void *)pair.second /*callback*/);
          properties.push_back(descriptor);
        }
    ...
    if (!instance.InstanceOf(schemaObjectConstructor)) {
      throw Napi::Error::New(
          env, "Realm object constructor must not return another value");
    }
...
template <typename ClassType>
inline void ObjectWrap<ClassType>::on_context_destroy(Napi::Env env,
...
template <typename ClassType>
inline bool ObjectWrap<ClassType>::is_instance(Napi::Env env,
                                               const Napi::Object &object) {
...
ObjectWrap<ClassType>::get_internal(Napi::Env env, const Napi::Object &object) {
  bool isRealmObjectClass =
      std::is_same<ClassType,
                   realm::js::RealmObjectClass<realm::node::Types>>::value;
  if (isRealmObjectClass) {
  ...
  WrappedObject<ClassType> *wrappedObject =
      WrappedObject<ClassType>::try_unwrap(object);
  return wrappedObject->get_internal();
}

template <typename ClassType>
void ObjectWrap<ClassType>::set_internal(
    Napi::Env env, const Napi::Object &object,
    typename ClassType::Internal *internal) {
  bool isRealmObjectClass =
      std::is_same<ClassType,
                   realm::js::RealmObjectClass<realm::node::Types>>::value;
  if (isRealmObjectClass) {
    ...
    obj.Set(ExternalSymbol,
            Napi::External<typename ClassType::Internal>::New(env, internal));
    return;
  }
...
template <typename ClassType>
Napi::Value
ObjectWrap<ClassType>::constructor_callback(const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  Napi::EscapableHandleScope scope(env);
  auto &s_class = get_class();
  ...
  if (reinterpret_cast<void *>(s_class.constructor) != nullptr) {
    ...
    return scope.Escape(
        env.Null()); // return a value to comply with Napi::FunctionCallback
  } else {
    bool isRealmObjectClass =
        std::is_same<ClassType,
                     realm::js::RealmObjectClass<realm::node::Types>>::value;
...
ObjectWrap<ClassType>::setup_method(Napi::Env env, const std::string &name,
                                    node::Types::FunctionCallback callback) {
  auto &s_nativeMethods = get_nativeMethods();
  auto methodCallback =
      (typename WrappedObject<ClassType>::InstanceMethodCallback)(
          &WrappedObject<ClassType>::method_callback);
  s_nativeMethods.insert(name);
  return WrappedObject<ClassType>::InstanceMethod(
      name.c_str(), methodCallback,
      napi_default | realm::js::PropertyAttributes::DontEnum, (void *)callback);
}

template <typename ClassType>
Napi::ClassPropertyDescriptor<WrappedObject<ClassType>>
ObjectWrap<ClassType>::setup_static_method(
    Napi::Env env, const std::string &name,
    node::Types::FunctionCallback callback) {
...
template <typename ClassType>
Napi::ClassPropertyDescriptor<WrappedObject<ClassType>>
ObjectWrap<ClassType>::setup_property(Napi::Env env, const std::string &name,
  ...
  auto getterCallback =
      (typename WrappedObject<ClassType>::InstanceGetterCallback)(
          &WrappedObject<ClassType>::getter_callback);
  auto setterCallback =
      (typename WrappedObject<ClassType>::InstanceSetterCallback)(
          &WrappedObject<ClassType>::readonly_setter_callback);
  if (property.setter) {
    setterCallback =
        (typename WrappedObject<ClassType>::InstanceSetterCallback)(
            &WrappedObject<ClassType>::setter_callback);
  }

  return WrappedObject<ClassType>::InstanceAccessor(
      name.c_str(), getterCallback, setterCallback, napi_attributes,
      (void *)&property);
}

template <typename ClassType>
Napi::ClassPropertyDescriptor<WrappedObject<ClassType>>
ObjectWrap<ClassType>::setup_static_property(Napi::Env env,
  ...
  auto getterCallback =
      (typename WrappedObject<ClassType>::StaticGetterCallback)(
          property.getter);
  ...
  if (property.setter) {
    setterCallback = (typename WrappedObject<ClassType>::StaticSetterCallback)(
        property.setter);
  }

  return WrappedObject<ClassType>::StaticAccessor(
      name.c_str(), getterCallback, setterCallback, napi_attributes, nullptr);
}
...
Napi::Value wrap(const Napi::CallbackInfo &info) {
  ...
  Napi::Object instanceProxy =
      info.This().As<Napi::Object>().Get("_instanceProxy").As<Napi::Object>();
