 ...
 * @param options
 */
async function describeRoundtrip({
  typeName,
  value,
  ...
  realm.write(() => {
    stuff.value = typeof value === "function" ? value(this.realm) : value;
    stuff.realm.create<MixedClass>("MixedClass", {
      _id: stuff._id,
      value: stuff.value,
  ...
  const obj = realm.objectForPrimaryKey<MixedClass>("MixedClass", this._id);
