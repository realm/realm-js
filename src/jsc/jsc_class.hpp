...
public:
    ...
    static JSObjectRef create_instance_by_schema(JSContextRef ctx, const realm::ObjectSchema& schema,
    ...
    static JSObjectRef create_instance_by_schema(JSContextRef ctx, JSObjectRef& constructor,
...
private:
    ...
    ObjectWrap(Internal* object = nullptr)
        : m_object(object)
    {
    }
    ...
    static bool set_readonly_property(JSContextRef ctx, JSObjectRef object, JSStringRef property, JSValueRef value,
                                      JSValueRef* exception)
    {
        *exception = Exception::value(ctx, std::string("Cannot assign to read only property '") +
    ...
    static void define_native_property_accessor(JSContextRef ctx, JSObjectRef& target, jsc::String* name,
                                                JSObjectGetPropertyCallback getCallback);
    static JSValueRef native_property_getter_callback(JSContextRef ctx, JSObjectRef function, JSObjectRef thisObject,
                                                      size_t argumentCount, const JSValueRef arguments[],
                                                      JSValueRef* exception);
    static JSValueRef accessor_getter(JSContextRef ctx, JSObjectRef function, JSObjectRef this_object, size_t argc,
                                      const JSValueRef arguments[], JSValueRef* exception);
    static JSValueRef accessor_setter(JSContextRef ctx, JSObjectRef function, JSObjectRef this_object, size_t argc,
                                      const JSValueRef arguments[], JSValueRef* exception);
};
...
template <typename ClassType>
std::unordered_map<std::string, std::unordered_map<std::string, SchemaObjectType*>*>
...
template <typename ClassType>
inline JSValueRef ObjectWrap<ClassType>::accessor_getter(JSContextRef ctx, JSObjectRef function,
...
template <typename ClassType>
inline JSValueRef ObjectWrap<ClassType>::accessor_setter(JSContextRef ctx, JSObjectRef function,
                                                         JSObjectRef this_object, size_t argc,
                                                         const JSValueRef arguments[], JSValueRef* exception)
{
    bool isRealmObjectClass = std::is_same<ClassType, realm::js::RealmObjectClass<realm::jsc::Types>>::value;
...
template <typename ClassType>
inline JSValueRef ObjectWrap<ClassType>::native_property_getter_callback(JSContextRef ctx, JSObjectRef function,
...
template <typename ClassType>
void ObjectWrap<ClassType>::define_accessor_for_schema_property(JSContextRef ctx, JSObjectRef& target,
                                                                jsc::String* name)
{
    ...
    // set the property name as private data. In the future this could be the realm::Property or the table index to
    // speed up the get operation
    JSObjectRef getter = JSObjectMake(ctx, m_getterAccessorClass, name);
    JSObjectSetPrototype(ctx, getter, FunctionPrototype);

    JSObjectSetProperty(ctx, descriptor, JSStringCreateWithUTF8CString("get"), getter, kJSPropertyAttributeReadOnly,
                        &exception);
    if (exception) {
    ...
    JSObjectSetProperty(ctx, descriptor, JSStringCreateWithUTF8CString("set"), setter, kJSPropertyAttributeReadOnly,
                        &exception);
    if (exception) {
        throw jsc::Exception(ctx, exception);
    }

    JSObjectSetProperty(ctx, descriptor, JSStringCreateWithUTF8CString("enumerable"), Value::from_boolean(ctx, true),
...
template <typename ClassType>
void ObjectWrap<ClassType>::define_native_property_accessor(JSContextRef ctx, JSObjectRef& target, jsc::String* name,
                                                            JSObjectGetPropertyCallback getCallback)
{
    ...
    JSObjectSetPrototype(ctx, getter, FunctionPrototype);
    JSObjectSetProperty(ctx, getter, JSStringCreateWithUTF8CString("propertyName"), Value::from_string(ctx, *name),
                        kJSPropertyAttributeReadOnly, &exception);
    if (exception) {
        throw jsc::Exception(ctx, exception);
    }
...
static void define_function_property(JSContextRef ctx, JSObjectRef& target, const char* name,
                                     const JSObjectCallAsFunctionCallback& callback)
{
    ...
    JSObjectSetProperty(ctx, descriptor, JSStringCreateWithUTF8CString("writable"), Value::from_boolean(ctx, true),
                        kJSPropertyAttributeNone, &exception);
    if (exception) {
        throw jsc::Exception(ctx, exception);
    }

    JSObjectSetProperty(ctx, descriptor, JSStringCreateWithUTF8CString("configurable"),
...
static inline void remove_schema_object(JSContextRef ctx,
...
bool ObjectWrap<ClassType>::has_instance(JSContextRef ctx, JSValueRef value)
{
    bool isRealmObjectClass = std::is_same<ClassType, realm::js::RealmObjectClass<realm::jsc::Types>>::value;
    if (isRealmObjectClass) {
        // Can't use JSValueIsObjectOfClass for RealmObjectClass instances created from a user defined constructor in
        // the schema. Can't use JSValueIsInstanceOfConstructor with the RealmObjectClass constructor since it will
        // recursively call this method again Check if the object has RealmObjectClassConstructorPrototype in its
        // proto chain (the definition of JS 'instanceof')
        ...
        // then this is not an RealmObject instance
        return false;
    }
...
template <typename ClassType>
bool ObjectWrap<ClassType>::has_instance(JSContextRef ctx, JSObjectRef constructor, JSValueRef value,
                                         JSValueRef* exception)
{
...
template <typename ClassType>
inline void ObjectWrap<ClassType>::define_schema_properties(JSContextRef ctx, JSObjectRef constructorPrototype,
                                                            const realm::ObjectSchema& schema, bool redefine)
{
    ...
    for (auto& property : schema.persisted_properties) {
        ...
        if (redefine ||
            !JSObjectHasProperty(ctx, constructorPrototype, JSStringCreateWithUTF8CString(propName.c_str()))) {
...
template <typename ClassType>
inline JSObjectRef ObjectWrap<ClassType>::create_instance_by_schema(JSContextRef ctx, JSObjectRef& constructor,
                                                                    const realm::ObjectSchema& schema,
                                                                    typename ClassType::Internal* internal)
{
    bool isRealmObjectClass = std::is_same<ClassType, realm::js::RealmObjectClass<realm::jsc::Types>>::value;
    if (!isRealmObjectClass) {
        JSValueRef exception =
            jsc::Exception::value(ctx, "Creating instances by schema is supported for RealmObjectClass only");
        throw jsc::Exception(ctx, exception);
    }

    if (isRealmObjectClass && !internal) {
        JSValueRef exception = jsc::Exception::value(
            ctx, "RealmObjectClass requires an internal realm object when creating instances by schema");
        throw jsc::Exception(ctx, exception);
    }
    ...
    if (constructor == nullptr) {
        ...
        bool isInstanceOfRealmObjectClass =
            JSValueIsInstanceOfConstructor(ctx, constructorPrototype, RealmObjectClassConstructor, &exception);
        if (exception) {
            throw jsc::Exception(ctx, exception);
        }
        ...
        if (!isInstanceOfRealmObjectClass) {
            ...
            for (auto& pair : s_class.methods) {
                ...
                if (!JSObjectHasProperty(ctx, constructorPrototype,
...
template <typename ClassType>
inline JSValueRef ObjectWrap<ClassType>::call(JSContextRef ctx, JSObjectRef function, JSObjectRef this_object,
...
template <typename ClassType>
inline JSObjectRef ObjectWrap<ClassType>::construct(JSContextRef ctx, JSObjectRef constructor, size_t argc,
                                                    const JSValueRef arguments[], JSValueRef* exception)
{
    if (!reinterpret_cast<void*>(s_class.constructor)) {
...
inline void ObjectWrap<ClassType>::initialize_constructor(JSContextRef ctx, JSObjectRef constructor)
{
    ...
    Object::set_prototype(ctx, constructor,
                          Object::get_prototype(ctx, JSObjectMakeFunctionWithCallback(ctx, nullptr, call)));

    // Set the constructor prototype to be the prototype generated from the instance JSClassRef.
    JSObjectRef prototype =
        Object::validated_get_object(ctx, JSObjectMakeConstructor(ctx, get_class(), construct), prototype_string);
...
template <typename ClassType>
inline void ObjectWrap<ClassType>::get_property_names(JSContextRef ctx, JSObjectRef object,
...
template <typename ClassType>
inline JSValueRef ObjectWrap<ClassType>::get_property(JSContextRef ctx, JSObjectRef object, JSStringRef property,
                                                      JSValueRef* exception)
{
...
template <typename ClassType>
inline bool ObjectWrap<ClassType>::set_property(JSContextRef ctx, JSObjectRef object, JSStringRef property,
    ...
    if (index_setter || s_class.index_accessor.getter) {
        ...
        if (try_get_int(property, num)) {
            ...
            if (util::int_cast_with_overflow_detect(num, index)) {
                *exception = Exception::value(ctx, util::format("Index %1 cannot be greater than %2.", num,
