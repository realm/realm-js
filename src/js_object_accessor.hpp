...
public:
    ...
    ValueType box(Obj obj) {
        ...
        return RealmObjectClass<JSEngine>::create_instance(m_ctx, realm::Object(m_realm, *m_object_schema, obj.get_key()));
    }
...
struct Unbox<JSEngine, ObjKey> {
    static ObjKey call(NativeAccessor<JSEngine> *ctx, typename JSEngine::Value const& value, bool create, bool try_update) {
        using Value = js::Value<JSEngine>;
        using ValueType = typename JSEngine::Value;
        ...
        if (js::Object<JSEngine>::template is_instance<RealmObjectClass<JSEngine>>(ctx->m_ctx, object)) {
            ...
            if (realm_object->realm() == ctx->m_realm) {
                return realm_object->obj().get_key();
            }
            if (!create) {
