...
using NetworkTransportFactory =
    typename NetworkTransport::NetworkTransportFactory;
...
public:
  ...
  template <typename Fn> json add_task(Fn &&);
...
private:
  ...
  // The key here is the same as the value in m_callbacks. We use the raw
  // pointer as a key here, because protecting the value in m_callbacks pins the
  // function object and prevents it from being moved by the garbage collector
  // upon compaction.
  ...
  std::map<std::pair<uint64_t, uint64_t>, std::promise<json>>
      m_pending_callbacks;

  NetworkTransportFactory previous_transport_generator;

  static JSValueRef run_callback(JSContextRef, JSObjectRef, JSObjectRef, size_t,
...
static const char *const RealmObjectTypesFetchResponseHandler =
    "fetchresponsehandler";
static const char *const RealmObjectTypesEmailPasswordAuth =
    "emailpasswordauth";
...
json serialize_object_schema(const realm::ObjectSchema &object_schema) {
  ...
  for (auto &prop : object_schema.persisted_properties) {
    properties.push_back(prop.public_name.empty() ? prop.name
                                                  : prop.public_name);
  }
...
template <typename Container> json get_type(Container const &c) {
  auto type = c.get_type();
  if (type == realm::PropertyType::Object) {
    return serialize_object_schema(c.get_object_schema());
  }
  return {{"type", js::local_string_for_property_type(type)},
...
RPCWorker::~RPCWorker() { stop(); }

template <typename Fn> json RPCWorker::add_task(Fn &&fn) {
  std::promise<json> p;
...
static json read_object_properties(Object &object) {
  ...
  for (auto &property : object_schema.persisted_properties) {
    ...
    auto cache_value = [&](auto &&v) { cache[property.name] = {{"value", v}}; };
    switch (property.type & ~PropertyType::Flags) {
    case PropertyType::Bool:
      cache_value(obj.get<bool>(property.column_key));
      break;
    case PropertyType::Int:
      cache_value(obj.get<int64_t>(property.column_key));
      break;
    case PropertyType::Float:
      cache_value(obj.get<float>(property.column_key));
      break;
    case PropertyType::Double:
      cache_value(obj.get<double>(property.column_key));
      break;
    case PropertyType::Date: {
      ...
      cache[property.name] = {
          ...
          {"value",
           ts.get_seconds() * 1000.0 + ts.get_nanoseconds() / 1000000.0},
      };
      break;
    } break;
    case PropertyType::String: {
      auto str = obj.get<StringData>(property.column_key);
      // A completely abitrary upper limit on how big of a string we'll
      // pre-cache
      if (str.size() < 100) {
        cache_value(str);
      }
    ...
    case PropertyType::Decimal:
      cache[property.name] = {
          ...
          {"value",
           {"$numberDecimal",
            obj.get<Decimal>(property.column_key).to_string()}},
      };
      break;
    case PropertyType::ObjectId:
      cache[property.name] = {
          {"type", RealmObjectTypesEJSON},
          {"value",
           {"$oid", obj.get<ObjectId>(property.column_key).to_string()}},
      };
      break;
...
RPCServerImpl::RPCServerImpl() {
  ...
  AppClass::transport_generator = [](jsc::Types::Context ctx,
                                     NetworkTransport::Dispatcher dispatcher)
      -> std::unique_ptr<app::GenericNetworkTransport> {
    (void)dispatcher; // We don't need to use the dispatcher because JSC
                      // separately guarantees thread-safety.
    return std::make_unique<RPCNetworkTransport>(ctx);
  };

  // JavaScriptCore crashes when trying to walk up the native stack to print the
  // stacktrace.
  // FIXME: Avoid having to do this!
  static void (*setIncludesNativeCallStack)(JSGlobalContextRef, bool) =
      (void (*)(JSGlobalContextRef, bool))dlsym(
          RTLD_DEFAULT,
  ...
  m_requests["/create_session"] = [this](const json dict) {
    ...
    JSObjectRef realm_constructor = jsc::Object::validated_get_constructor(
        m_context, JSContextGetGlobalObject(m_context), realm_string);

    // Enable the RCP network transport to issue calls to the remote fetch
    // function
    jsc::Types::Function fetch_function = Value::validated_to_function(
        m_context, deserialize_json_value(dict["fetch"]), "fetch");
    RPCNetworkTransport::fetch_function =
        js::Protected(m_context, fetch_function);
  ...
  m_requests["/create_realm"] = [this](const json dict) {
    ...
    JSObjectRef realm_object = jsc::Function::construct(
        m_context, realm_constructor, arg_count, arg_values);

    JSObjectRef add_listener_method = (JSObjectRef)jsc::Object::get_property(
        m_context, realm_object, "addListener");
    JSValueRef listener_args[] = {
        jsc::Value::from_string(m_context, "beforenotify"),
        deserialize_json_value(dict["beforeNotify"])};
    jsc::Function::call(m_context, add_listener_method, realm_object, 2,
                        listener_args);
  ...
  m_requests["/create_app"] = [this](const json dict) {
    JSObjectRef realm_constructor = get_realm_constructor();
    JSObjectRef app_constructor = (JSObjectRef)jsc::Object::get_property(
        m_context, realm_constructor, "App");
    ...
    JSObjectRef app_object = jsc::Function::construct(
        m_context, app_constructor, arg_count, arg_values);
    return (json){{"result", serialize_json_value(app_object)}};
  };
  m_requests["/create_user"] = [this](const json dict) {
    JSObjectRef realm_constructor = get_realm_constructor();

    JSObjectRef sync_constructor = (JSObjectRef)jsc::Object::get_property(
        m_context, realm_constructor, "Sync");
    JSObjectRef user_constructor = (JSObjectRef)jsc::Object::get_property(
        m_context, sync_constructor, "User");
    JSObjectRef create_user_method = (JSObjectRef)jsc::Object::get_property(
        m_context, user_constructor, "createUser");
    ...
    JSObjectRef user_object = (JSObjectRef)jsc::Function::call(
        m_context, create_user_method, arg_count, arg_values);
  ...
  m_requests["/call_sync_function"] = [this](const json dict) {
    ...
    JSObjectRef method = (JSObjectRef)jsc::Object::get_property(
        m_context, sync_constructor, name);
  ...
  m_requests["/_asyncOpen"] = [this](const json dict) {
    ...
    JSObjectRef _asyncOpen_method = (JSObjectRef)jsc::Object::get_property(
        m_context, realm_constructor, "_asyncOpen");
    json::array_t args = dict["arguments"];
    size_t arg_count = args.size();
    JSValueRef arg_values[arg_count];
    ...
    auto result = jsc::Function::call(m_context, _asyncOpen_method, arg_count,
                                      arg_values);
    return (json){{"result", serialize_json_value(result)}};
  };
  m_requests["/call_method"] = [this](const json dict) {
    ...
    JSObjectRef function =
        jsc::Object::validated_get_function(m_context, object, method_string);
    ...
    JSValueRef result =
        jsc::Function::call(m_context, function, object, arg_count, arg_values);
    return (json){{"result", serialize_json_value(result)}};
  };
  m_requests["/get_object"] = [this](const json dict) -> json {
    ...
    json result;
    if (jsc::Object::is_instance<js::RealmObjectClass<jsc::Types>>(m_context,
                                                                   object)) {
      auto obj = jsc::Object::get_internal<js::RealmObjectClass<jsc::Types>>(
          m_context, object);
    ...
    if (result.find(name) == result.end()) {
      if (name.is_number()) {
        ...
        result[key] = serialize_json_value(
            jsc::Object::get_property(m_context, object, key));
      } else {
  ...
  m_requests["/get_property"] = [this](const json dict) {
    ...
    if (JSObjectRef object = get_object(oid)) {
      if (name.is_number()) {
        value = jsc::Object::get_property(m_context, object,
  ...
  m_requests["/set_property"] = [this](const json dict) {
    ...
    if (name.is_number()) {
      jsc::Object::set_property(m_context, get_object(oid),
                                name.get<unsigned int>(), value);
    } else {
      jsc::Object::set_property(m_context, get_object(oid),
                                name.get<std::string>(), value);
    }
  ...
  m_requests["/clear_test_state"] = [this](const json dict) {
    // The session ID points to the Realm constructor object, which should
    // remain.
  ...
  m_requests["/set_versions"] = [this](const json dict) {
    JSObjectRef versions = jsc::Value::validated_to_object(
        m_context, deserialize_json_value(dict["versions"]), "versions");
    AppClass::package_version = jsc::Object::validated_get_string(
        m_context, versions, "packageVersion");
    AppClass::platform_context = jsc::Object::validated_get_string(
        m_context, versions, "platformContext");
    AppClass::platform_os =
        jsc::Object::validated_get_string(m_context, versions, "platformOs");
    AppClass::platform_version = jsc::Object::validated_get_string(
        m_context, versions, "platformVersion");
    return json::object();
  };
  m_requests["/_anonymous"] = [this](const json dict) {
    JSObjectRef realm_constructor = get_realm_constructor();
    JSObjectRef credentials_constructor =
        (JSObjectRef)jsc::Object::get_property(m_context, realm_constructor,
                                               "Credentials");
    JSObjectRef anonymous_method = (JSObjectRef)jsc::Object::get_property(
        m_context, credentials_constructor, "anonymous");
    ...
    JSObjectRef credentials_object = (JSObjectRef)jsc::Function::call(
        m_context, anonymous_method, arg_count, arg_values);
  ...
  m_requests["/_facebook"] = [this](const json dict) {
    ...
    JSObjectRef credentials_constructor =
        (JSObjectRef)jsc::Object::get_property(m_context, realm_constructor,
                                               "Credentials");
    JSObjectRef facebook_method = (JSObjectRef)jsc::Object::get_property(
        m_context, credentials_constructor, "facebook");
    ...
    JSObjectRef credentials_object = (JSObjectRef)jsc::Function::call(
        m_context, facebook_method, arg_count, arg_values);
    return (json){{"result", serialize_json_value(credentials_object)}};
  };
  m_requests["/_apple"] = [this](const json dict) {
    ...
    JSObjectRef credentials_constructor =
        (JSObjectRef)jsc::Object::get_property(m_context, realm_constructor,
                                               "Credentials");
    JSObjectRef apple_method = (JSObjectRef)jsc::Object::get_property(
        m_context, credentials_constructor, "apple");
    ...
    JSObjectRef credentials_object = (JSObjectRef)jsc::Function::call(
        m_context, apple_method, arg_count, arg_values);
    return (json){{"result", serialize_json_value(credentials_object)}};
  };
  m_requests["/_emailPassword"] = [this](const json dict) {
    ...
    JSObjectRef credentials_constructor =
        (JSObjectRef)jsc::Object::get_property(m_context, realm_constructor,
                                               "Credentials");
    JSObjectRef email_password_method = (JSObjectRef)jsc::Object::get_property(
        m_context, credentials_constructor, "emailPassword");
    ...
    JSObjectRef credentials_object = (JSObjectRef)jsc::Function::call(
        m_context, email_password_method, arg_count, arg_values);
    return (json){{"result", serialize_json_value(credentials_object)}};
  };
  m_requests["/_function"] = [this](const json dict) {
    ...
    JSObjectRef credentials_constructor =
        (JSObjectRef)jsc::Object::get_property(m_context, realm_constructor,
                                               "Credentials");
    JSObjectRef function_method = (JSObjectRef)jsc::Object::get_property(
        m_context, credentials_constructor, "function");
    ...
    JSObjectRef credentials_object = (JSObjectRef)jsc::Function::call(
        m_context, function_method, arg_count, arg_values);
    return (json){{"result", serialize_json_value(credentials_object)}};
  };
  m_requests["/_google"] = [this](const json dict) {
    ...
    JSObjectRef credentials_constructor =
        (JSObjectRef)jsc::Object::get_property(m_context, realm_constructor,
                                               "Credentials");
    JSObjectRef google_method = (JSObjectRef)jsc::Object::get_property(
        m_context, credentials_constructor, "google");
    ...
    JSObjectRef credentials_object = (JSObjectRef)jsc::Function::call(
        m_context, google_method, arg_count, arg_values);
    return (json){{"result", serialize_json_value(credentials_object)}};
  };
  m_requests["/_userApiKey"] = [this](const json dict) {
    ...
    JSObjectRef credentials_constructor =
        (JSObjectRef)jsc::Object::get_property(m_context, realm_constructor,
                                               "Credentials");
    JSObjectRef user_api_key_method = (JSObjectRef)jsc::Object::get_property(
        m_context, credentials_constructor, "userApiKey");
    ...
    JSObjectRef credentials_object = (JSObjectRef)jsc::Function::call(
        m_context, user_api_key_method, arg_count, arg_values);
    return (json){{"result", serialize_json_value(credentials_object)}};
  };
  m_requests["/_serverApiKey"] = [this](const json dict) {
    ...
    JSObjectRef credentials_constructor =
        (JSObjectRef)jsc::Object::get_property(m_context, realm_constructor,
                                               "Credentials");
    JSObjectRef server_api_key_method = (JSObjectRef)jsc::Object::get_property(
        m_context, credentials_constructor, "serverApiKey");
    ...
    JSObjectRef credentials_object = (JSObjectRef)jsc::Function::call(
        m_context, server_api_key_method, arg_count, arg_values);
    return (json){{"result", serialize_json_value(credentials_object)}};
  };
  m_requests["/_jwt"] = [this](const json dict) {
    ...
    JSObjectRef credentials_constructor =
        (JSObjectRef)jsc::Object::get_property(m_context, realm_constructor,
                                               "Credentials");
    JSObjectRef jwt_method = (JSObjectRef)jsc::Object::get_property(
        m_context, credentials_constructor, "jwt");
    ...
    JSObjectRef credentials_object = (JSObjectRef)jsc::Function::call(
        m_context, jwt_method, arg_count, arg_values);
    return (json){{"result", serialize_json_value(credentials_object)}};
  };
}
 ...
 */
JSValueRef RPCServerImpl::run_callback(JSContextRef ctx, JSObjectRef function,
                                       JSObjectRef this_object, size_t argc,
  ...
  JSObjectRef arguments_array =
      jsc::Object::create_array(ctx, uint32_t(argc), arguments);
  ...
  // This captures references since it will be executed before exiting this
  // function.
  server->m_worker.invoke_callback({{"callback", callback_id},
                                    {"this", this_json},
  ...
  if (!error.is_null()) {
    JSStringRef message =
        JSStringCreateWithUTF8CString(error.get<std::string>().c_str());
    ...
    *exception = error;
    ...
    if (stack.is_string()) {
      JSStringRef stack_json =
          JSStringCreateWithUTF8CString(stack.get<std::string>().c_str());
...
json RPCServerImpl::perform_request(std::string const &name, json &&args) {
  ...
  // Only create_session is allowed without the correct session id (since it
  // creates the session id).
  if (name != "/create_session" &&
      m_session_id != args["sessionId"].get<RPCObjectID>()) {
  ...
  // The callback_result message contains the return value (or exception) of a
  // callback ran by run_callback().
  ...
  return m_worker.add_task([=] {
    try {
      ...
      try {
        ...
        exceptionAsJson = {
            {"error", "An exception occured while processing the request. "
                      "Could not serialize the exception as JSON"}};
      }
      return (json){
...
bool RPCServerImpl::try_run_task() { return m_worker.try_run_task(); }
...
JSObjectRef RPCServerImpl::get_realm_constructor() const {
  JSObjectRef realm_constructor =
      m_session_id ? JSObjectRef(get_object(m_session_id)) : NULL;
  if (!realm_constructor) {
...
json RPCServerImpl::serialize_json_value(JSValueRef js_value) {
  ...
  if (jsc::Object::is_instance<js::RealmObjectClass<jsc::Types>>(m_context,
                                                                 js_object)) {
    auto object = jsc::Object::get_internal<js::RealmObjectClass<jsc::Types>>(
        m_context, js_object);
    return {{"type", RealmObjectTypesObject},
            {"id", store_object(js_object)},
            {"schema", serialize_object_schema(object->get_object_schema())},
            {"cache", read_object_properties(*object)}};
  } else if (jsc::Object::is_instance<js::ListClass<jsc::Types>>(m_context,
                                                                 js_object)) {
    auto list = jsc::Object::get_internal<js::ListClass<jsc::Types>>(m_context,
                                                                     js_object);
    return {
        {"type", RealmObjectTypesList},
        {"id", store_object(js_object)},
        {"dataType", js::local_string_for_property_type(
                         list->get_type() & ~realm::PropertyType::Flags)},
        {"optional", is_nullable(list->get_type())},
    };
  } else if (jsc::Object::is_instance<js::ResultsClass<jsc::Types>>(
                 m_context, js_object)) {
    auto results = jsc::Object::get_internal<js::ResultsClass<jsc::Types>>(
        m_context, js_object);
    return {
        {"type", RealmObjectTypesResults},
        {"id", store_object(js_object)},
        {"dataType", js::local_string_for_property_type(
                         results->get_type() & ~realm::PropertyType::Flags)},
        {"optional", is_nullable(results->get_type())},
    };
  } else if (jsc::Object::is_instance<js::RealmClass<jsc::Types>>(m_context,
                                                                  js_object)) {
    auto realm = jsc::Object::get_internal<js::RealmClass<jsc::Types>>(
        m_context, js_object);
    json realm_dict{
        {"_isPartialRealm", serialize_json_value(jsc::Object::get_property(
                                m_context, js_object, "_isPartialRealm"))},
        {"inMemory", serialize_json_value(jsc::Object::get_property(
                         m_context, js_object, "inMemory"))},
        {"path", serialize_json_value(
                     jsc::Object::get_property(m_context, js_object, "path"))},
        {"readOnly", serialize_json_value(jsc::Object::get_property(
                         m_context, js_object, "readOnly"))},
        {"syncSession", serialize_json_value(jsc::Object::get_property(
                            m_context, js_object, "syncSession"))},
    };
    return {{"type", RealmObjectTypesRealm},
            {"id", store_object(js_object)},
            {"realmId", (uintptr_t)realm->get()},
            {"data", realm_dict}};
  }
#if REALM_ENABLE_SYNC
  else if (jsc::Object::is_instance<js::UserClass<jsc::Types>>(m_context,
                                                               js_object)) {
    return {
        {"type", RealmObjectTypesUser},
        {"id", store_object(js_object)},
    };
  } else if (jsc::Object::is_instance<js::SessionClass<jsc::Types>>(
                 m_context, js_object)) {
    json session_dict{
        {"user", serialize_json_value(
                     jsc::Object::get_property(m_context, js_object, "user"))},
        {"config", serialize_json_value(jsc::Object::get_property(
                       m_context, js_object, "config"))},
    };
    return {{"type", RealmObjectTypesSession},
            {"id", store_object(js_object)},
            {"data", session_dict}};
  } else if (jsc::Object::is_instance<js::AsyncOpenTaskClass<jsc::Types>>(
                 m_context, js_object)) {
    return {
        {"type", RealmObjectTypesAsyncOpenTask},
        {"id", store_object(js_object)},
    };
  } else if (jsc::Object::is_instance<js::AppClass<jsc::Types>>(m_context,
                                                                js_object)) {
    return {
        {"type", RealmObjectTypesApp},
        {"id", store_object(js_object)},
    };
  } else if (jsc::Object::is_instance<js::CredentialsClass<jsc::Types>>(
                 m_context, js_object)) {
    return {
        {"type", RealmObjectTypesCredentials},
        {"id", store_object(js_object)},
    };
  } else if (jsc::Object::is_instance<js::ResponseHandlerClass<jsc::Types>>(
                 m_context, js_object)) {
    return {
        {"type", RealmObjectTypesFetchResponseHandler},
        {"id", store_object(js_object)},
    };
  } else if (jsc::Object::is_instance<js::EmailPasswordAuthClass<jsc::Types>>(
                 m_context, js_object)) {
    return {
    ...
    for (uint32_t i = 0; i < length; i++) {
      array.push_back(serialize_json_value(
          jsc::Object::get_property(m_context, js_object, i)));
    }
    return {{"value", array}};
    ...
    encoded.resize(realm::util::base64_encode(
        data.data(), data.size(), encoded.data(), encoded.capacity()));
    ...
    return {
        ...
        {"message", serialize_json_value(jsc::Object::get_property(
                        m_context, js_object, "message"))},
        {"stack", serialize_json_value(jsc::Object::get_property(
                      m_context, js_object, "stack"))},
    };
  } else if (jsc::Value::is_function(m_context, js_object)) {
    auto it = m_callback_ids.find(js_object);
    if (it != m_callback_ids.end()) {
      return {{"type", RealmObjectTypesFunction}, {"value", it->second}};
    } else {
      return {{"type", RealmObjectTypesFunction}, {"value", it->second}};
    }
    return json::object();
  } else {
    // Serialize this JS object as a plain object since it doesn't match any
    // known types above.
    std::vector<std::string> keys;
    std::vector<json> values;

    // Use the enumarable properties
    std::vector<jsc::String> js_keys =
        jsc::Object::get_property_names(m_context, js_object);
    for (auto &js_key : js_keys) {
      JSValueRef js_value =
          jsc::Object::get_property(m_context, js_object, js_key);
    ...
    if (type_string == RealmObjectTypesFunction) {
      ...
      if (!m_callbacks.count(callback_id)) {
        JSObjectRef callback =
            JSObjectMakeFunctionWithCallback(m_context, nullptr, run_callback);
        m_callbacks.emplace(callback_id,
                            js::Protected<JSObjectRef>(m_context, callback));
      ...
      auto bytes =
          realm::util::base64_decode_to_vector(value.get<std::string>());
      if (!bytes) {
        throw std::runtime_error("Failed to decode base64 encoded data");
      }
      return jsc::Value::from_binary(
          m_context, realm::BinaryData(bytes->data(), bytes->size()));
      ...
      for (auto &el : value.items()) {
        auto el_value =
            jsc::Value::from_string(m_context, el.value().get<std::string>());
...
std::string RPCServer::perform_request(std::string const &name,
                                       std::string const &json_args) {
  return m_impl->perform_request(name, json::parse(json_args)).dump();
}

bool RPCServer::try_run_task() { return m_impl->try_run_task(); }
