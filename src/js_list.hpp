...
template <typename T> class List : public realm::List {
public:
...
struct ListClass : ClassDefinition<T, realm::js::List<T>, CollectionClass<T>> {
  ...
  MethodMap<T> const methods = {
      ...
      {"min",
       wrap<compute_aggregate_on_collection<ListClass<T>, AggregateFunc::Min>>},
      {"max",
       wrap<compute_aggregate_on_collection<ListClass<T>, AggregateFunc::Max>>},
      {"sum",
       wrap<compute_aggregate_on_collection<ListClass<T>, AggregateFunc::Sum>>},
      {"avg",
       wrap<compute_aggregate_on_collection<ListClass<T>, AggregateFunc::Avg>>},
  ...
  IndexPropertyType<T> const index_accessor = {wrap<get_index>,
                                               wrap<set_index>};
...
template <typename T>
typename T::Object ListClass<T>::create_instance(ContextType ctx,
                                                 realm::List list) {
  return create_object<T, ListClass<T>>(
      ctx, new realm::js::List<T>(std::move(list)));
}

template <typename T>
void ListClass<T>::get_length(ContextType ctx, ObjectType object,
...
template <typename T>
void ListClass<T>::get_type(ContextType ctx, ObjectType object,
                            ReturnValue &return_value) {
  auto list = get_internal<T, ListClass<T>>(ctx, object);
  return_value.set(local_string_for_property_type(list->get_type() &
                                                  ~realm::PropertyType::Flags));
}

template <typename T>
void ListClass<T>::get_optional(ContextType ctx, ObjectType object,
                                ReturnValue &return_value) {
  auto list = get_internal<T, ListClass<T>>(ctx, object);
...
template <typename T>
void ListClass<T>::get_index(ContextType ctx, ObjectType object, uint32_t index,
                             ReturnValue &return_value) {
  auto list = get_internal<T, ListClass<T>>(ctx, object);
...
template <typename T>
bool ListClass<T>::set_index(ContextType ctx, ObjectType object, uint32_t index,
                             ValueType value) {
  auto list = get_internal<T, ListClass<T>>(ctx, object);
...
template <typename T>
void ListClass<T>::push(ContextType ctx, ObjectType this_object,
...
template <typename T>
void ListClass<T>::pop(ContextType ctx, ObjectType this_object, Arguments &args,
                       ReturnValue &return_value) {
...
template <typename T>
void ListClass<T>::unshift(ContextType ctx, ObjectType this_object,
                           Arguments &args, ReturnValue &return_value) {
  auto list = get_internal<T, ListClass<T>>(ctx, this_object);
  for (size_t i = 0; i < args.count; i++) {
    validate_value(ctx, *list, args[i]);
...
template <typename T>
void ListClass<T>::shift(ContextType ctx, ObjectType this_object,
                         Arguments &args, ReturnValue &return_value) {
  args.validate_maximum(0);
...
template <typename T>
void ListClass<T>::splice(ContextType ctx, ObjectType this_object,
                          Arguments &args, ReturnValue &return_value) {
  auto list = get_internal<T, ListClass<T>>(ctx, this_object);
...
template <typename T>
void ListClass<T>::snapshot(ContextType ctx, ObjectType this_object,
                            Arguments &args, ReturnValue &return_value) {
  args.validate_maximum(0);
  auto list = get_internal<T, ListClass<T>>(ctx, this_object);
...
template <typename T>
void ListClass<T>::filtered(ContextType ctx, ObjectType this_object,
                            Arguments &args, ReturnValue &return_value) {
  auto list = get_internal<T, ListClass<T>>(ctx, this_object);
...
template <typename T>
void ListClass<T>::sorted(ContextType ctx, ObjectType this_object,
                          Arguments &args, ReturnValue &return_value) {
  auto list = get_internal<T, ListClass<T>>(ctx, this_object);
  return_value.set(ResultsClass<T>::create_instance(
      ctx, list->sort(ResultsClass<T>::get_keypaths(ctx, args))));
}

template <typename T>
void ListClass<T>::is_valid(ContextType ctx, ObjectType this_object,
                            Arguments &args, ReturnValue &return_value) {
  return_value.set(get_internal<T, ListClass<T>>(ctx, this_object)->is_valid());
}

template <typename T>
void ListClass<T>::is_empty(ContextType ctx, ObjectType this_object,
                            Arguments &args, ReturnValue &return_value) {
  return_value.set(get_internal<T, ListClass<T>>(ctx, this_object)->size() ==
                   0);
}

template <typename T>
void ListClass<T>::index_of(ContextType ctx, ObjectType this_object,
                            Arguments &args, ReturnValue &return_value) {
  auto fn = [&](auto &&row) {
    auto list = get_internal<T, ListClass<T>>(ctx, this_object);
    NativeAccessor<T> accessor(ctx, *list);
...
template <typename T>
void ListClass<T>::add_listener(ContextType ctx, ObjectType this_object,
                                Arguments &args, ReturnValue &return_value) {
  auto list = get_internal<T, ListClass<T>>(ctx, this_object);
...
template <typename T>
void ListClass<T>::remove_listener(ContextType ctx, ObjectType this_object,
                                   Arguments &args, ReturnValue &return_value) {
  auto list = get_internal<T, ListClass<T>>(ctx, this_object);
...
template <typename T>
void ListClass<T>::remove_all_listeners(ContextType ctx, ObjectType this_object,
                                        Arguments &args,
                                        ReturnValue &return_value) {
  args.validate_maximum(0);
  auto list = get_internal<T, ListClass<T>>(ctx, this_object);
...
template <typename T>
void ListClass<T>::validate_value(ContextType ctx, realm::List &list,
                                  ValueType value) {
