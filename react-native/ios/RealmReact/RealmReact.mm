    ...
    [bridge
        dispatchBlock:^{
          ...
          realm_jsi_init(rt, exports, ^{
            // Calling jsCallInvokver->invokeAsync tells React Native to execute
            // the lambda passed in on the JS thread, and then flush the
            // internal "microtask queue", which has the effect of flushing any
            // pending UI updates.
            //
            // We call this after we have called into JS from C++, in order to
            // ensure that the RN UI updates in response to any changes from
            // Realm. We need to do this as we bypass the usual RN bridge
            // mechanism for communicating between C++ and JS, so without doing
            // this RN has no way to know that a change has occurred which might
            // require an update (see #4389, facebook/react-native#33006).
            //
            // Calls are debounced using the waitingForUiFlush flag, so if an
            // async flush is already pending when another JS to C++ call
            // happens, we don't call invokeAsync again. This works because the
            // work is performed before the microtask queue is flushed - see
            // sequence diagram at https://bit.ly/3kexhHm. It might be possible
            // to further optimize this, e.g. only flush the queue a maximum of
            // once per frame, but this seems reasonable.
