...
export function _anonymousRPC(args) {
    args = args.map((arg) => serialize(null, arg));
    const result = sendRequest('_anonymous', { arguments: args });
    return deserialize(undefined, result);
}

export function _appleRPC(args) {
    args = args.map((arg) => serialize(null, arg));
...
export function _facebookRPC(args) {
    args = args.map((arg) => serialize(null, arg));
    const result = sendRequest('_facebook', { arguments: args });
    return deserialize(undefined, result);
}

export function _userApiKeyRPC(args) {
    args = args.map((arg) => serialize(null, arg));
    const result = sendRequest('_userAPIKey', { arguments: args });
    return deserialize(undefined, result);
}

export function _serverApiKeyRPC(args) {
    args = args.map((arg) => serialize(null, arg));
