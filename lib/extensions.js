        ...
        Object.defineProperties(realmConstructor.prototype, getOwnPropertyDescriptors({
            ...
            subscriptions(arg) {
                ...
                if (arg) {
                    if (arg.includes('*') || arg.includes('?')) {
                        all_subscriptions = all_subscriptions.filtered(`name LIKE '${arg}'`);
                    } else {
                        all_subscriptions = all_subscriptions.filtered(`name == '${arg}'`);
                    }
                }
                let listOfSubscriptions = [];
                for (var i = 0; i < all_subscriptions.length; ++i) {
                    let matches_property = all_subscriptions[i]['matches_property'];
                    let sub = {
                        name: all_subscriptions[i]['name'],
                        objectType: matches_property.substr(0, matches_property.length-8), // remove _matches
                        query: all_subscriptions[i]['query'],
                    }
            ...
            unsubscribe(name) {
                if (name && typeof(name) === 'string') {
                    ...
                    throw new Error(`string expected - got ${typeof(string)}`);
                }
            }
        }));
    }
