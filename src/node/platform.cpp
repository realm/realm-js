...
public:
  UVException(uv_errno_t error)
      : std::runtime_error(uv_strerror(error)), m_error(error) {}

  const uv_errno_t m_error;
};

struct FileSystemRequest : uv_fs_t {
  ~FileSystemRequest() { uv_fs_req_cleanup(this); }
};
...
void ensure_directory_exists_for_file(const std::string &file_path) {
  ...
  while ((pos = file_path.find_first_of('/', pos)) != std::string::npos) {
    ...
    if (uv_fs_mkdir(uv_default_loop(), &req, dir_path.c_str(), 0755, nullptr) <
            0 &&
...
void remove_realm_files_from_directory(const std::string &dir_path) {
  ...
  if (uv_fs_scandir(uv_default_loop(), &scandir_req, dir_path.c_str(), 0,
                    nullptr) < 0) {
  ...
  while (uv_fs_scandir_next(&scandir_req, &entry) != UV_EOF) {
    ...
    if (entry.type == UV_DIRENT_DIR) {
      ...
      if (ends_with(path, realm_management_extension)) {
        ...
        if (uv_fs_scandir(uv_default_loop(), &management_scandir_req,
                          path.c_str(), 0, nullptr) < 0) {
          throw UVException(static_cast<uv_errno_t>(scandir_req.result));
        }

        while (uv_fs_scandir_next(&management_scandir_req, &management_entry) !=
               UV_EOF) {
          std::string management_entry_path =
              path + '/' + management_entry.name;
          FileSystemRequest delete_req;
          if (uv_fs_unlink(uv_default_loop(), &delete_req,
                           management_entry_path.c_str(), nullptr) != 0) {
            throw UVException(static_cast<uv_errno_t>(delete_req.result));
          }
        }

        FileSystemRequest management_rmdir_req;
        if (uv_fs_rmdir(uv_default_loop(), &management_rmdir_req, path.c_str(),
                        nullptr)) {
          throw UVException(
              static_cast<uv_errno_t>(management_rmdir_req.result));
        }
      }
    } else {
      ...
      if (ends_with(path, realm_extension) ||
          ends_with(path, realm_note_extension) ||
          ends_with(path, realm_lock_extension) ||
          ends_with(path, realm_log_extension) ||
          ends_with(path, realm_log_a_extension) ||
          ends_with(path, realm_log_b_extension)) {
        FileSystemRequest delete_req;
        if (uv_fs_unlink(uv_default_loop(), &delete_req, path.c_str(),
                         nullptr) != 0) {
          throw UVException(static_cast<uv_errno_t>(delete_req.result));
        }
      }
...
void remove_directory(const std::string &path) {
  ...
  if (uv_fs_scandir(uv_default_loop(), &dir_scan_req, path.c_str(), 0,
                    nullptr) < 0) {
  ...
  while (uv_fs_scandir_next(&dir_scan_req, &dir_entry) != UV_EOF) {
    ...
    FileSystemRequest delete_req;
    if (uv_fs_unlink(uv_default_loop(), &delete_req, dir_entry_path.c_str(),
                     nullptr) != 0) {
      throw UVException(static_cast<uv_errno_t>(delete_req.result));
    }
  }
...
void remove_file(const std::string &path) {
  ...
  FileSystemRequest delete_req;
  if (uv_fs_unlink(uv_default_loop(), &delete_req, path.c_str(), nullptr) !=
      0) {
    throw UVException(static_cast<uv_errno_t>(delete_req.result));
  }
}
