...
static inline bool is_realm_file(const char *str) {
  ...
  if (REALM_FILE_FILTER_LEN > lenstr)
    return false;
  return strncmp(str + lenstr - REALM_FILE_FILTER_LEN, REALM_FILE_FILTER,
                 REALM_FILE_FILTER_LEN) == 0;
}
...
std::string default_realm_file_directory() { return s_default_realm_directory; }
...
void copy_bundled_realm_files() {
  ...
  while ((filename = AAssetDir_getNextFileName(assetDir)) != nullptr) {
    if (is_realm_file(filename)) {
      AAsset *asset =
          AAssetManager_open(s_asset_manager, filename, AASSET_MODE_STREAMING);
