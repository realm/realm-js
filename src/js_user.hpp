...
template <typename T>
void WatchStreamClass<T>::get_state(ContextType ctx, ObjectType object,
...
template <typename T>
void WatchStreamClass<T>::get_error(ContextType ctx, ObjectType object,
                                    ReturnValue &return_value) {
  WatchStream *ws = get_internal<T, WatchStreamClass<T>>(ctx, object);
...
template <typename T>
void WatchStreamClass<T>::feed_buffer(ContextType ctx, ObjectType object,
                                      Arguments &args,
                                      ReturnValue &return_value) {
...
template <typename T>
void WatchStreamClass<T>::next_event(ContextType ctx, ObjectType object,
                                     Arguments &args,
                                     ReturnValue &return_value) {
  args.validate_count(0);
  WatchStream *ws = get_internal<T, WatchStreamClass<T>>(ctx, object);
  return return_value.set(
      Value::from_string(ctx, String::from_bson(ws->next_event())));
}

template <typename T> class User : public SharedUser {
public:
  User(SharedUser user, SharedApp app)
      : SharedUser(std::move(user)), m_app(std::move(app)) {}
...
template <typename T> class UserClass : public ClassDefinition<T, User<T>> {
  using GlobalContextType = typename T::GlobalContext;
  using ContextType = typename T::Context;
  using FunctionType = typename T::Function;
  using ObjectType = typename T::Object;
...
public:
  ...
  static void session_for_on_disk_path(ContextType, ObjectType, Arguments &,
                                       ReturnValue &);
  static void link_credentials(ContextType, ObjectType, Arguments &,
                               ReturnValue &);
  static void call_function(ContextType, ObjectType, Arguments &,
                            ReturnValue &);
  static void refresh_custom_data(ContextType, ObjectType, Arguments &,
                                  ReturnValue &);
  static void push_register(ContextType, ObjectType, Arguments &,
                            ReturnValue &);
  static void push_deregister(ContextType, ObjectType, Arguments &,
                              ReturnValue &);
  static void make_streaming_request(ContextType, ObjectType, Arguments &,
                                     ReturnValue &);
  static void new_watch_stream(ContextType, ObjectType, Arguments &,
                               ReturnValue &);
...
inline typename T::Function UserClass<T>::create_constructor(ContextType ctx) {
  // WatchStream isn't directly nameable from JS, so we don't need to do
  // anything with the returned function object, but we still need to initialize
  // it here.
...
template <typename T>
typename T::Object
UserClass<T>::create_instance(ContextType ctx, SharedUser user, SharedApp app) {
  return create_object<T, UserClass<T>>(
      ctx, new User<T>(std::move(user), std::move(app)));
}

template <typename T>
void UserClass<T>::get_id(ContextType ctx, ObjectType object,
                          ReturnValue &return_value) {
  std::string id =
      get_internal<T, UserClass<T>>(ctx, object)->get()->identity();
  return_value.set(id);
}

template <typename T>
void UserClass<T>::get_identities(ContextType ctx, ObjectType object,
                                  ReturnValue &return_value) {
  std::vector<SyncUserIdentity> identities =
      get_internal<T, UserClass<T>>(ctx, object)->get()->identities();

  std::vector<ValueType> identity_objects;
  for (auto identity : identities) {
    auto identity_object = Object::create_empty(ctx);
    Object::set_property(ctx, identity_object, "id",
                         Value::from_string(ctx, identity.id));
    Object::set_property(ctx, identity_object, "providerType",
                         Value::from_string(ctx, identity.provider_type));
...
template <typename T>
void UserClass<T>::get_device_id(ContextType ctx, ObjectType object,
                                 ReturnValue &return_value) {
...
template <typename T>
void UserClass<T>::get_provider_type(ContextType ctx, ObjectType object,
                                     ReturnValue &return_value) {
  std::string provider_type =
      get_internal<T, UserClass<T>>(ctx, object)->get()->provider_type();
  return_value.set(provider_type);
}

template <typename T>
void UserClass<T>::get_access_token(ContextType ctx, ObjectType object,
                                    ReturnValue &return_value) {
  std::string token =
      get_internal<T, UserClass<T>>(ctx, object)->get()->access_token();
  return_value.set(token);
}

template <typename T>
void UserClass<T>::get_refresh_token(ContextType ctx, ObjectType object,
                                     ReturnValue &return_value) {
  std::string token =
      get_internal<T, UserClass<T>>(ctx, object)->get()->refresh_token();
  return_value.set(token);
}

template <typename T>
void UserClass<T>::is_logged_in(ContextType ctx, ObjectType object,
                                ReturnValue &return_value) {
  auto logged_in =
      get_internal<T, UserClass<T>>(ctx, object)->get()->is_logged_in();
  return_value.set(logged_in);
}

template <typename T>
void UserClass<T>::get_state(ContextType ctx, ObjectType object,
                             ReturnValue &return_value) {
...
template <typename T>
void UserClass<T>::get_custom_data(ContextType ctx, ObjectType object,
                                   ReturnValue &return_value) {
  auto custom_data =
      get_internal<T, UserClass<T>>(ctx, object)->get()->custom_data();
  if (!custom_data)
...
template <typename T>
void UserClass<T>::get_profile(ContextType ctx, ObjectType object,
                               ReturnValue &return_value) {
  ...
  auto user_profile =
      get_internal<T, UserClass<T>>(ctx, object)->get()->user_profile();
...
#define STRING_TO_PROP(propname)                                               \
  ...
  if (optional_##propname) {                                                   \
    Object::set_property(ctx, profile_object, string_##propname,               \
                         Value::from_string(ctx, *optional_##propname));       \
  }
...
template <typename T>
void UserClass<T>::logout(ContextType ctx, ObjectType this_object,
...
template <typename T>
void UserClass<T>::link_credentials(ContextType ctx, ObjectType this_object,
                                    Arguments &args, ReturnValue &) {
  args.validate_count(2);
  auto user = get_internal<T, UserClass<T>>(ctx, this_object);

  auto credentials = *get_internal<T, CredentialsClass<T>>(
      ctx, Value::validated_to_object(ctx, args[0], "credentials"));
  auto callback = Value::validated_to_function(ctx, args[1], "callback");

  user->m_app->link_user(
      *user, credentials,
...
template <typename T>
void UserClass<T>::call_function(ContextType ctx, ObjectType this_object,
                                 Arguments &args, ReturnValue &) {
  args.validate_count(4);
  auto user = get_internal<T, UserClass<T>>(ctx, this_object);

  auto name = Value::validated_to_string(ctx, args[0], "name");
  auto stringified_ejson_args =
      Value::validated_to_string(ctx, args[1], "args");
  auto service = Value::is_undefined(ctx, args[2])
                     ? util::none
                     : util::Optional<std::string>(
                           Value::validated_to_string(ctx, args[2], "service"));
  ...
  user->m_app->call_function(
      *user, name, bson_args.operator const bson::BsonArray &(), service,
      Function::wrap_callback_error_first(
          ctx, this_object, callback,
...
template <typename T>
void UserClass<T>::get_api_keys(ContextType ctx, ObjectType this_object,
                                ReturnValue &return_value) {
  auto user = get_internal<T, UserClass<T>>(ctx, this_object);
  return_value.set(
      ApiKeyAuthClass<T>::create_instance(ctx, user->m_app, *user));
}

template <typename T>
void UserClass<T>::refresh_custom_data(ContextType ctx, ObjectType this_object,
                                       Arguments &args,
                                       ReturnValue &return_value) {
  args.validate_count(1);
  auto user = get_internal<T, UserClass<T>>(ctx, this_object);
...
template <typename T>
void UserClass<T>::push_register(ContextType ctx, ObjectType this_object,
...
template <typename T>
void UserClass<T>::push_deregister(ContextType ctx, ObjectType this_object,
                                   Arguments &args, ReturnValue &return_value) {
  args.validate_count(2);
  auto user = get_internal<T, UserClass<T>>(ctx, this_object);
  auto service = Value::validated_to_string(ctx, args[0], "service");
...
template <typename T>
void UserClass<T>::make_streaming_request(ContextType ctx,
  ...
  auto stringified_ejson_args =
      Value::validated_to_string(ctx, args[2], "args");
  auto bson_args = String::to_bson(stringified_ejson_args);

  auto req = user->m_app->make_streaming_request(
      *user, name, bson_args.operator const bson::BsonArray &(),
      std::string(service));
  return return_value.set(JavaScriptNetworkTransport<T>::makeRequest(ctx, req));
}

template <typename T>
void UserClass<T>::new_watch_stream(ContextType ctx, ObjectType this_object,
                                    Arguments &args,
                                    ReturnValue &return_value) {
  args.validate_count(0);
  return return_value.set(
      create_object<T, WatchStreamClass<T>>(ctx, new WatchStream()));
}
} // namespace js
} // namespace realm
