...
export function createSubscription(prototype, realmId, info, _mutable) {
    let subscription = Object.create(prototype);
    ...
    Object.defineProperties(subscription, {
        ...
        'error': {
            get: getterForProperty('error',)
        }
    });

    subscription[keys.realm] = realmId;
    subscription[keys.id] = info.id;
    subscription[keys.type] = info.type;
    subscription[mutable] = _mutable;
