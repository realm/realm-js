...
/** Hook that is used to gain access to the {@link Realm} instance */
export type UseRealmHook = {
  (throwWhenMissing?: true): Realm;
  (throwWhenMissing: false): Realm | null;
};
...
export const createUseRealm = (RealmContext: React.Context<Realm | null>): UseRealmHook => {
  function useRealm(throwWhenMissing?: true): Realm;
  function useRealm(throwWhenMissing: false): Realm | null;
  function useRealm(throwWhenMissing = true) {
    // This is the context setup by `createRealmContext`
    const context = useContext(RealmContext);
    if (context === null && throwWhenMissing) {
