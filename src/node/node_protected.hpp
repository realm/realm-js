...
template <typename MemberType> class Protected {
protected:
  Napi::Env m_env;
  napi_ref m_ref;
  bool m_isValue;
...
public:
  ...
  Protected(Napi::Env env, MemberType value) : m_env(env), m_isValue(false) {
    ...
    if (status != napi_ok) {
      throw std::runtime_error(util::format(
          "Can't create protected reference: napi_status %1", status));
    }
  }

  Protected(const Protected &other)
      : m_env(other.m_env), m_ref(other.m_ref), m_isValue(other.m_isValue) {
    uint32_t result;
    napi_status status = napi_reference_ref(m_env, m_ref, &result);
    if (status != napi_ok) {
      throw std::runtime_error(util::format(
          "Can't increase protected reference count: napi_status %1", status));
    }
  }

  Protected(Protected &&other) : Protected() { swap(*this, other); }
  ...
  ~Protected() {
    ...
    try {
      ...
      REALM_ASSERT((status == napi_ok) &&
                   "~Protected: Can't decrease protected reference count");

      if (result == 0) {
        napi_status status = napi_delete_reference(m_env, m_ref);
        REALM_ASSERT((status == napi_ok) &&
                     "~Protected: Can't unallocate protected reference");
      }
  ...
  operator MemberType() const {
    ...
    if (status != napi_ok) {
      throw std::runtime_error(util::format(
          "Can't get protected reference: napi_status %1", status));
    }

    if (value == nullptr) {
      throw std::runtime_error(
          util::format("Can not use unallocated protected reference"));
    }
  ...
  void SuppressDestruct() { m_suppressDestruct = true; }

  struct Comparator {
    bool operator()(const Protected<MemberType> &a,
                    const Protected<MemberType> &b) const {
      MemberType aValue = a;
      MemberType bValue = b;
...
template <> class Protected<node::Types::GlobalContext> {
  node::Types::GlobalContext m_ctx;

public:
  Protected(node::Types::GlobalContext ctx) : m_ctx(ctx) {}

  operator Napi::Env() const { return m_ctx; }
...
template <>
class Protected<node::Types::Function>
    : public node::Protected<Napi::Function> {
public:
