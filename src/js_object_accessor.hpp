...
namespace js {
namespace _impl {
template <typename JSEngine, typename T> struct Unbox;
}

template <typename JSEngine> class NativeAccessor {
public:
  ...
  NativeAccessor(ContextType ctx, std::shared_ptr<Realm> realm,
  ...
  NativeAccessor(ContextType ctx, Collection const &collection)
      : m_ctx(ctx), m_realm(collection.get_realm()),
        m_object_schema(collection.get_type() == realm::PropertyType::Object
                            ? &collection.get_object_schema()
                            : nullptr) {
    register_plugins();
  }

  NativeAccessor(NativeAccessor &na, Obj parent, Property const &prop)
      : m_ctx(na.m_ctx), m_realm(na.m_realm), m_parent(std::move(parent)),
        m_property(&prop), m_object_schema(nullptr) {
  ...
  NativeAccessor(NativeAccessor &parent, const Property &prop)
      : m_ctx(parent.m_ctx), m_realm(parent.m_realm), m_object_schema(nullptr) {
    auto schema = m_realm->schema().find(prop.object_type);
    if (schema != m_realm->schema().end()) {
      m_object_schema = &*schema;
    }
  ...
  void register_plugins() { MixedLink<JSEngine>::add_strategy(m_realm); }

  ~NativeAccessor() { MixedLink<JSEngine>::remove_strategy(); }

  OptionalValue value_for_property(ValueType dict, Property const &prop,
                                   size_t) {
    ObjectType object = Value::validated_to_object(m_ctx, dict);
    ValueType value = Object::get_property(
        m_ctx, object,
        !prop.public_name.empty() ? prop.public_name : prop.name);
  ...
  OptionalValue default_value_for_property(const ObjectSchema &object_schema,
                                           const Property &prop) {
    auto defaults =
        get_delegate<JSEngine>(m_realm.get())->m_defaults[object_schema.name];
    auto it = defaults.find(prop.name);
    return it != defaults.end() ? util::make_optional(ValueType(it->second))
                                : util::none;
  }

  template <typename T>
  T unbox(ValueType value,
          realm::CreatePolicy policy = realm::CreatePolicy::Skip,
          ObjKey current_obj = ObjKey());
  ...
  template <typename T> util::Optional<T> unbox_optional(ValueType value) {
    return is_null(value) ? util::none : util::make_optional(unbox<T>(value));
  }

  template <typename T> ValueType box(util::Optional<T> v) {
  ...
  ValueType box(Mixed mixed) {
    return TypeMixed<JSEngine>::get_instance().wrap(m_ctx, mixed);
  }
  ...
  ValueType box(Timestamp ts) {
    ...
    return Object::create_date(m_ctx, ts.get_seconds() * 1000 +
                                          ts.get_nanoseconds() / 1000000);
  }
  ValueType box(realm::Object realm_object) {
    return RealmObjectClass<JSEngine>::create_instance(m_ctx,
                                                       std::move(realm_object));
  }
  ValueType box(Obj obj) {
    ...
    return RealmObjectClass<JSEngine>::create_instance(
        m_ctx, realm::Object(m_realm, *m_object_schema, obj));
  }
  ...
  ValueType box(realm::object_store::Dictionary dictionary) {
    return DictionaryClass<JSEngine>::create_instance(m_ctx,
                                                      std::move(dictionary));
  }
  ...
  ValueType null_value() { return Value::from_null(m_ctx); }
  ...
  template <typename Fn> void enumerate_dictionary(ValueType value, Fn &&func) {
  ...
  bool is_same_list(realm::List const &list,
                    ValueType const &value) const noexcept {
    auto object = Value::validated_to_object(m_ctx, value);
    if (js::Object<JSEngine>::template is_instance<ListClass<JSEngine>>(
            m_ctx, object)) {
      return list ==
             *get_internal<JSEngine, ListClass<JSEngine>>(m_ctx, object);
    }
    return false;
  }

  bool is_same_set(realm::object_store::Set const &set,
                   ValueType const &value) const {
    auto object = Value::validated_to_object(m_ctx, value);
    if (js::Object<JSEngine>::template is_instance<SetClass<JSEngine>>(
            m_ctx, object)) {
  ...
  bool is_same_dictionary(realm::object_store::Dictionary const &dictionary,
                          ValueType const &value) const {
    auto object = Value::validated_to_object(m_ctx, value);
    if (Object::template is_instance<DictionaryClass<JSEngine>>(m_ctx,
                                                                object)) {
      return dictionary ==
             *get_internal<JSEngine, DictionaryClass<JSEngine>>(m_ctx, object);
    }
    return false;
  }
...
private:
  ...
  template <typename, typename> friend struct _impl::Unbox;
};

namespace _impl {
template <typename JSEngine> struct Unbox<JSEngine, bool> {
  static bool call(NativeAccessor<JSEngine> *ctx,
                   typename JSEngine::Value const &value, realm::CreatePolicy,
                   ObjKey) {
    return js::Value<JSEngine>::validated_to_boolean(ctx->m_ctx, value,
                                                     "Property");
  }
};

template <typename JSEngine> struct Unbox<JSEngine, int64_t> {
  static int64_t call(NativeAccessor<JSEngine> *ctx,
                      typename JSEngine::Value const &value,
                      realm::CreatePolicy, ObjKey) {
    return js::Value<JSEngine>::validated_to_number(ctx->m_ctx, value,
                                                    "Property");
  }
};

template <typename JSEngine> struct Unbox<JSEngine, float> {
  static float call(NativeAccessor<JSEngine> *ctx,
                    typename JSEngine::Value const &value, realm::CreatePolicy,
                    ObjKey) {
    return js::Value<JSEngine>::validated_to_number(ctx->m_ctx, value,
                                                    "Property");
...
template <typename JSEngine> struct Unbox<JSEngine, double> {
  static double call(NativeAccessor<JSEngine> *ctx,
                     typename JSEngine::Value const &value, realm::CreatePolicy,
                     ObjKey) {
    return js::Value<JSEngine>::validated_to_number(ctx->m_ctx, value,
                                                    "Property");
...
template <typename JSEngine> struct Unbox<JSEngine, Decimal128> {
  static Decimal128 call(NativeAccessor<JSEngine> *ctx,
                         typename JSEngine::Value const &value,
                         realm::CreatePolicy, ObjKey) {
    if (ctx->is_null(value)) {
      return Decimal128(realm::null());
    }
    return js::Value<JSEngine>::validated_to_decimal128(ctx->m_ctx, value,
                                                        "Property");
  }
};

template <typename JSEngine> struct Unbox<JSEngine, ObjectId> {
  static ObjectId call(NativeAccessor<JSEngine> *ctx,
                       typename JSEngine::Value const &value,
                       realm::CreatePolicy, ObjKey) {
    return js::Value<JSEngine>::validated_to_object_id(ctx->m_ctx, value,
                                                       "Property");
  }
};

template <typename JSEngine> struct Unbox<JSEngine, util::Optional<bool>> {
  static util::Optional<bool> call(NativeAccessor<JSEngine> *ctx,
                                   typename JSEngine::Value const &value,
                                   realm::CreatePolicy, ObjKey) {
...
template <typename JSEngine> struct Unbox<JSEngine, util::Optional<int64_t>> {
  static util::Optional<int64_t> call(NativeAccessor<JSEngine> *ctx,
                                      typename JSEngine::Value const &value,
                                      realm::CreatePolicy, ObjKey) {
...
template <typename JSEngine> struct Unbox<JSEngine, util::Optional<float>> {
  static util::Optional<float> call(NativeAccessor<JSEngine> *ctx,
                                    typename JSEngine::Value const &value,
                                    realm::CreatePolicy, ObjKey) {
...
template <typename JSEngine> struct Unbox<JSEngine, util::Optional<double>> {
  static util::Optional<double> call(NativeAccessor<JSEngine> *ctx,
                                     typename JSEngine::Value const &value,
                                     realm::CreatePolicy, ObjKey) {
...
template <typename JSEngine> struct Unbox<JSEngine, util::Optional<ObjectId>> {
  static util::Optional<ObjectId> call(NativeAccessor<JSEngine> *ctx,
                                       typename JSEngine::Value const &value,
                                       realm::CreatePolicy, ObjKey) {
...
template <typename JSEngine> struct Unbox<JSEngine, StringData> {
  static StringData call(NativeAccessor<JSEngine> *ctx,
                         typename JSEngine::Value const &value,
                         realm::CreatePolicy, ObjKey) {
    if (ctx->is_null(value)) {
      return StringData();
    ...
    ctx->m_string_buffer =
        js::Value<JSEngine>::validated_to_string(ctx->m_ctx, value, "Property");
    return ctx->m_string_buffer;
  }
};

template <typename JSEngine> struct Unbox<JSEngine, BinaryData> {
  static BinaryData call(NativeAccessor<JSEngine> *ctx,
                         typename JSEngine::Value value, realm::CreatePolicy,
                         ObjKey) {
    if (ctx->is_null(value)) {
      return BinaryData();
    }
    ...
    if (js::Value<JSEngine>::is_string(ctx->m_ctx, value)) {
      ...
      if (auto size = util::base64_decode(str, data.get(), max_size)) {
        ...
        throw std::runtime_error("Attempting to populate BinaryData from "
                                 "string that is not valid base64");
      }
    }
#endif

    ctx->m_owned_binary_data =
        js::Value<JSEngine>::validated_to_binary(ctx->m_ctx, value);
    return ctx->m_owned_binary_data.get();
  }
};

template <typename JSEngine> struct Unbox<JSEngine, Mixed> {
  static Mixed call(NativeAccessor<JSEngine> *ctx,
                    typename JSEngine::Value const &value, realm::CreatePolicy,
                    ObjKey) {
...
template <typename JSEngine> struct Unbox<JSEngine, UUID> {
  static UUID call(NativeAccessor<JSEngine> *ctx,
                   typename JSEngine::Value const &value, realm::CreatePolicy,
                   ObjKey) {
    return js::Value<JSEngine>::validated_to_uuid(ctx->m_ctx, value,
                                                  "Property");
  }
};

template <typename JSEngine> struct Unbox<JSEngine, ObjLink> {
  static ObjLink call(NativeAccessor<JSEngine> *ctx,
                      typename JSEngine::Value const &value,
                      realm::CreatePolicy, ObjKey) {
...
template <typename JSEngine> struct Unbox<JSEngine, util::Optional<UUID>> {
  static util::Optional<UUID> call(NativeAccessor<JSEngine> *ctx,
                                   typename JSEngine::Value const &value,
                                   realm::CreatePolicy, ObjKey) {
...
template <typename JSEngine> struct Unbox<JSEngine, Timestamp> {
  static Timestamp call(NativeAccessor<JSEngine> *ctx,
                        typename JSEngine::Value const &value,
                        realm::CreatePolicy, ObjKey) {
    if (ctx->is_null(value)) {
      return Timestamp();
    ...
    if (js::Value<JSEngine>::is_string(ctx->m_ctx, value)) {
      // the incoming value might be a date string, so let the Date constructor
      // have at it
...
template <typename JSEngine> struct Unbox<JSEngine, Obj> {
  static Obj call(NativeAccessor<JSEngine> *native_accessor,
                  typename JSEngine::Value const &value,
    ...
    if (Value::is_array(native_accessor->m_ctx, object)) {
      object = Schema<JSEngine>::dict_for_property_array(
          native_accessor->m_ctx, *native_accessor->m_object_schema, object);
    }

    auto child = realm::Object::create<ValueType>(
        *native_accessor, native_accessor->m_realm,
        *native_accessor->m_object_schema, static_cast<ValueType>(object),
        policy, current_row);
    return child.obj();
  }
};

// FIXME: Why do we need this? It is required in order to compile and seems to
// be used by the query builder
template <typename JSEngine> struct Unbox<JSEngine, ObjKey> {
  static ObjKey call(NativeAccessor<JSEngine> *ctx,
                     typename JSEngine::Value const &value,
                     realm::CreatePolicy policy, ObjKey current_row) {
    return Unbox<JSEngine, Obj>::call(ctx, value, policy, current_row)
        .get_key();
  }
};
...
template <typename T>
template <typename U>
U NativeAccessor<T>::unbox(ValueType value, realm::CreatePolicy policy,
                           ObjKey current_row) {
...
void NativeAccessor<T>::print(std::string &str, ValueType const &value) {
  if (Value::is_null(m_ctx, value)) {
    ...
    if (Object::template is_instance<RealmObjectClass<T>>(m_ctx, object)) {
      ...
      str += "{";
      for (size_t i = 0, count = object_schema.persisted_properties.size();
           i < count; ++i) {
