...
class RealmReactModule extends ReactContextBaseJavaModule {
    ...
    class AndroidWebServer extends NanoHTTPD {
        ...
        public Response serve(IHTTPSession session) {
            ...
            //final String[] taskResult = new String[1];
            //RunnableFuture<Void> task = new FutureTask<>(new Runnable() {
            //    @Override
            //    public void run() {
            //        taskResult[0] = processChromeDebugCommand(cmdUri, json);
            //    }
            //}, null);
            //reactApplicationContext.runOnUiQueueThread(task);
            //try {
            //    task.get(); //wait for the result
            //} catch (InterruptedException | ExecutionException e) {
            //    // handle exception
            //}
            //final String jsonResponse  = taskResult[0];
