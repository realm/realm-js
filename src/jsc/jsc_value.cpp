...
bool jsc::Value::is_binary(JSContextRef ctx, const JSValueRef &value) {
  ...
  JSObjectRef array_buffer_constructor = jsc::Object::validated_get_constructor(
      ctx, global_object, s_array_buffer);

  // Value should either be an ArrayBuffer or ArrayBufferView (i.e. TypedArray
  // or DataView).
  if (JSValueIsInstanceOfConstructor(ctx, value, array_buffer_constructor,
                                     nullptr)) {
    return true;
  }
  if (JSObjectRef object = JSValueToObject(ctx, value, nullptr)) {
    ...
    JSValueRef is_view = jsc::Object::call_method(ctx, array_buffer_constructor,
                                                  s_is_view, 1, &object);
...
JSValueRef jsc::Value::from_nonnull_binary(JSContextRef ctx, BinaryData data) {
  ...
  JSObjectRef uint8_array_constructor = jsc::Object::validated_get_constructor(
      ctx, JSContextGetGlobalObject(ctx), s_uint8_array);
  JSObjectRef uint8_array = jsc::Function::construct(
      ctx, uint8_array_constructor, 1, &byte_count_value);
...
JSValueRef jsc::Value::from_decimal128(JSContextRef ctx,
  ...
  JSObjectRef realm_constructor =
      jsc::Object::validated_get_constructor(ctx, global_object, s_realm);
  JSObjectRef decimal_constructor =
      jsc::Object::validated_get_constructor(ctx, realm_constructor, s_decimal);

  std::array<JSValueRef, 1> args = {
      {jsc::Value::from_nonnull_string(ctx, jsc::String(value.to_string()))}};

  return jsc::Object::call_method(ctx, decimal_constructor, s_from_string,
                                  args.size(), args.data());
}
...
JSValueRef jsc::Value::from_object_id(JSContextRef ctx, const ObjectId &value) {
  ...
  JSObjectRef realm_constructor =
      jsc::Object::validated_get_constructor(ctx, global_object, s_realm);
  JSObjectRef object_id_constructor = jsc::Object::validated_get_constructor(
      ctx, realm_constructor, s_object_id);

  std::array<JSValueRef, 1> args{
      {jsc::Value::from_nonnull_string(ctx, jsc::String(value.to_string()))}};
  return jsc::Function::construct(ctx, object_id_constructor, args.size(),
                                  args.data());
}
...
JSValueRef jsc::Value::from_uuid(JSContextRef ctx, const UUID &value) {
  ...
  JSObjectRef realm_constructor =
      jsc::Object::validated_get_constructor(ctx, global_object, s_realm);
  JSObjectRef uuid_constructor =
      jsc::Object::validated_get_constructor(ctx, realm_constructor, s_uuid);

  std::array<JSValueRef, 1> args{
      {jsc::Value::from_nonnull_string(ctx, jsc::String(value.to_string()))}};
  return jsc::Function::construct(ctx, uuid_constructor, args.size(),
                                  args.data());
}
...
OwnedBinaryData jsc::Value::to_binary(JSContextRef ctx,
  ...
  JSObjectRef uint8_array_constructor =
      jsc::Object::validated_get_constructor(ctx, global_object, s_uint8_array);
  ...
  if (JSValueIsInstanceOfConstructor(ctx, value, array_buffer_constructor,
    ...
    if (jsc::Value::to_boolean(ctx, is_view)) {
      uint8_array_arguments[0] =
          jsc::Object::validated_get_object(ctx, object, s_buffer);
      uint8_array_arguments[1] =
          jsc::Object::get_property(ctx, object, s_byte_offset);
      uint8_array_arguments[2] =
          jsc::Object::get_property(ctx, object, s_byte_length);
      uint8_array_argc = 3;
    }
  }
  ...
  if (!uint8_array_argc) {
    throw std::runtime_error(
        "Can only convert ArrayBuffer and TypedArray objects to binary");
  }

  JSObjectRef uint8_array = jsc::Function::construct(
      ctx, uint8_array_constructor, uint8_array_argc, uint8_array_arguments);
