...
class RNSqliteTests extends Tests {
  ...
  async setup(testName) {
    if (testName == "insertions" || testName == "binsertions") {
      try {
        ...
        console.warn("Error deleting SQLite DB", err);
      }
    }
  ...
  async querycount() {
    var len;
    await this.db.readTransaction(async (tx) => {
      let [, results] = await tx.executeSql(
        "SELECT * FROM t1 WHERE int = 0 AND double < " + numBatchTestObjects / 2 + ";",
      );
      len = results.rows.length;
    });
    return len;
...
export default class ReactNativeBenchmarks extends React.Component {
  ...
  render() {
    return (
      <View style={styles.container}>
        <Text style={styles.title}>ReactNative Storage Benchmarks</Text>
  ...
  _renderRow(rowData) {
    return <Text style={styles.item}>{rowData.join("\t\t")}</Text>;
  }
