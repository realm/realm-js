...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *) {
  ...
  ssl_helper_class = reinterpret_cast<jclass>(
      env->NewGlobalRef(env->FindClass("io/realm/react/util/SSLHelper")));
...
Java_io_realm_react_RealmReactModule_setDefaultRealmFileDirectory(
    JNIEnv *env, jclass, jstring fileDir, jobject javaAssetManager) {
  __android_log_print(ANDROID_LOG_VERBOSE, "JSRealm",
                      "setDefaultRealmFileDirectory");
  ...
  if (assetManager == NULL) {
    __android_log_print(ANDROID_LOG_ERROR, "JSRealm",
                        "Error loading the AssetManager");
  }
...
Java_io_realm_react_RealmReactModule_setupChromeDebugModeRealmJsContext(
    JNIEnv *, jclass) {
  __android_log_print(ANDROID_LOG_VERBOSE, "JSRealm",
...
Java_io_realm_react_RealmReactModule_processChromeDebugCommand(
...
Java_io_realm_react_RealmReactModule_tryRunTask(JNIEnv *env, jclass) {
  jboolean result = s_rpc_server->try_run_task();
  return result;
}

JNIEXPORT jboolean JNICALL
Java_io_realm_react_RealmReactModule_isContextInjected(JNIEnv *env, jclass) {
  return realmContextInjected;
}

JNIEXPORT void JNICALL
Java_io_realm_react_RealmReactModule_clearContextInjectedFlag(JNIEnv *env,
                                                              jclass) {
