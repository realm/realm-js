...
struct DictionaryClass
    : ClassDefinition<T, realm::js::Dictionary<T>, CollectionClass<T>> {
  using Type = T;
  ...
  // helpers
  static ValueType create_dictionary_change_set(ContextType,
                                                DictionaryChangeSet const &);
  static void validate_value(ContextType,
                             const realm::object_store::Dictionary &,
                             ValueType);
...
template <typename T>
typename T::Object DictionaryClass<T>::create_instance(
    ContextType ctx, realm::object_store::Dictionary dictionary) {
  auto object = create_object<T, DictionaryClass<T>>(
      ctx, new realm::js::Dictionary<T>(std::move(dictionary)));

  ObjectType realm_constructor =
      Value::validated_to_object(ctx, Object::get_global(ctx, "Realm"));
  FunctionType realm_dictionary_proxy = Value::to_function(
      ctx, Object::get_property(ctx, realm_constructor, "DictionaryProxy"));
  ValueType arguments[] = {object};
  return Value::to_object(
      ctx, Function<T>::call(ctx, realm_dictionary_proxy, 1, arguments));
}

template <typename T>
void DictionaryClass<T>::validate_value(
    ContextType ctx, const realm::object_store::Dictionary &dictionary,
    ValueType value) {
...
template <typename T>
void DictionaryClass<T>::setter(ContextType ctx, ObjectType this_object,
...
template <typename T>
void DictionaryClass<T>::getter(ContextType ctx, ObjectType this_object,
                                Arguments &args, ReturnValue &return_value) {
...
template <typename T>
void DictionaryClass<T>::set(ContextType ctx, ObjectType this_object,
                             Arguments &args, ReturnValue &return_value) {
  args.validate_count(1);
...
template <typename T>
void DictionaryClass<T>::remove(ContextType ctx, ObjectType this_object,
                                Arguments &args, ReturnValue &return_value) {
...
template <typename T>
void DictionaryClass<T>::has(ContextType ctx, ObjectType this_object,
                             Arguments &args, ReturnValue &return_value) {
  args.validate_maximum(1);
  auto dictionary = get_internal<T, DictionaryClass<T>>(ctx, this_object);
  std::string key = Value::validated_to_string(ctx, args[0]);
...
template <typename T>
void DictionaryClass<T>::keys(ContextType ctx, ObjectType this_object,
                              Arguments &args, ReturnValue &return_value) {
...
template <typename T>
typename T::Value DictionaryClass<T>::create_dictionary_change_set(
    ContextType ctx, DictionaryChangeSet const &change_set) {
  ObjectType object = Object::create_empty(ctx);
  std::vector<ValueType> scratch;

  scratch.reserve(
      std::max({change_set.deletions.size(), change_set.insertions.size(),
                change_set.modifications.size()}));
...
template <typename T>
void DictionaryClass<T>::add_listener(ContextType ctx, ObjectType this_object,
                                      Arguments &args,
  ...
  auto token = dictionary->add_key_based_notification_callback(
      [=](DictionaryChangeSet const &change_set, std::exception_ptr exception) {
        ...
        ValueType arguments[]{
            ...
            DictionaryClass<T>::create_dictionary_change_set(protected_ctx,
                                                             change_set)};
...
template <typename T>
void DictionaryClass<T>::remove_listener(ContextType ctx,
  ...
  args.validate_maximum(1);
  auto callback = Value::validated_to_function(ctx, args[0]);
  auto protected_function = Protected<FunctionType>(
      ctx, callback); // Protecting for comparison, not to extend lifetime.

  auto &listeners = dictionary->m_listeners;
  auto compare = [&](auto &&func_and_tok) {
    return typename Protected<FunctionType>::Comparator()(func_and_tok.first,
                                                          protected_function);
  };
  listeners.erase(std::remove_if(listeners.begin(), listeners.end(), compare),
                  listeners.end());
}

template <typename T>
void DictionaryClass<T>::remove_all_listeners(ContextType ctx,
                                              ObjectType this_object,
                                              Arguments &args,
                                              ReturnValue &return_value) {
  auto dictionary = get_internal<T, DictionaryClass<T>>(ctx, this_object);
