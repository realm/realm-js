...
template <typename T> class RealmObject : public realm::Object {
public:
...
struct RealmObjectClass : ClassDefinition<T, realm::js::RealmObject<T>> {
  ...
  static void get_property(ContextType, ObjectType, const String &,
                           ReturnValue &);
  ...
  static void get_object_schema(ContextType, ObjectType, Arguments &,
                                ReturnValue &);
  static void linking_objects(ContextType, ObjectType, Arguments &,
                              ReturnValue &);
  static void linking_objects_count(ContextType, ObjectType, Arguments &,
                                    ReturnValue &);
  static void get_object_id(ContextType, ObjectType, Arguments &,
                            ReturnValue &);
  static void is_same_object(ContextType, ObjectType, Arguments &,
                             ReturnValue &);
...
template <typename T>
void RealmObjectClass<T>::is_valid(ContextType ctx, ObjectType this_object,
                                   Arguments &, ReturnValue &return_value) {
...
template <typename T>
void RealmObjectClass<T>::get_object_schema(ContextType ctx,
  ...
  return_value.set(Schema<T>::object_for_object_schema(
      ctx, realm_object->get_object_schema()));
}

template <typename T>
typename T::Object
RealmObjectClass<T>::create_instance(ContextType ctx,
                                     realm::js::RealmObject<T> realm_object) {
  ...
  try {
    if (!delegate || !delegate->m_constructors.count(name)) {
      return create_instance_by_schema<T, RealmObjectClass<T>>(ctx, schema,
                                                               internal);
    }

    FunctionType constructor = delegate->m_constructors.at(name);
    auto object = create_instance_by_schema<T, RealmObjectClass<T>>(
...
template <typename T>
void RealmObjectClass<T>::get_property(ContextType ctx, ObjectType object,
  ...
  const Property *prop =
      realm_object->get_object_schema().property_for_public_name(prop_name);
  if (prop) {
    NativeAccessor<T> accessor(ctx, realm_object->realm(),
                               realm_object->get_object_schema());
    auto result =
        realm_object->template get_property_value<ValueType>(accessor, *prop);
    return_value.set(result);
  }
}

template <typename T>
bool RealmObjectClass<T>::set_property(ContextType ctx, ObjectType object,
                                       const String &property_name,
                                       ValueType value) {
  auto realm_object = get_internal<T, RealmObjectClass<T>>(ctx, object);
  if (!realm_object) {
  ...
  if (!Value::is_valid_for_property(ctx, value, *prop)) {
    throw TypeErrorException(accessor, realm_object->get_object_schema().name,
                             *prop, value);
  }

  realm_object->set_property_value(accessor, prop->name, value,
                                   realm::CreatePolicy::UpdateAll);
  return true;
}

template <typename T>
void RealmObjectClass<T>::set_link(ContextType ctx, ObjectType object,
  ...
  std::string property_name =
      Value::validated_to_string(ctx, args[0], "propertyName");
  const Property *prop =
      realm_object->get_object_schema().property_for_name(property_name);
  if (!prop) {
    throw std::invalid_argument(
        util::format("No such property: %1", property_name));
  }
  ...
  auto &linked_schema =
      *realm_object->realm()->schema().find(prop->object_type);
  ...
  if (linked_pk->type == realm::PropertyType::String) {
    obj_key = linked_table->find_first(
        linked_pk->column_key, accessor.template unbox<StringData>(args[1]));
  } else if (is_nullable(linked_pk->type)) {
    obj_key = linked_table->find_first(
        linked_pk->column_key,
        accessor.template unbox<util::Optional<int64_t>>(args[1]));
  } else {
    obj_key = linked_table->find_first(
        linked_pk->column_key, accessor.template unbox<int64_t>(args[1]));
  }
...
template <typename T>
void RealmObjectClass<T>::get_realm(ContextType ctx, ObjectType object,
                                    ReturnValue &return_value) {
  return_value.set_undefined();
  auto realm_object = get_internal<T, RealmObjectClass<T>>(ctx, object);
  if (realm_object) {
    ObjectType realm_obj = create_object<T, RealmClass<T>>(
        ctx, new SharedRealm(realm_object->realm()));
...
template <typename T>
std::vector<String<T>>
RealmObjectClass<T>::get_property_names(ContextType ctx, ObjectType object) {
  ...
  names.reserve(object_schema.persisted_properties.size() +
                object_schema.computed_properties.size());
...
template <typename T>
void RealmObjectClass<T>::get_object_id(ContextType ctx, ObjectType object,
                                        Arguments &args,
                                        ReturnValue &return_value) {
...
template <typename T>
void RealmObjectClass<T>::is_same_object(ContextType ctx, ObjectType object,
                                         Arguments &args,
                                         ReturnValue &return_value) {
...
template <typename T>
void RealmObjectClass<T>::linking_objects_count(ContextType ctx,
                                                ObjectType object, Arguments &,
                                                ReturnValue &return_value) {
  auto realm_object = get_internal<T, RealmObjectClass<T>>(ctx, object);
  if (!realm_object) {
    throw std::runtime_error("Invalid 'this' object");
...
template <typename T>
void RealmObjectClass<T>::add_listener(ContextType ctx, ObjectType this_object,
                                       Arguments &args,
                                       ReturnValue &return_value) {
  ...
  auto token = realm_object->add_notification_callback(
      [=](CollectionChangeSet const &change_set, std::exception_ptr exception) {
        ...
        if (!change_set.deletions.empty()) {
          ...
          for (const auto &col : change_set.columns) {
            ...
            scratch.push_back(Value::from_string(
                protected_ctx, std::string(table->get_column_name(col_key))));
          }
        }

        ObjectType object = Object::create_empty(protected_ctx);
        Object::set_property(protected_ctx, object, "deleted",
                             Value::from_boolean(protected_ctx, deleted));
  ...
  realm_object->m_notification_tokens.emplace_back(protected_callback,
                                                   std::move(token));
}

template <typename T>
void RealmObjectClass<T>::remove_listener(ContextType ctx,
  ...
  auto compare = [&](auto &&token) {
    return typename Protected<FunctionType>::Comparator()(token.first,
                                                          protected_function);
  };
  tokens.erase(std::remove_if(tokens.begin(), tokens.end(), compare),
               tokens.end());
}

template <typename T>
void RealmObjectClass<T>::remove_all_listeners(ContextType ctx,
                                               ObjectType this_object,
                                               Arguments &args,
                                               ReturnValue &return_value) {
  args.validate_maximum(0);
...
// move this all the way here because it needs to include "js_results.hpp" which
// in turn includes this file
...
void realm::js::RealmObjectClass<T>::linking_objects(
  ...
  std::string object_type =
      Value::validated_to_string(ctx, args[0], "objectType");
  std::string property_name =
      Value::validated_to_string(ctx, args[1], "property");
  ...
  if (target_object_schema == realm_object->realm()->schema().end()) {
    throw std::logic_error(
        util::format("Could not find schema for type '%1'", object_type));
  }

  auto link_property = target_object_schema->property_for_name(property_name);
  if (!link_property) {
    throw std::logic_error(
        util::format("Type '%1' does not contain property '%2'", object_type,
                     property_name));
  }

  if (link_property->object_type != realm_object->get_object_schema().name) {
    throw std::logic_error(
        util::format("'%1.%2' is not a relationship to '%3'", object_type,
                     property_name, realm_object->get_object_schema().name));
  }

  realm::TableRef table = ObjectStore::table_for_object_type(
      realm_object->realm()->read_group(), target_object_schema->name);
  auto obj = realm_object->obj();
  auto tv = obj.get_backlink_view(table, link_property->column_key);

  return_value.set(ResultsClass<T>::create_instance(
      ctx, realm::Results(realm_object->realm(), std::move(tv))));
}
