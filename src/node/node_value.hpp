...
template <>
inline const char *node::Value::typeof(Napi::Env env,
...
template <>
inline bool node::Value::is_array_buffer(Napi::Env env,
                                         const Napi::Value &value) {
  return value.IsArrayBuffer();
}

template <>
inline bool node::Value::is_array_buffer_view(Napi::Env env,
                                              const Napi::Value &value) {
...
// if rebuilding the binary on Node.js with NAPI 4. On CI we should always be
// building with Node.js NAPI 5
#if NAPI_VERSION >= 5
  uint32_t version;
  ...
  return value.IsObject() && value.As<Napi::Object>().InstanceOf(
...
template <>
inline bool node::Value::is_constructor(Napi::Env env,
                                        const Napi::Value &value) {
...
inline bool node::Value::is_error(Napi::Env env, const Napi::Value &value) {
  return value.IsObject() &&
         value.As<Napi::Object>().InstanceOf(
             env.Global().Get("Error").As<Napi::Function>());
}
...
inline bool is_bson_type(Napi::Env env, const Napi::Value &value,
                         std::string type) {
...
template <>
inline bool node::Value::is_decimal128(Napi::Env env,
                                       const Napi::Value &value) {
...
template <> inline bool node::Value::is_valid(const Napi::Value &value) {
...
template <> inline Napi::Value node::Value::from_null(Napi::Env env) {
...
template <>
inline Napi::Value
node::Value::from_nonnull_string(Napi::Env env, const node::String &string) {
  return Napi::String::New(env, string);
}

template <>
inline Napi::Value node::Value::from_nonnull_binary(Napi::Env env,
                                                    BinaryData data) {
...
template <> inline Napi::Value node::Value::from_undefined(Napi::Env env) {
...
template <>
inline node::String node::Value::to_string(Napi::Env env,
                                           const Napi::Value &value) {
...
template <>
inline OwnedBinaryData node::Value::to_binary(Napi::Env env,
                                              const Napi::Value &value) {
  ...
  if (node_binary == nullptr) {
    throw std::runtime_error("Can only convert Buffer, ArrayBuffer, and "
                             "ArrayBufferView objects to binary");
  }
...
template <>
inline Napi::Object node::Value::to_object(Napi::Env env,
                                           const Napi::Value &value) {
  return value.ToObject();
}

template <>
inline Napi::Object node::Value::to_array(Napi::Env env,
                                          const Napi::Value &value) {
  return to_object(env, value);
}

template <>
inline Napi::Function node::Value::to_function(Napi::Env env,
                                               const Napi::Value &value) {
  return value.IsFunction() ? value.As<Napi::Function>() : Napi::Function();
}

template <>
inline Napi::Function node::Value::to_constructor(Napi::Env env,
                                                  const Napi::Value &value) {
  return to_function(env, value);
}

template <>
inline Napi::Object node::Value::to_date(Napi::Env env,
                                         const Napi::Value &value) {
  if (value.IsString()) {
    Napi::Function date_constructor =
        to_constructor(env, env.Global().Get("Date"));
    std::array<Napi::Value, 1> args{{value}};
    return node::Function::construct(env, date_constructor, args.size(),
                                     args.data());
  }
...
template <>
inline Napi::Value node::Value::from_decimal128(Napi::Env env,
  ...
  Napi::Object decimal_constructor =
      realm_constructor.Get("_Decimal128").As<Napi::Object>();
  Napi::Function fromStringFunc =
      decimal_constructor.Get("fromString").As<Napi::Function>();
...
template <>
inline Decimal128 node::Value::to_decimal128(Napi::Env env,
                                             const Napi::Value &value) {
...
template <>
inline Napi::Value node::Value::from_object_id(Napi::Env env,
                                               const ObjectId &objectId) {
  Napi::EscapableHandleScope scope(env);

  Napi::Function realm_constructor = node::RealmClassConstructor.Value();
  Napi::Function object_id_constructor =
      realm_constructor.Get("_ObjectId").As<Napi::Function>();
...
template <>
inline ObjectId node::Value::to_object_id(Napi::Env env,
                                          const Napi::Value &value) {
  Napi::HandleScope scope(env);

  Napi::Object objectId = value.As<Napi::Object>();
  Napi::Function toHexStringFunc =
      objectId.Get("toHexString").As<Napi::Function>();
...
Napi::Value node::Value::from_uuid(Napi::Env env, const UUID &uuid) {
  ...
  Napi::Function realm_constructor = node::RealmClassConstructor.Value();
  Napi::Function uuid_constructor =
      realm_constructor.Get("_UUID").As<Napi::Function>();

  napi_value args[] = {Napi::Buffer<std::uint8_t>::Copy(
      env, uuid.to_bytes().data(), UUID::num_bytes)};
...
inline UUID node::Value::to_uuid(Napi::Env env, const Napi::Value &value) {
  ...
  // TODO: Temp implementation of JS UUID has a buffer on the "id" key. This is
  // corresponding to the official ObjectId implementation - BUT could change.
