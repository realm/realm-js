...
module.exports = function(realmConstructor) {
    ...
    if (realmConstructor.Sync) {
        ...
        realmConstructor.permissions = function(arg) {
            // If no argument is provided, return the Realm-level permissions
            if (arg === undefined) {
                return this.objects(permissionsSchema.Realm).filtered(`id = 0`)[0];
            } else {
                // Else try to find the corresponding Class-level permissions
                return this.objects(permissionsSchema.Class).filtered(`name = $s`, this._schemaName(arg))[0];
            }
        }
    }
