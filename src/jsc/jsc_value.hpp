...
static inline bool is_object_of_type(JSContextRef ctx, JSValueRef value,
                                     jsc::String type) {
  JSObjectRef global_object = JSContextGetGlobalObject(ctx);
  JSValueRef exception = nullptr;
  JSValueRef constructor =
      JSObjectGetProperty(ctx, global_object, type, &exception);
  if (exception) {
    throw jsc::Exception(ctx, exception);
  }

  bool result = JSValueIsInstanceOfConstructor(
      ctx, value, jsc::Value::validated_to_constructor(ctx, constructor),
      &exception);
  if (exception) {
    throw jsc::Exception(ctx, exception);
  }
...
template <>
inline const char *jsc::Value::typeof(JSContextRef ctx,
...
template <>
inline bool jsc::Value::is_array_buffer(JSContextRef ctx,
                                        const JSValueRef &value) {
...
template <>
inline bool jsc::Value::is_constructor(JSContextRef ctx,
                                       const JSValueRef &value) {
  return JSValueIsObject(ctx, value) &&
         JSObjectIsConstructor(ctx, (JSObjectRef)value);
}
...
inline bool jsc::Value::is_function(JSContextRef ctx, const JSValueRef &value) {
  return JSValueIsObject(ctx, value) &&
         JSObjectIsFunction(ctx, (JSObjectRef)value);
}
...
template <>
inline bool jsc::Value::is_undefined(JSContextRef ctx,
                                     const JSValueRef &value) {
  return JSValueIsUndefined(ctx, value);
}

template <> inline bool jsc::Value::is_valid(const JSValueRef &value) {
  return value != nullptr;
}

inline bool is_bson_type(JSContextRef ctx, const JSValueRef &value,
                         std::string type) {
  ...
  JSValueRef bson_type = JSObjectGetProperty(
      ctx, object, JSStringCreateWithUTF8CString("_bsontype"), &error);
  if (error) {
    throw jsc::Exception(ctx, error);
  }
  ...
  // Since the string's retain value is +2 here, we need to manually release it
  // before returning.
...
/**
 * Checks if a `value` is an EJSON representation of a particular type
 * (determined by the existance of a particular property).
 */
inline bool is_ejson_type(JSContextRef ctx, const JSValueRef &value,
  ...
  JSStringRef property_name_string =
      JSStringCreateWithUTF8CString(property_name.c_str());
  auto property =
      JSObjectGetProperty(ctx, object, property_name_string, &error);
  if (error) {
    throw jsc::Exception(ctx, error);
  }
...
template <>
inline bool jsc::Value::is_decimal128(JSContextRef ctx,
                                      const JSValueRef &value) {
  return is_bson_type(ctx, value, "Decimal128") ||
         is_ejson_type(ctx, value, "$numberDecimal");
}

template <>
inline bool jsc::Value::is_object_id(JSContextRef ctx,
                                     const JSValueRef &value) {
  return is_bson_type(ctx, value, "ObjectID") ||
         is_ejson_type(ctx, value, "$oid");
}
...
template <> inline JSValueRef jsc::Value::from_null(JSContextRef ctx) {
...
template <>
inline JSValueRef jsc::Value::from_nonnull_string(JSContextRef ctx,
                                                  const jsc::String &string) {
  return JSValueMakeString(ctx, string);
}

template <> inline JSValueRef jsc::Value::from_undefined(JSContextRef ctx) {
...
template <>
JSValueRef jsc::Value::from_decimal128(JSContextRef ctx,
...
template <>
inline jsc::String jsc::Value::to_string(JSContextRef ctx,
                                         const JSValueRef &value) {
  JSValueRef exception = nullptr;
...
template <>
inline JSObjectRef jsc::Value::to_object(JSContextRef ctx,
                                         const JSValueRef &value) {
  JSValueRef exception = nullptr;
...
template <>
inline JSObjectRef jsc::Value::to_array(JSContextRef ctx,
                                        const JSValueRef &value) {
  return to_object(ctx, value);
}

template <>
inline JSObjectRef jsc::Value::to_constructor(JSContextRef ctx,
                                              const JSValueRef &value) {
  return to_object(ctx, value);
}

template <>
inline JSObjectRef jsc::Value::to_date(JSContextRef ctx,
                                       const JSValueRef &value) {
  if (JSValueIsString(ctx, value)) {
    JSValueRef error;
    std::array<JSValueRef, 1> args{value};
    if (JSObjectRef result =
            JSObjectMakeDate(ctx, args.size(), args.data(), &error)) {
      return result;
    } else {
      throw jsc::Exception(ctx, error);
    }
...
template <>
inline JSObjectRef jsc::Value::to_function(JSContextRef ctx,
                                           const JSValueRef &value) {
  return to_object(ctx, value);
}

template <>
OwnedBinaryData jsc::Value::to_binary(JSContextRef ctx,
                                      const JSValueRef &value);
...
template <> UUID jsc::Value::to_uuid(JSContextRef ctx, const JSValueRef &value);
