 ...
 * @property {string} [fifoFilesFallbackPath] - Opening a Realm creates a number of FIFO files in order to
 * coordinate access to the Realm across threads and processes. If the Realm file is stored in a location
 * that does not allow the creation of FIFO files (e.g. FAT32 filesystems), then the Realm cannot be opened.
 * In that case Realm needs a different location to store these files and this property defines that location.
 * The FIFO files are very lightweight and the main Realm file will still be stored in the location defined
