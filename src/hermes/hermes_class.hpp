...
public:
    ...
    static JsiFunc init(JsiEnv env, 
...
private:
    ...
    private:
        static JsiVal bind_native_function(JsiEnv env, const std::string& functionName, const JsiFunc& function, const JsiObj& thisObject);
...
private:
    ...
    static jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> setup_method(JsiEnv env, const std::string& name, hermes::Types::FunctionCallback callback);
    static jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> setup_static_method(JsiEnv env, const std::string& name, hermes::Types::FunctionCallback callback);
    static jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> setup_property(JsiEnv env, const std::string& name, const PropertyType&);
    static jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> setup_static_property(JsiEnv env, const std::string& name, const PropertyType&);
...
template<typename ClassType>
WrappedObject<ClassType>::WrappedObject(const jsi::CallbackInfo& info) : JsiObjWrap<WrappedObject<ClassType>>(info) {
...
template<typename ClassType>
JsiFunc WrappedObject<ClassType>::init(JsiEnv env, 
                                             const std::string& name, 
                                             hermes::Types::FunctionCallback constructor_callback, 
                                             std::function<bool(const std::string&)> has_native_method_callback,
                                             const std::vector<jsi::ClassPropertyDescriptor<WrappedObject<ClassType>>>& properties, 
    ...
    JsiFunc ctor = JsiObjWrap<WrappedObject<ClassType>>::DefineClass(env, name.c_str(), properties, (void*)constructor_callback);
...
JsiVal WrappedObject<ClassType>::create_instance_with_proxy(const jsi::CallbackInfo& info) {
    ...
    try {
        ...
        info.This().As<JsiObj>().DefineProperty(jsi::PropertyDescriptor::Value("isRealmCtor", jsi::Boolean::New(env, true), napi_configurable)); 
...
inline bool WrappedObject<ClassType>::is_instance(JsiEnv env, const JsiObj& object) {
    ...
    if (!isInstanceOf) {
        ...
        if (isRealmObjectClass) {
            jsi::External<typename ClassType::Internal> external = object.Get(ExternalSymbol).As<jsi::External<typename ClassType::Internal>>();
...
JsiVal WrappedObject<ClassType>::ProxyHandler::get_instance_proxy_handler(JsiEnv env) {
    ...
    jsi::PropertyDescriptor instanceGetTrapFunc = jsi::PropertyDescriptor::Function("get", &WrappedObject<ClassType>::ProxyHandler::get_proxy_trap);
    jsi::PropertyDescriptor instanceSetTrapFunc = jsi::PropertyDescriptor::Function("set", &WrappedObject<ClassType>::ProxyHandler::set_proxy_trap);
    jsi::PropertyDescriptor ownKeysTrapFunc = jsi::PropertyDescriptor::Function("ownKeys", &WrappedObject<ClassType>::ProxyHandler::own_keys_proxy_trap);
    jsi::PropertyDescriptor hasTrapFunc = jsi::PropertyDescriptor::Function("has", &WrappedObject<ClassType>::ProxyHandler::has_proxy_trap);
    jsi::PropertyDescriptor getOwnPropertyDescriptorTrapFunc = jsi::PropertyDescriptor::Function("getOwnPropertyDescriptor", &WrappedObject<ClassType>::ProxyHandler::get_own_property_descriptor_trap);
    jsi::PropertyDescriptor getPrototypeOfFunc = jsi::PropertyDescriptor::Function("getPrototypeOf", &WrappedObject<ClassType>::ProxyHandler::get_prototype_of_proxy_trap);
    jsi::PropertyDescriptor setPrototypeOfFunc = jsi::PropertyDescriptor::Function("setPrototypeOf", &WrappedObject<ClassType>::ProxyHandler::set_prototype_of_proxy_trap);
...
static JsiVal bind_function(JsiEnv env, const std::string& functionName, const JsiFunc& function, const JsiObj& thisObject) {
...
template<typename ClassType>
JsiVal WrappedObject<ClassType>::ProxyHandler::bind_native_function(JsiEnv env, const std::string& functionName, const JsiFunc& function, const JsiObj& thisObject) {
    jsi::EscapableHandleScope scope(env);
...
JsiFunc ObjectWrap<ClassType>::init_class(JsiEnv env) {
    ...
    if (!isRealmObjectClass) {
        ...
        for (auto& pair : s_class.static_properties) {
            jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> statc_property = setup_static_property(env, pair.first, pair.second);
            properties.push_back(statc_property);
        }

        for (auto& pair : s_class.static_methods) {
            jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> staticMethod = setup_static_method(env, pair.first, pair.second);
            properties.push_back(staticMethod);
        }

        for (auto& pair : s_class.methods) {
            jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> method = setup_method(env, pair.first, pair.second);
            properties.push_back(method);
        }

        for (auto& pair : s_class.properties) {
            jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> property = setup_property(env, pair.first, pair.second);
    ...
    if (isRealmObjectClass) {
        ...
        for (auto& pair : s_class.methods) {
            auto descriptor = jsi::PropertyDescriptor::Function(env, ctorPrototype, JsiString::New(env, pair.first) /*name*/, &method_callback, napi_default | realm::js::PropertyAttributes::DontEnum, (void*)pair.second /*callback*/);
        ...
        for (auto& pair : s_class.properties) {
            ...
            auto descriptor = jsi::PropertyDescriptor::Accessor<property_getter_callback>(JsiString::New(env, pair.first) /*name*/, napi_attributes, (void*)&pair.second /*callback*/);
            properties.push_back(descriptor);
        }
...
template<typename ClassType>
inline std::vector<jsi::PropertyDescriptor> ObjectWrap<ClassType>::create_napi_property_descriptors(JsiEnv env, const JsiObj& constructorPrototype, const realm::ObjectSchema& schema, bool redefine) {
    std::vector<jsi::PropertyDescriptor> properties;
    ...
    for (auto& property : schema.persisted_properties) {
        ...
        if (redefine || !constructorPrototype.HasOwnProperty(propName)) {
            ...
            auto descriptor = jsi::PropertyDescriptor::Accessor<property_getter, property_setter>(name->ToString(env), napi_enumerable, (void*)name);
            properties.push_back(descriptor);
        }
    }
...
template<typename ClassType>
JsiObj ObjectWrap<ClassType>::create_instance_by_schema(JsiEnv env, JsiFunc& constructor, const realm::ObjectSchema& schema, Internal* internal) {
    jsi::EscapableHandleScope scope(env);
    auto& s_schemaObjectTypes = get_schemaObjectTypes();
    auto& s_class = get_class();
    ...
    if (isRealmObjectClass && !internal) {
        throw jsi::Error::New(env, "RealmObjectClass requires an internal realm object when creating instances by schema");
    }
    ...
    if (constructor.IsEmpty()) {
        ...
        if (!schemaObjects->count(schemaName)) {
            ...
            //get all properties from the schema 
            std::vector<jsi::PropertyDescriptor> properties = create_napi_property_descriptors(env, childPrototypeProperty, schema, true /*redefine*/);
        ...
        //get all properties from the schema 
        std::vector<jsi::PropertyDescriptor> properties = create_napi_property_descriptors(env, constructorPrototype, schema, false /*redefine*/);
        ...
        if (!isInstanceOfRealmObjectClass) {
            ...
            for (auto& pair : s_class.methods) {
                ...
                if (!constructorPrototype.HasOwnProperty(pair.first)) {
                    auto descriptor = jsi::PropertyDescriptor::Function(env, constructorPrototype, JsiString::New(env, pair.first) /*name*/, &method_callback, napi_default | realm::js::PropertyAttributes::DontEnum, (void*)pair.second /*callback*/);
                    properties.push_back(descriptor);
                }
            }
...
template<typename ClassType>
jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> ObjectWrap<ClassType>::setup_method(JsiEnv env, const std::string& name, hermes::Types::FunctionCallback callback) {
    auto& s_nativeMethods = get_nativeMethods();
...
template<typename ClassType>
jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> ObjectWrap<ClassType>::setup_static_method(JsiEnv env, const std::string& name, hermes::Types::FunctionCallback callback) {
    return JsiObjWrap<WrappedObject<ClassType>>::StaticMethod(name.c_str(), callback, napi_static | realm::js::PropertyAttributes::DontEnum);
}

template<typename ClassType>
jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> ObjectWrap<ClassType>::setup_property(JsiEnv env, const std::string& name, const PropertyType& property) {
    napi_property_attributes napi_attributes = napi_default | (realm::js::PropertyAttributes::DontEnum | realm::js::PropertyAttributes::DontDelete);
...
template<typename ClassType>
jsi::ClassPropertyDescriptor<WrappedObject<ClassType>> ObjectWrap<ClassType>::setup_static_property(JsiEnv env, const std::string& name, const PropertyType& property) {
