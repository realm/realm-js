...
static void *hacked_memmove(void *s1, const void *s2, size_t n) {
  // adapted from
  // https://github.com/dryc/libc11/blob/master/src/string/memmove.c
  char *dest = (char *)s1;
...
// There is a bug in memmove for some Samsung devices which will return "dest-n"
// instead of dest. The bug was originally found by QT, see
// https://bugreports.qt.io/browse/QTBUG-34984 . To work around it, we use
// linker's wrap feature to use a pure C implementation of memmove if the device
// has the problem.
static void check_memmove() {
  ...
  if (ptr != array + 1 || strncmp(array, "FFooba", len) != 0) {
    __android_log_print(ANDROID_LOG_DEBUG, "JSRealm",
                        "memmove is broken on this device. Switching to the "
