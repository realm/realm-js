...
- <How to hit and notice issue? what was the impact?> ([#????](https://github.com/realm/realm-js/issues/????), since v?.?.?)
- None.
>>>>>>> 75ca5acbc (Initial commit for feature branch)
...
- Support arithmetic operations (+, -, _, /) in queries. Operands can be properties and/or constants of numeric types (`int`, `float`, `double` or `Decimal128`). You can now say something like `(age + 5) _ 2 > child.age`.
