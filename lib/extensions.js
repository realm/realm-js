...
module.exports = function(realmConstructor, context) {
    ...
    Object.defineProperties(realmConstructor, getOwnPropertyDescriptors({
        open: function (config) {
            ...
            let openLocalRealmImmediately = false; // Default is syncBeforeOpen
            if (config.sync[behavior] !== undefined) {
                ...
                switch (type) {
                    ...
                    case 'openImmediately': 
                        ...
                        break;
                    default:
                        throw Error("Invalid type: '" + type + "'. Only 'syncBeforeOpen' and 'openImmediately' is allowed.")
                }
            }
            ...
            if (openLocalRealmImmediately) {
                ...
                let openPromise = Promise.race([timeOutPromise, openRemoteRealmPromise]);
                openPromise._canceled = false;
                openPromise.cancel = () => {
                    openPromise._canceled = true;
                    if (asyncOpenTask) {
                        asyncOpenTask.cancel();
                    }
                };
