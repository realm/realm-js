...
describe("ArrayBuffer", () => {
  ...
  it("supports arrayBuffer", () => {
    const realm = new Realm({ schema: [SingleSchema] });
  ...
  it("supports arrayBuffer-nodeBuffer", () => {
    if (!isNodeProcess) {
      return;
    }
  ...
  it("supports inserting empty arrayBuffer", () => {
    ...
    expect(() => {
      realm.write(() => realm.create(SingleSchema.name, { a: new ArrayBuffer(0) }));
    }).to.not.throw();
  });
  it("supports inserting null for optional arrayBuffer", () => {
    SingleSchema.properties.a = "data?";
    expect(() => {
  ...
  it("handles wrong input", () => {
    ...
    }).throws(Error, "Expected value to be an instance of ArrayBuffer, got an object");
  });
});
