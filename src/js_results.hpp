...
template <typename T> class Results : public realm::Results {
public:
...
template <typename T>
struct ResultsClass
    : ClassDefinition<T, realm::js::Results<T>, CollectionClass<T>> {
  using Type = T;
  ...
  static ObjectType create_instance(ContextType, SharedRealm,
                                    const std::string &object_type);

  template <typename U>
  static ObjectType create_filtered(ContextType, const U &, Arguments &);

  static std::vector<std::pair<std::string, bool>> get_keypaths(ContextType,
                                                                Arguments &);
  ...
  MethodMap<T> const methods = {
      ...
      {"min", wrap<compute_aggregate_on_collection<ResultsClass<T>,
                                                   AggregateFunc::Min>>},
      {"max", wrap<compute_aggregate_on_collection<ResultsClass<T>,
                                                   AggregateFunc::Max>>},
      {"sum", wrap<compute_aggregate_on_collection<ResultsClass<T>,
                                                   AggregateFunc::Sum>>},
      {"avg", wrap<compute_aggregate_on_collection<ResultsClass<T>,
                                                   AggregateFunc::Avg>>},
...
template <typename T>
typename T::Object ResultsClass<T>::create_instance(ContextType ctx,
                                                    realm::Results results) {
  return create_object<T, ResultsClass<T>>(
      ctx, new realm::js::Results<T>(std::move(results)));
}

template <typename T>
typename T::Object
ResultsClass<T>::create_instance(ContextType ctx, SharedRealm realm,
                                 const std::string &object_type) {
  auto table =
      ObjectStore::table_for_object_type(realm->read_group(), object_type);
  if (!table) {
    throw std::runtime_error("Table does not exist. Object type: " +
                             object_type);
  }
  return create_object<T, ResultsClass<T>>(
      ctx, new realm::js::Results<T>(realm, table));
}

template <typename T>
template <typename U>
typename T::Object ResultsClass<T>::create_filtered(ContextType ctx,
                                                    const U &collection,
                                                    Arguments &args) {
  if (collection.get_type() != realm::PropertyType::Object) {
    throw std::runtime_error(
        "Filtering non-object Lists and Results is not yet implemented.");
  }
  ...
  query_parser::ArgumentConverter<ValueType, NativeAccessor<T>> converter(
      accessor, &args.value[1], args.count - 1);
  ...
  if (ordering)
    return create_instance(ctx, collection.filter(std::move(query))
                                    .apply_ordering(std::move(*ordering)));
  else
    return create_instance(ctx, collection.filter(std::move(query)));
}
...
ResultsClass<T>::get_keypaths(ContextType ctx, Arguments &args) {
  ...
  if (args.count == 0) {
    ...
    validate_argument_count(args.count, 1,
                            "Second argument is not allowed if passed an array "
                            "of sort descriptors");
...
template <typename T>
void ResultsClass<T>::get_length(ContextType ctx, ObjectType object,
...
template <typename T>
void ResultsClass<T>::get_type(ContextType ctx, ObjectType object,
                               ReturnValue &return_value) {
  auto results = get_internal<T, ResultsClass<T>>(ctx, object);
  return_value.set(local_string_for_property_type(results->get_type() &
                                                  ~realm::PropertyType::Flags));
}

template <typename T>
void ResultsClass<T>::get_optional(ContextType ctx, ObjectType object,
                                   ReturnValue &return_value) {
  auto results = get_internal<T, ResultsClass<T>>(ctx, object);
...
template <typename T>
void ResultsClass<T>::get_index(ContextType ctx, ObjectType object,
...
template <typename T>
void ResultsClass<T>::description(ContextType ctx, ObjectType this_object,
  ...
  std::string serialized_query = query.get_description() + " " +
                                 descriptor.get_description(query.get_table());
  return_value.set(Value::from_string(ctx, serialized_query));
}

template <typename T>
void ResultsClass<T>::snapshot(ContextType ctx, ObjectType this_object,
                               Arguments &args, ReturnValue &return_value) {
  args.validate_maximum(0);
  auto results = get_internal<T, ResultsClass<T>>(ctx, this_object);
...
template <typename T>
void ResultsClass<T>::filtered(ContextType ctx, ObjectType this_object,
                               Arguments &args, ReturnValue &return_value) {
  auto results = get_internal<T, ResultsClass<T>>(ctx, this_object);
...
template <typename T>
void ResultsClass<T>::sorted(ContextType ctx, ObjectType this_object,
                             Arguments &args, ReturnValue &return_value) {
  auto results = get_internal<T, ResultsClass<T>>(ctx, this_object);
  return_value.set(ResultsClass<T>::create_instance(
      ctx, results->sort(ResultsClass<T>::get_keypaths(ctx, args))));
}

template <typename T>
void ResultsClass<T>::is_valid(ContextType ctx, ObjectType this_object,
                               Arguments &args, ReturnValue &return_value) {
  return_value.set(
      get_internal<T, ResultsClass<T>>(ctx, this_object)->is_valid());
}

template <typename T>
void ResultsClass<T>::is_empty(ContextType ctx, ObjectType this_object,
                               Arguments &args, ReturnValue &return_value) {
  return_value.set(get_internal<T, ResultsClass<T>>(ctx, this_object)->size() ==
                   0);
}

template <typename T>
template <typename Fn>
void ResultsClass<T>::index_of(ContextType ctx, Fn &fn, Arguments &args,
                               ReturnValue &return_value) {
  ...
  try {
    ...
    throw std::runtime_error(
        "Object type does not match the type contained in result");
...
template <typename T>
void ResultsClass<T>::update(ContextType ctx, ObjectType this_object,
                             Arguments &args, ReturnValue &return_value) {
  args.validate_maximum(2);
...
template <typename T>
template <typename U>
void ResultsClass<T>::add_listener(ContextType ctx, U &collection,
  ...
  auto token = collection.add_notification_callback(
      [=](CollectionChangeSet const &change_set, std::exception_ptr exception) {
        ...
        ValueType arguments[]{static_cast<ObjectType>(protected_this),
                              CollectionClass<T>::create_collection_change_set(
                                  protected_ctx, change_set)};
        Function<T>::callback(protected_ctx, protected_callback, protected_this,
                              2, arguments);
      });
  collection.m_notification_tokens.emplace_back(protected_callback,
                                                std::move(token));
}

template <typename T>
void ResultsClass<T>::add_listener(ContextType ctx, ObjectType this_object,
                                   Arguments &args, ReturnValue &return_value) {
  auto results = get_internal<T, ResultsClass<T>>(ctx, this_object);
...
template <typename T>
template <typename U>
void ResultsClass<T>::remove_listener(ContextType ctx, U &collection,
                                      ObjectType this_object, Arguments &args) {
  args.validate_maximum(1);
...
template <typename T>
void ResultsClass<T>::remove_listener(ContextType ctx, ObjectType this_object,
                                      Arguments &args,
                                      ReturnValue &return_value) {
  auto results = get_internal<T, ResultsClass<T>>(ctx, this_object);
...
template <typename T>
void ResultsClass<T>::remove_all_listeners(ContextType ctx,
