...
template <typename T> class AppClass : public ClassDefinition<T, SharedApp> {
...
public:
  ...
  /**
   * Generates instances of GenericNetworkTransport, eventually allowing Realm
   * Object Store to perform network requests. Exposed to allow other components
   * (ex the RPCServer) to override the underlying implementation.
   */
  static inline NetworkTransportFactory transport_generator =
      +[](ContextType ctx, typename NetworkTransport::Dispatcher eld)
  ...
  // static methods
  static void clear_app_cache(ContextType, ObjectType, Arguments &,
                              ReturnValue &);
...
template <typename T>
inline typename T::Object AppClass<T>::create_instance(ContextType ctx,
                                                       SharedApp app) {
  return create_object<T, AppClass<T>>(ctx, new SharedApp(app));
}

template <typename T>
void AppClass<T>::constructor(ContextType ctx, ObjectType this_object,
                              Arguments &args) {
  ...
  if (Value::is_object(ctx, args[0])) {
    ...
    ValueType config_id_value =
        Object::get_property(ctx, config_object, config_id);
    ...
    ValueType config_base_url_value =
        Object::get_property(ctx, config_object, config_base_url);
    if (!Value::is_undefined(ctx, config_base_url_value)) {
      config.base_url = util::Optional<std::string>(
          Value::validated_to_string(ctx, config_base_url_value, "baseUrl"));
    }

    ValueType config_timeout_value =
        Object::get_property(ctx, config_object, config_timeout);
    if (!Value::is_undefined(ctx, config_timeout_value)) {
      config.default_request_timeout_ms = util::Optional<uint64_t>(
          Value::validated_to_number(ctx, config_timeout_value, "timeout"));
    }

    ValueType config_app_value =
        Object::get_property(ctx, config_object, config_app);
    if (!Value::is_undefined(ctx, config_app_value)) {
      ObjectType config_app_object =
          Value::validated_to_object(ctx, config_app_value, "app");

      ValueType config_app_name_value =
          Object::get_property(ctx, config_app_object, config_app_name);
      if (!Value::is_undefined(ctx, config_app_name_value)) {
        config.local_app_name = util::Optional<std::string>(
            Value::validated_to_string(ctx, config_app_name_value, "name"));
      }

      ValueType config_app_version_value =
          Object::get_property(ctx, config_app_object, config_app_version);
      if (!Value::is_undefined(ctx, config_app_version_value)) {
        config.local_app_version =
            util::Optional<std::string>(Value::validated_to_string(
                ctx, config_app_version_value, "version"));
      }
    }
  ...
  config.transport_generator = [ctx =
                                    Protected(Context::get_global_context(ctx)),
                                eld = NetworkTransport::make_dispatcher()] {
...
template <typename T> std::string AppClass<T>::get_user_agent() {
  return "RealmJS/" + package_version + " (" + platform_context + ", " +
         platform_os + ", v" + platform_version + ")";
}

template <typename T>
void AppClass<T>::get_app_id(ContextType ctx, ObjectType this_object,
...
template <typename T>
void AppClass<T>::log_in(ContextType ctx, ObjectType this_object,
  ...
  app::AppCredentials app_credentials =
      *get_internal<T, CredentialsClass<T>>(ctx, credentials_object);

  app->log_in_with_credentials(app_credentials,
                               Function::wrap_callback_result_first(
                                   ctx, this_object, callback_function,
                                   [app](ContextType ctx, SharedUser user) {
                                     REALM_ASSERT_RELEASE(user);
                                     return UserClass<T>::create_instance(
                                         ctx, std::move(user), std::move(app));
                                   }));
}

template <typename T>
void AppClass<T>::get_all_users(ContextType ctx, ObjectType this_object,
                                ReturnValue &return_value) {
  auto app = *get_internal<T, AppClass<T>>(ctx, this_object);
...
template <typename T>
void AppClass<T>::get_current_user(ContextType ctx, ObjectType this_object,
                                   ReturnValue &return_value) {
  auto app = *get_internal<T, AppClass<T>>(ctx, this_object);
...
template <typename T>
void AppClass<T>::switch_user(ContextType ctx, ObjectType this_object,
                              Arguments &args, ReturnValue &return_value) {
  args.validate_count(1);

  auto app = *get_internal<T, AppClass<T>>(ctx, this_object);
  auto user = get_internal<T, UserClass<T>>(
      ctx, Value::validated_to_object(ctx, args[0], "user"));
...
template <typename T>
void AppClass<T>::remove_user(ContextType ctx, ObjectType this_object,
                              Arguments &args, ReturnValue &return_value) {
...
template <typename T>
void AppClass<T>::get_email_password_auth(ContextType ctx,
...
template <typename T>
void AppClass<T>::clear_app_cache(ContextType ctx, ObjectType this_object,
                                  Arguments &args, ReturnValue &return_value) {
...
template <typename T>
void AppClass<T>::get_app(ContextType ctx, ObjectType this_object,
                          Arguments &args, ReturnValue &return_value) {
  args.validate_count(1);
...
template <typename T>
void AppClass<T>::set_versions(ContextType ctx, ObjectType this_object,
                               Arguments &args, ReturnValue &return_value) {
  args.validate_count(1);
  auto versions = Value::validated_to_object(ctx, args[0]);
  AppClass<T>::package_version =
      Object::validated_get_string(ctx, versions, "packageVersion");
  AppClass<T>::platform_context =
      Object::validated_get_string(ctx, versions, "platformContext");
  AppClass<T>::platform_os =
      Object::validated_get_string(ctx, versions, "platformOs");
  AppClass<T>::platform_version =
      Object::validated_get_string(ctx, versions, "platformVersion");
}
