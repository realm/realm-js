...
describe("Flexible sync", function () {
  ...
  describe("temp outer until we can modify subs", function () {
  ...
  describe("synchronising data", function () {
    // Temporary until we can change subscriptions
    importAppBeforeEach("with-db-flx", {});
    // importAppBefore("with-db-flx", {}, "all");
    authenticateUserBeforeEach();
    ...
    // Waiting on https://jira.mongodb.org/browse/REALMC-10860
    xit("starts syncing items if the filter changes to match some items", async function (this: RealmContext) {
      const { person, sub } = await addSubscriptionAndPerson(
        this,
        this.realm.objects(PersonSchema.name).filtered("age > 30"),
      );
      expect(this.realm.objectForPrimaryKey(PersonSchema.name, person._id)).to.be.undefined;
    ...
    // Waiting on https://jira.mongodb.org/browse/REALMC-10860
    xit("stops syncing items when a subscription is removed", async function (this: RealmContext) {
      const { person, sub } = await addSubscriptionAndPerson(this, this.realm.objects(PersonSchema.name));
      expect(this.realm.objectForPrimaryKey(PersonSchema.name, person._id)).to.not.be.undefined;
    ...
    // Waiting on https://jira.mongodb.org/browse/REALMC-10860
    xit("stops syncing items if the filter changes to not match some items", async function (this: RealmContext) {
      const { person, sub } = await addSubscriptionAndPerson(
        this,
        this.realm.objects(PersonSchema.name).filtered("age > 30"),
      );
