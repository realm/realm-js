...
template <typename VM, typename GetterSetters = EmptyGetterSetters,
          typename Builder = NoBuilder<VM>, typename Collection = NoData>
...
public:
  ...
  JSObject(ContextType _context, RealmData _data)
    ...
    collection->on_change([=](collection::Notification notification) {
      ...
      // This is to control when JS-VM is shutting down but they are still
      // updates pending by Realm. We basically ignore any update if the object
      // has been disposed.
  ...
  template <typename Realm_ChangeSet> void update(Realm_ChangeSet &change_set) {
    /* This is necessary for NodeJS. */
    HANDLESCOPE(context)

    // This is to control when JS-VM is shutting down but they are still updates
    // pending by Realm. We basically ignore any update if the object has been
    // disposed.
  ...
  template <typename CB> void setup_finalizer(CB &&cb) {
