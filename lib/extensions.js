...
module.exports = function(realmConstructor) {
    ...
    // A default Realm instance
    realmConstructor.default = function() {
        var user;
        var config;
        ...
        if (realmConstructor.Sync) {
            ...
            if (identities.length === 1) {
                ...
                let url = URL.parse(user.server);
                let secure = (url.protocol === 'https:')?'s':'';
                let realmUrl = `realm${secure}://${url.hostname}:${url.port}/~/default`;
                config = {
                    sync: {
                        user,
                        url: realmUrl
                ...
                return Promise.reject(new Error(`One and only one user should be logged in but found ${users.length} users.`));
            }
        }
        else {
            config = {
                path: realmConstructor.defaultPath
            };
        }
