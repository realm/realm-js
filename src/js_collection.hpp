...
template <typename T> class RealmObject;
...
struct CollectionClass : ClassDefinition<T, Collection, ObservableClass<T>> {
  ...
  create_collection_change_set(ContextType ctx, StringData object_type,
                               const ObjectChangeSet &change_set,
                               realm::SharedRealm old_realm,
                               realm::SharedRealm new_realm);
  static inline ValueType
  create_collection_change_set(ContextType ctx,
                               const CollectionChangeSet &change_set);
};

template <typename T>
typename T::Value CollectionClass<T>::create_collection_change_set(
    ContextType ctx, StringData object_type, const ObjectChangeSet &change_set,
  ...
  auto make_object_array_from_modifications = [&](auto const &keys,
                                                  auto realm) {
    scratch.clear();
    scratch.reserve(keys.size());
    for (auto key : keys) {
      auto realm_object =
          realm::Object(realm, object_type, realm::ObjKey(key.first));
      auto obj = RealmObjectClass<T>::create_instance(ctx, realm_object);
      scratch.push_back(obj);
    }
    return Object::create_array(ctx, scratch);
  ...
  Object::set_property(
      ctx, object, "deletions",
      make_object_array(change_set.get_deletions(), old_realm));
  Object::set_property(
      ctx, object, "insertions",
      make_object_array(change_set.get_insertions(), new_realm));

  auto old_modifications = make_object_array_from_modifications(
      change_set.get_modifications(), old_realm);
  Object::set_property(ctx, object, "oldModifications", old_modifications);

  auto new_modifications = make_object_array_from_modifications(
      change_set.get_modifications(), new_realm);
...
template <typename T>
typename T::Value CollectionClass<T>::create_collection_change_set(
    ContextType ctx, const CollectionChangeSet &change_set) {
  ObjectType object = Object::create_empty(ctx);
  std::vector<ValueType> scratch;
  ...
  if (change_set.deletions.count() == std::numeric_limits<size_t>::max()) {
    Object::set_property(ctx, object, "deletions",
                         Object::create_array(ctx, {Value::from_null(ctx)}));
  } else {
    Object::set_property(ctx, object, "deletions",
                         make_array(change_set.deletions));
  }
  Object::set_property(ctx, object, "insertions",
                       make_array(change_set.insertions));
  Object::set_property(ctx, object, "newModifications",
                       make_array(change_set.modifications_new));
