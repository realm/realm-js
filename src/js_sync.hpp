...
template <typename T>
static std::string
partition_value_bson_to_string(typename T::Context ctx,
                               typename T::Value partition_value_value) {
  ...
  if (Value<T>::is_string(ctx, partition_value_value)) {
    ...
    if (pv.length() == 0) {
      throw std::runtime_error(
          "partitionValue of type 'string' may not be an empty string.");
    }
    ...
    if (pv > JS_MAX_SAFE_INTEGER || pv < -JS_MAX_SAFE_INTEGER ||
        fabs(fractionalPart) > 0.0) {
      throw std::runtime_error(
          "partitionValue of type 'number' must be an integer in the range: "
    ...
    throw std::runtime_error("partitionValue must be of type 'string', "
                             "'number', 'objectId', or 'null'.");
  }
...
public:
  ...
  using StateHandler = void(SyncSession::PublicState old_state,
                            SyncSession::PublicState new_state);
  using ConnectionHandler = void(SyncSession::ConnectionState new_state,
                                 SyncSession::ConnectionState old_state);
  ...
  static void simulate_error(ContextType, ObjectType, Arguments &,
                             ReturnValue &);
  static void add_progress_notification(ContextType ctx, ObjectType this_object,
                                        Arguments &, ReturnValue &);
  static void remove_progress_notification(ContextType ctx,
                                           ObjectType this_object, Arguments &,
                                           ReturnValue &);
  static void add_state_notification(ContextType ctx, ObjectType this_object,
                                     Arguments &, ReturnValue &);
  static void remove_state_notification(ContextType ctx, ObjectType this_object,
                                        Arguments &, ReturnValue &);
  static void add_connection_notification(ContextType ctx,
                                          ObjectType this_object, Arguments &,
                                          ReturnValue &);
  static void remove_connection_notification(ContextType ctx,
                                             ObjectType this_object,
                                             Arguments &, ReturnValue &);
  static void is_connected(ContextType ctx, ObjectType this_object, Arguments &,
                           ReturnValue &);
  static void resume(ContextType ctx, ObjectType this_object, Arguments &,
                     ReturnValue &);
  static void pause(ContextType ctx, ObjectType this_object, Arguments &,
                    ReturnValue &);
  //    static void override_server(ContextType, ObjectType, Arguments &,
  //    ReturnValue &);
  static void wait_for_download_completion(ContextType, ObjectType, Arguments &,
                                           ReturnValue &);
  static void wait_for_upload_completion(ContextType, ObjectType, Arguments &,
                                         ReturnValue &);
...
private:
  ...
  static std::string
  get_connection_state_value(SyncSession::ConnectionState state);
  static void wait_for_completion(Direction direction, ContextType ctx,
                                  ObjectType this_object, Arguments &args);
};

template <typename T> class SyncSessionErrorHandlerFunctor {
public:
  SyncSessionErrorHandlerFunctor(typename T::Context ctx,
                                 typename T::Function error_func)
      : m_ctx(Context<T>::get_global_context(ctx)), m_func(ctx, error_func) {}
  ...
  void operator()(std::shared_ptr<SyncSession> session, SyncError error) {
    ...
    if (error.is_client_reset_requested()) {
      ...
      Object<T>::set_property(
          ...
          Value<T>::from_string(
              m_ctx, error.user_info[SyncError::c_recovery_file_path_key]));
      Object<T>::set_property(m_ctx, config_object, "readOnly",
                              Value<T>::from_boolean(m_ctx, true));
      Object<T>::set_property(m_ctx, error_object, "config", config_object);
      name = "ClientReset";
    }

    Object<T>::set_property(m_ctx, error_object, "name",
                            Value<T>::from_string(m_ctx, name));
    Object<T>::set_property(m_ctx, error_object, "message",
                            Value<T>::from_string(m_ctx, error.message));
    Object<T>::set_property(m_ctx, error_object, "isFatal",
                            Value<T>::from_boolean(m_ctx, error.is_fatal));
    Object<T>::set_property(
        m_ctx, error_object, "category",
        Value<T>::from_string(m_ctx, error.error_code.category().name()));
    Object<T>::set_property(
        m_ctx, error_object, "code",
        Value<T>::from_number(m_ctx, error.error_code.value()));

    auto user_info = Object<T>::create_empty(m_ctx);
    for (auto &kvp : error.user_info) {
      Object<T>::set_property(m_ctx, user_info, kvp.first,
                              Value<T>::from_string(m_ctx, kvp.second));
    }
...
// An object of type SSLVerifyCallbackSyncThreadFunctor is registered with the
// sync client in order to verify SSL certificates. The
// SSLVerifyCallbackSyncThreadFunctor object's operator() is called on the sync
// client's event loop thread.
template <typename T> class SSLVerifyCallbackSyncThreadFunctor {
public:
  SSLVerifyCallbackSyncThreadFunctor(typename T::Context ctx,
                                     typename T::Function ssl_verify_func)
      : m_ctx(Context<T>::get_global_context(ctx)),
        m_func(ctx, ssl_verify_func),
        m_event_loop_dispatcher{
            SSLVerifyCallbackSyncThreadFunctor<T>::main_loop_handler},
  ...
  bool operator()(const std::string &server_address,
                  util::network::Endpoint::port_type server_port,
                  const char *pem_data, size_t pem_size, int preverify_ok,
                  int depth) {
    ...
    m_event_loop_dispatcher(this, server_address, server_port, pem_certificate,
                            preverify_ok, depth);

    bool ssl_certificate_accepted = false;
    {
      // Wait for the return value of the callback function on the node.js main
      // thread. The sync client blocks during this wait.
      std::unique_lock<std::mutex> lock(*m_mutex);
      m_cond_var->wait(
          lock, [this] { return this->m_ssl_certificate_callback_done; });
  ...
  // main_loop_handler calls the user callback (m_func) and sends the return
  // value back to the sync client's event loop thread through a condition
  // variable.
  static void
  main_loop_handler(SSLVerifyCallbackSyncThreadFunctor<T> *this_object,
                    const std::string &server_address,
                    util::network::Endpoint::port_type server_port,
                    const std::string &pem_certificate, int preverify_ok,
                    int depth) {
    ...
    Object<T>::set_property(ctx, ssl_certificate_object, "depth",
                            Value<T>::from_number(ctx, double(depth)));

    const int argc = 1;
    typename T::Value arguments[argc] = {ssl_certificate_object};
    typename T::Value ret_val =
        Function<T>::callback(ctx, this_object->m_func, 1, arguments);
...
private:
  const Protected<typename T::GlobalContext> m_ctx;
  const Protected<typename T::Function> m_func;
  util::EventLoopDispatcher<void(
      SSLVerifyCallbackSyncThreadFunctor<T> *this_object,
      const std::string &server_address,
      util::network::Endpoint::port_type server_port,
...
// TODO: We should move this function to js_user.hpp but hard due to circular
// dependency
template <typename T>
void UserClass<T>::session_for_on_disk_path(ContextType ctx,
                                            ObjectType this_object,
                                            Arguments &args,
  ...
  if (internal == nullptr) {
    throw std::runtime_error(
        "Invalid User instance. No internal instance is set");
  }

  auto user = internal->get();
  // user.get();
  if (auto session = user->session_for_on_disk_path(
          Value::validated_to_string(ctx, args[0]))) {
    return_value.set(
...
template <typename T>
void SessionClass<T>::get_config(ContextType ctx, ObjectType object,
                                 ReturnValue &return_value) {
  if (auto session = get_internal<T, SessionClass<T>>(ctx, object)->lock()) {
    ...
    Object::set_property(
        ...
        create_object<T, UserClass<T>>(
            ctx, new User<T>(session->config().user,
                             nullptr))); // FIXME: nullptr is not an app object
    // TODO: add app id
    bson::Bson partition_value_bson =
        bson::parse(session->config().partition_value);
    Object::set_property(
        ctx, config, "partitionValue",
        Value::from_string(ctx, String::from_bson(partition_value_bson)));
    if (auto dispatcher =
            session->config()
                .error_handler.template target<
                    util::EventLoopDispatcher<SyncSessionErrorHandler>>()) {
      if (auto handler =
              dispatcher->func()
                  .template target<SyncSessionErrorHandlerFunctor<T>>()) {
    ...
    if (!session->config().custom_http_headers.empty()) {
      ...
      for (auto it = session->config().custom_http_headers.begin();
        ...
        Object::set_property(ctx, custom_http_headers_object, it->first,
                             Value::from_string(ctx, it->second));
      }
      Object::set_property(ctx, config, "custom_http_headers",
                           custom_http_headers_object);
    }
...
template <typename T>
void SessionClass<T>::get_user(ContextType ctx, ObjectType object,
                               ReturnValue &return_value) {
  if (auto session = get_internal<T, SessionClass<T>>(ctx, object)->lock()) {
...
template <typename T>
void SessionClass<T>::get_state(ContextType ctx, ObjectType object,
                                ReturnValue &return_value) {
...
template <typename T>
std::string SessionClass<T>::get_connection_state_value(
    SyncSession::ConnectionState state) {
  switch (state) {
...
template <typename T>
void SessionClass<T>::get_connection_state(ContextType ctx, ObjectType object,
                                           ReturnValue &return_value) {
  return_value.set(
      get_connection_state_value(SyncSession::ConnectionState::Disconnected));
  if (auto session = get_internal<T, SessionClass<T>>(ctx, object)->lock()) {
...
template <typename T>
void SessionClass<T>::simulate_error(ContextType ctx, ObjectType this_object,
                                     Arguments &args, ReturnValue &) {
  args.validate_count(2);

  if (auto session =
          get_internal<T, SessionClass<T>>(ctx, this_object)->lock()) {
    std::error_code error_code(Value::validated_to_number(ctx, args[0]),
    ...
    SyncSession::OnlyForTesting::handle_error(
        *session, SyncError(error_code, message, false));
  }
}

template <typename T>
void SessionClass<T>::add_progress_notification(ContextType ctx,
                                                ObjectType this_object,
                                                Arguments &args,
                                                ReturnValue &return_value) {
  ...
  if (auto session =
    ...
    std::string direction =
        Value::validated_to_string(ctx, args[0], "direction");
    ...
    if (direction == "download") {
      ...
      throw std::invalid_argument(
          "Invalid argument 'direction'. Only 'download' and 'upload' progress "
    ...
    if (mode == "reportIndefinitely") {
      ...
      throw std::invalid_argument(
          "Invalid argument 'mode'. Only 'reportIndefinitely' and "
          "'forCurrentlyOutstandingWork' progress notification modes are "
          "supported");
    }

    auto callback_function =
        Value::validated_to_function(ctx, args[2], "callback");
    ...
    auto registrationToken = session->register_progress_notifier(
        std::move(progressFunc), notifierType, is_streaming);
    auto syncSession =
        create_object<T, SessionClass<T>>(ctx, new WeakSession(session));
    PropertyAttributes attributes = ReadOnly | DontEnum | DontDelete;
    Object::set_property(ctx, callback_function, "_syncSession", syncSession,
                         attributes);
    Object::set_property(ctx, callback_function, "_registrationToken",
                         Value::from_number(protected_ctx, registrationToken),
                         attributes);
  }
}

template <typename T>
void SessionClass<T>::remove_progress_notification(ContextType ctx,
                                                   ObjectType this_object,
                                                   Arguments &args,
                                                   ReturnValue &return_value) {
  args.validate_count(1);
  auto callback_function =
      Value::validated_to_function(ctx, args[0], "callback");
  auto syncSessionProp =
      Object::get_property(ctx, callback_function, "_syncSession");
  if (Value::is_undefined(ctx, syncSessionProp) ||
      Value::is_null(ctx, syncSessionProp)) {
    return;
  }

  auto syncSession = Value::validated_to_object(ctx, syncSessionProp);
  auto registrationToken =
      Object::get_property(ctx, callback_function, "_registrationToken");

  if (auto session =
          get_internal<T, SessionClass<T>>(ctx, syncSession)->lock()) {
...
template <typename T>
void SessionClass<T>::add_connection_notification(ContextType ctx,
                                                  ObjectType this_object,
                                                  Arguments &args,
                                                  ReturnValue &return_value) {
  args.validate_count(1);
  if (auto session =
    ...
    util::EventLoopDispatcher<ConnectionHandler> connection_handler(
        [=](SyncSession::ConnectionState old_state,
            SyncSession::ConnectionState new_state) {
          HANDLESCOPE(protected_ctx)
          ValueType callback_arguments[2] = {
              Value::from_string(protected_ctx,
                                 get_connection_state_value(new_state)),
              Value::from_string(protected_ctx,
                                 get_connection_state_value(old_state)),
          };
          Function<T>::callback(protected_ctx, protected_callback, 2,
                                callback_arguments);
        });
    ...
    auto notificationToken =
        session->register_connection_change_callback(std::move(connectionFunc));
    auto syncSession =
        create_object<T, SessionClass<T>>(ctx, new WeakSession(session));
    PropertyAttributes attributes = ReadOnly | DontEnum | DontDelete;
    Object::set_property(ctx, callback_function, "_syncSession", syncSession,
    ...
    Object::set_property(ctx, callback_function, "_connectionNotificationToken",
                         Value::from_number(protected_ctx, notificationToken),
                         attributes);
  }
}

template <typename T>
void SessionClass<T>::remove_connection_notification(
  ...
  auto syncSession = Value::validated_to_object(ctx, syncSessionProp);
  auto registrationToken = Object::get_property(ctx, callback_function,
...
template <typename T>
void SessionClass<T>::is_connected(ContextType ctx, ObjectType this_object,
  ...
  if (auto session =
    ...
    if (connection_state == SyncSession::ConnectionState::Connected &&
        (state == SyncSession::PublicState::Active ||
         state == SyncSession::PublicState::Dying)) {
...
template <typename T>
void SessionClass<T>::resume(ContextType ctx, ObjectType this_object,
                             Arguments &args, ReturnValue &return_value) {
  args.validate_count(0);
  return_value.set(false);
  if (auto session =
...
template <typename T>
void SessionClass<T>::pause(ContextType ctx, ObjectType this_object,
                            Arguments &args, ReturnValue &return_value) {
  args.validate_count(0);
  return_value.set(false);
  if (auto session =
...
/*template<typename T>
void SessionClass<T>::override_server(ContextType ctx, ObjectType this_object,
Arguments &args, ReturnValue&) { args.validate_count(2);
    ...
    if (port < 1 || port > 65535 || uint16_t(port) != port) {
        ...
        message << "Invalid port number. Expected an integer in the range
1-65,535, got '" << port << "'"; throw std::invalid_argument(message.str());
    }

    if (auto session = get_internal<T, SessionClass<T>>(ctx,
this_object)->lock()) { session->override_server(std::move(address),
uint16_t(port));
    }
}*/

template <typename T>
void SessionClass<T>::wait_for_completion(Direction direction, ContextType ctx,
                                          ObjectType this_object,
                                          Arguments &args) {
  args.validate_count(1);
  if (auto session =
    ...
    util::EventLoopDispatcher<DownloadUploadCompletionHandler>
        completion_handler(
             ...
             callback = Protected(ctx, callback)](std::error_code error) {
              ...
              Function<T>::callback(
                  ...
                  {!error ? Value::from_undefined(ctx)
                          : Object::create_obj(
                                ctx, {
                                         {"message", Value::from_string(
                                                         ctx, error.message())},
                                         {"errorCode", Value::from_number(
                                                           ctx, error.value())},
                                     })});
            });
    ...
    auto syncSession =
        create_object<T, SessionClass<T>>(ctx, new WeakSession(session));
    PropertyAttributes attributes = ReadOnly | DontEnum | DontDelete;
    Object::set_property(ctx, callback, "_syncSession", syncSession,
                         attributes);
  }
}

template <typename T>
void SessionClass<T>::wait_for_upload_completion(ContextType ctx,
                                                 ObjectType this_object,
                                                 Arguments &args,
                                                 ReturnValue &) {
...
template <typename T>
void SessionClass<T>::wait_for_download_completion(ContextType ctx,
                                                   ObjectType this_object,
                                                   Arguments &args,
                                                   ReturnValue &) {
...
template <typename T> class SyncClass : public ClassDefinition<T, void *> {
...
public:
  ...
  static void set_sync_log_level(ContextType, ObjectType, Arguments &,
                                 ReturnValue &);
  static void set_sync_logger(ContextType, ObjectType, Arguments &,
                              ReturnValue &);
  static void set_sync_user_agent(ContextType, ObjectType, Arguments &,
                                  ReturnValue &);
  static void initiate_client_reset(ContextType, ObjectType, Arguments &,
                                    ReturnValue &);
  static void reconnect(ContextType, ObjectType, Arguments &, ReturnValue &);
  static void has_existing_sessions(ContextType, ObjectType, Arguments &,
                                    ReturnValue &);
  static void enable_multiplexing(ContextType, ObjectType, Arguments &,
                                  ReturnValue &);
  static void deserialize_change_set(ContextType, ObjectType, Arguments &,
                                     ReturnValue &);
  static void get_all_sync_sessions(ContextType, ObjectType, Arguments &,
                                    ReturnValue &);
  static void get_sync_session(ContextType, ObjectType, Arguments &,
                               ReturnValue &);

  // private
  static void populate_sync_config(ContextType, ObjectType realm_constructor,
                                   ObjectType config_object, Realm::Config &);
  static void populate_sync_config_for_ssl(ContextType, ObjectType,
                                           SyncConfig &);
...
inline typename T::Function SyncClass<T>::create_constructor(ContextType ctx) {
  FunctionType sync_constructor =
      ObjectWrap<T, SyncClass<T>>::create_constructor(ctx);

  PropertyAttributes attributes = ReadOnly | DontEnum | DontDelete;
  Object::set_property(ctx, sync_constructor, "User",
                       ObjectWrap<T, UserClass<T>>::create_constructor(ctx),
                       attributes);
  Object::set_property(ctx, sync_constructor, "Session",
                       ObjectWrap<T, SessionClass<T>>::create_constructor(ctx),
                       attributes);
...
template <typename T>
void SyncClass<T>::get_sync_session(ContextType ctx, ObjectType this_object,
                                    Arguments &args,
                                    ReturnValue &return_value) {
  args.validate_count(2);
  ...
  std::string partition_value =
      partition_value_bson_to_string<T>(ctx, partition_value_value);
...
template <typename T>
void SyncClass<T>::get_all_sync_sessions(ContextType ctx,
                                         ObjectType this_object,
                                         Arguments &args,
                                         ReturnValue &return_value) {
  args.validate_count(1);
...
template <typename T>
void SyncClass<T>::initiate_client_reset(ContextType ctx,
                                         ObjectType this_object,
                                         Arguments &args,
                                         ReturnValue &return_value) {
  args.validate_count(2);
  auto app = *get_internal<T, AppClass<T>>(
      ctx, Value::validated_to_object(ctx, args[0], "app"));
  std::string path = Value::validated_to_string(ctx, args[1]);
  if (!app->sync_manager()->immediately_run_file_actions(std::string(path))) {
    throw std::runtime_error(
        util::format("Realm was not configured correctly. Client Reset could "
                     "not be run for Realm at: %1",
                     path));
  }
}

template <typename T>
void SyncClass<T>::set_sync_log_level(ContextType ctx, ObjectType this_object,
                                      Arguments &args,
                                      ReturnValue &return_value) {
  args.validate_count(2);
...
template <typename T>
void SyncClass<T>::set_sync_logger(ContextType ctx, ObjectType this_object,
                                   Arguments &args, ReturnValue &return_value) {
  args.validate_count(2);
  ...
  auto app = *get_internal<T, AppClass<T>>(
      ctx, Value::validated_to_object(ctx, args[0], "app"));
  auto callback_fn =
      Value::validated_to_function(ctx, args[1], "logger_callback");
...
template <typename T>
void SyncClass<T>::set_sync_user_agent(ContextType ctx, ObjectType this_object,
                                       Arguments &args,
                                       ReturnValue &return_value) {
  args.validate_count(2);
  auto app = *get_internal<T, AppClass<T>>(
      ctx, Value::validated_to_object(ctx, args[0], "app"));
  std::string application_user_agent =
      Value::validated_to_string(ctx, args[1], "user agent");
  app->sync_manager()->set_user_agent(application_user_agent);
}

template <typename T>
void SyncClass<T>::reconnect(ContextType ctx, ObjectType this_object,
                             Arguments &args, ReturnValue &return_value) {
  args.validate_count(1);
  auto app = *get_internal<T, AppClass<T>>(
      ctx, Value::validated_to_object(ctx, args[0], "app"));
...
template <typename T>
void SyncClass<T>::has_existing_sessions(ContextType ctx,
                                         ObjectType this_object,
                                         Arguments &args,
                                         ReturnValue &return_value) {
  args.validate_count(1);
...
template <typename T>
void SyncClass<T>::populate_sync_config(ContextType ctx,
                                        ObjectType realm_constructor,
                                        ObjectType config_object,
                                        Realm::Config &config) {
  ...
  if (Value::is_boolean(ctx, sync_config_value)) {
    ...
    auto sync_config_object =
        Value::validated_to_object(ctx, sync_config_value);

    std::function<SyncSessionErrorHandler> error_handler;
    ValueType error_func =
        Object::get_property(ctx, sync_config_object, "error");
    if (!Value::is_undefined(ctx, error_func)) {
      error_handler = util::EventLoopDispatcher<SyncSessionErrorHandler>(
          SyncSessionErrorHandlerFunctor<T>(
              ctx, Value::validated_to_function(ctx, error_func)));
    }

    ObjectType user_object =
        Object::validated_get_object(ctx, sync_config_object, "user");
    SharedUser user = *get_internal<T, UserClass<T>>(ctx, user_object);
    if (user->state() != SyncUser::State::LoggedIn) {
      throw std::runtime_error("User is no longer valid.");
    }

    ValueType partition_value_value =
        Object::get_property(ctx, sync_config_object, "partitionValue");
    std::string partition_value =
        partition_value_bson_to_string<T>(ctx, partition_value_value);

    config.sync_config =
        std::make_shared<SyncConfig>(user, std::move(partition_value));
    config.sync_config->error_handler = std::move(error_handler);

    SyncSessionStopPolicy session_stop_policy =
        SyncSessionStopPolicy::AfterChangesUploaded;
    ValueType session_stop_policy_value =
        Object::get_property(ctx, sync_config_object, "_sessionStopPolicy");
    if (!Value::is_undefined(ctx, session_stop_policy_value)) {
      std::string stop_session = Value::validated_to_string(
          ctx, session_stop_policy_value, "_sessionStopPolicy");
      if (stop_session == std::string("immediately")) {
        ...
        throw std::invalid_argument(
            "Unknown argument for _sessionStopPolicy: " + stop_session);
      }
    }
    config.sync_config->stop_policy = session_stop_policy;

    // Custom HTTP headers
    ValueType sync_custom_http_headers_value =
        Object::get_property(ctx, sync_config_object, "customHttpHeaders");
    if (!Value::is_undefined(ctx, sync_custom_http_headers_value)) {
      auto sync_custom_http_headers =
          Value::validated_to_object(ctx, sync_custom_http_headers_value);
      auto property_names =
          Object::get_property_names(ctx, sync_custom_http_headers);
      std::map<std::string, std::string> http_headers;
      for (auto it = property_names.begin(); it != property_names.end(); ++it) {
        auto name = *it;
        ValueType prop_value =
            Object::get_property(ctx, sync_custom_http_headers, name);
    ...
    for (auto env_var : env_vars) {
      ...
      if (http_proxy != NULL) {
        ...
        if (protocol == "http") {
          ...
          throw std::runtime_error(
              "Expected either 'http' or 'https' as protocol for " + env_var +
              " (got " + protocol + ")");
        }
        ...
        config.sync_config->proxy_config =
            util::Optional<SyncConfig::ProxyConfig>(std::move(proxy_config));
        break;
      }
    }
    ...
    ValueType ssl_config_value =
        Object::get_property(ctx, sync_config_object, "ssl");
    ...
    if (!config.encryption_key.empty()) {
      ...
      std::copy_n(config.encryption_key.begin(),
                  config.sync_config->realm_encryption_key->size(),
                  config.sync_config->realm_encryption_key->begin());
    }
...
template <typename T>
void SyncClass<T>::populate_sync_config_for_ssl(ContextType ctx,
                                                ObjectType config_object,
                                                SyncConfig &config) {
  ...
  ValueType certificate_path =
      Object::get_property(ctx, config_object, "certificatePath");
  if (Value::is_string(ctx, certificate_path)) {
    config.ssl_trust_certificate_path =
        std::string(Value::to_string(ctx, certificate_path));
  }

  ValueType validate_callback =
      Object::get_property(ctx, config_object, "validateCallback");
  if (Value::is_function(ctx, validate_callback)) {
    config.ssl_verify_callback = SSLVerifyCallbackSyncThreadFunctor<T>{
        ctx, Value::to_function(ctx, validate_callback)};
  }
}

template <typename T>
void SyncClass<T>::enable_multiplexing(ContextType ctx, ObjectType this_object,
                                       Arguments &args,
                                       ReturnValue &return_value) {
  args.validate_count(1);
  auto app = *get_internal<T, AppClass<T>>(
