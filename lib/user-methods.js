...
function fetchWithTimeout(input, init) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        xhr.onerror = () => reject(new TypeError('Network request error'));
        xhr.ontimeout = () => {
            let init2 = Object.assign({}, init);
            if (xhr.timeout >= 10 * 60 * 1000) {
                init2.timeout = xhr.timeout;
            } else {
                init2.timeout = xhr.timeout * 2;
            }
            fetchWithTimeout(input, init2);
        };
