...
template <typename> struct ResultsClass;
template <typename> struct ListClass;

template <typename> struct SetClass;
...
inline PropertyAttributes operator|(PropertyAttributes a,
                                    PropertyAttributes b) {
  return PropertyAttributes(static_cast<unsigned>(a) |
...
public:
  ...
  TypeErrorException(NativeAccessor &accessor, StringData object_type,
      ...
      : std::invalid_argument(util::format(
            "%1.%2 must be of type '%3', got '%4' (%5)", object_type, prop.name,
            type_string(prop), accessor.typeof(value), accessor.print(value))) {
  }

  TypeErrorException(const char *name, std::string const &type,
                     std::string const &value)
      : std::invalid_argument(util::format("%1 must be of type '%2', got (%3)",
                                           name ? name : "JS value", type,
                                           value)) {}
...
template <typename T> struct Value {
  ...
  static bool is_valid_for_property(ContextType, const ValueType &,
                                    const Property &);
  static bool is_valid_for_property_type(ContextType, const ValueType &,
...
#define VALIDATED(return_t, type)                                              \
  static return_t validated_to_##type(ContextType ctx, const ValueType &value, \
...
template <typename T> struct Function {
  ...
  static ValueType callback(ContextType, const FunctionType &,
                            const ObjectType &, size_t, const ValueType[]);
  static ValueType callback(ContextType ctx, const FunctionType &f,
                            const ObjectType &o,
                            std::initializer_list<ValueType> args) {
    return callback(ctx, f, o, args.size(), args.begin());
  }
  static ValueType callback(ContextType ctx, const FunctionType &f,
                            size_t count, const ValueType args[]) {
  ...
  static ValueType call(ContextType, const FunctionType &, const ObjectType &,
                        size_t, const ValueType[]);
  template <size_t N>
  static ValueType call(ContextType ctx, const FunctionType &function,
                        const ObjectType &this_object,
                        const ValueType (&arguments)[N]) {
    return call(ctx, function, this_object, N, arguments);
  }
  static ValueType call(ContextType ctx, const FunctionType &function,
                        size_t argument_count, const ValueType arguments[]) {
  ...
  static ObjectType construct(ContextType ctx, const FunctionType &f,
                              std::initializer_list<ValueType> args) {
    return construct(ctx, f, args.size(), args.begin());
  }
  static ObjectType construct(ContextType, const FunctionType &, size_t,
                              const ValueType[]);
  ...
  /**
   * These wrap a js callback into a C++ callback that takes an
   * Optional<AppError> and possibly a result. The versions that accept a result
   * use the converter argument to convert the C++ result into a js result. The
   * returned callbacks have the following signatures:
   *
   * wrap_void_callback         - void(const util::Optional<app::AppError>&
   * error) wrap_callback_error_first  - void(const
   * util::Optional<app::AppError>& error, auto&& result)
   * wrap_callback_result_first - void(auto&& result, const
   * util::Optional<app::AppError>& error)
   *
   * In all cases, the converter should have a signature like
   * JsResultType(ContextType, CppResultType), possibly with const and reference
   * qualifiers on CppResultType. The converter will only be called when there
   * is no error.
   */
  static auto wrap_void_callback(ContextType, const ObjectType &this_object,
                                 const FunctionType &callback);
  template <typename Converter>
  static auto wrap_callback_error_first(ContextType,
                                        const ObjectType &this_object,
                                        const FunctionType &callback,
                                        Converter &&converter);
  template <typename Converter>
  static auto wrap_callback_result_first(ContextType,
                                         const ObjectType &this_object,
                                         const FunctionType &callback,
                                         Converter &&converter);
};
...
public:
  ...
  static ValueType get_property(ContextType c, const ObjectType &o,
                                const char *s) {
  ...
  static ValueType get_property(ContextType, const ObjectType &,
                                const String<T> &);
  static ValueType get_property(ContextType, const ObjectType &, uint32_t);
  static void set_property(ContextType ctx, ObjectType &&obj,
                           const String<T> &field, const ValueType &val,
                           PropertyAttributes attributes = None) {
    set_property(ctx, obj, field, val, attributes);
  }
  static void set_property(ContextType ctx, ObjectType &&obj, uint32_t field,
                           const ValueType &val) {
    set_property(ctx, obj, field, val);
  }
  static void set_property(ContextType, ObjectType &, const String<T> &,
                           const ValueType &,
                           PropertyAttributes attributes = None);
  static void set_property(ContextType, ObjectType &, uint32_t,
                           const ValueType &);
  static std::vector<String<T>> get_property_names(ContextType,
                                                   const ObjectType &);
  ...
  template <typename P>
  static ValueType
  validated_get_property(ContextType ctx, const ObjectType &object,
                         const P &property, const char *message = nullptr) {
    auto value = get_property(ctx, object, property);
    if (Value<T>::is_undefined(ctx, value)) {
      throw std::out_of_range(message ? message
                                      : "Object missing expected property: " +
                                            util::to_string(property));
    }
    return value;
  }

  static uint32_t validated_get_length(ContextType ctx,
                                       const ObjectType &object) {
    static const String<T> length_string = "length";
    return Value<T>::validated_to_number(
        ctx, get_property(ctx, object, length_string));
  }

#define VALIDATED(return_t, type)                                              \
  static return_t validated_get_##type(                                        \
      ContextType ctx, const ObjectType &object, const String<T> &key,         \
      const char *message = nullptr) {                                         \
    try {                                                                      \
      return Value<T>::validated_to_##type(                                    \
          ctx, get_property(ctx, object, key), std::string(key).c_str());      \
    } catch (std::invalid_argument & e) {                                      \
      throw message ? std::invalid_argument(util::format(                      \
                          "Failed to read %1: %2", message, e.what()))         \
                    : e;                                                       \
    }                                                                          \
  }                                                                            \
  static return_t validated_get_##type(                                        \
      ContextType ctx, const ObjectType &object, uint32_t index,               \
      const char *message = nullptr) {                                         \
    try {                                                                      \
      return Value<T>::validated_to_##type(ctx,                                \
                                           get_property(ctx, object, index));  \
    } catch (std::invalid_argument & e) {                                      \
      throw message ? std::invalid_argument(util::format(                      \
                          "Failed to read %1: %2", message, e.what()))         \
                    : e;                                                       \
...
#undef VALIDATED
  ...
  static ValueType call_method(ContextType ctx, const ObjectType &object,
                               const String<T> &name, uint32_t argc,
  ...
  static ValueType call_method(ContextType ctx, const ObjectType &object,
                               const String<T> &name,
                               const std::vector<ValueType> &arguments) {
    return call_method(ctx, object, name, (uint32_t)arguments.size(),
                       arguments.data());
  }
  static ValueType
  call_method(ContextType ctx, const ObjectType &object, const String<T> &name,
  ...
  static ObjectType
  create_obj(ContextType ctx,
             std::initializer_list<std::pair<String<T>, ValueType>> values) {
  ...
  static ObjectType create_instance(ContextType,
                                    typename ClassType::Internal *);

  static ObjectType create_bson_type(ContextType, StringData type,
                                     std::initializer_list<ValueType> args);

  template <typename ClassType>
  static ObjectType create_instance_by_schema(ContextType,
                                              typename T::Function &constructor,
  ...
  template <typename ClassType>
  static typename ClassType::Internal *get_internal(ContextType ctx,
                                                    const ObjectType &);

  template <typename ClassType>
  static void set_internal(ContextType ctx, const ObjectType &,
                           typename ClassType::Internal *);

  static ObjectType create_from_app_error(ContextType, const app::AppError &);
  static ValueType
  create_from_optional_app_error(ContextType,
                                 const util::Optional<app::AppError> &);
};

template <typename ValueType> class Protected {
  ...
  struct Comparator {
    bool operator()(const Protected<ValueType> &a,
                    const Protected<ValueType> &b) const;
  };
};
template <typename GlobalCtx> Protected(GlobalCtx) -> Protected<GlobalCtx>;
template <typename Ctx, typename T> Protected(Ctx, T) -> Protected<T>;

template <typename T> struct Exception : public std::runtime_error {
  using ContextType = typename T::Context;
  using ValueType = typename T::Value;
...
create_object(typename T::Context ctx,
...
template <typename T, typename ClassType>
REALM_JS_INLINE typename T::Object
create_instance_by_schema(typename T::Context ctx,
                          typename T::Function &constructor,
                          const realm::ObjectSchema &schema,
                          typename ClassType::Internal *internal = nullptr) {
  return Object<T>::template create_instance_by_schema<ClassType>(
      ctx, constructor, schema, internal);
}
...
create_instance_by_schema(typename T::Context ctx,
                          const realm::ObjectSchema &schema,
                          typename ClassType::Internal *internal = nullptr) {
  return Object<T>::template create_instance_by_schema<ClassType>(ctx, schema,
                                                                  internal);
}

template <typename T, typename ClassType>
REALM_JS_INLINE typename ClassType::Internal *
get_internal(typename T::Context ctx, const typename T::Object &object) {
  return Object<T>::template get_internal<ClassType>(ctx, object);
}

template <typename T, typename ClassType>
REALM_JS_INLINE void set_internal(typename T::Context ctx,
                                  const typename T::Object &object,
                                  typename ClassType::Internal *ptr) {
  Object<T>::template set_internal<ClassType>(ctx, object, ptr);
}

template <typename T>
inline bool Value<T>::is_valid_for_property(ContextType context,
                                            const ValueType &value,
                                            const Property &prop) {
  return is_valid_for_property_type(context, value, prop.type,
                                    prop.object_type);
}

template <typename T>
inline bool Value<T>::is_valid_for_property_type(ContextType context,
                                                 const ValueType &value,
                                                 realm::PropertyType type,
                                                 StringData object_type) {
  ...
  auto check_value = [&](auto &&value) {
    if (is_nullable(type) &&
        (is_null(context, value) || is_undefined(context, value))) {
      return true;
    }
  ...
  auto check_collection_type = [&](auto &&list) {
    ...
    return list_type == type && is_nullable(list_type) == is_nullable(type) &&
           (type != PropertyType::Object ||
            list->get_object_schema().name == object_type);
  };

  if (!realm::is_array(type) && !realm::is_set(type) &&
      !realm::is_dictionary(type)) {
  ...
  if (is_object(context, value)) {
    ...
    if (Object<T>::template is_instance<ResultsClass<T>>(context, object)) {
      return check_collection_type(
          get_internal<T, ResultsClass<T>>(context, object));
    }
    if (Object<T>::template is_instance<ListClass<T>>(context, object)) {
      return check_collection_type(
          get_internal<T, ListClass<T>>(context, object));
    }
    if (Object<T>::template is_instance<SetClass<T>>(context, object)) {
      return check_collection_type(
          get_internal<T, SetClass<T>>(context, object));
    }
...
template <typename T>
inline typename T::Value Value<T>::from_timestamp(typename T::Context ctx,
                                                  Timestamp ts) {
  return Object<T>::create_date(ctx, ts.get_seconds() * 1000 +
                                         ts.get_nanoseconds() / 1000000);
}

template <typename T>
inline typename T::Object
Object<T>::create_from_app_error(ContextType ctx, const app::AppError &error) {
  return Object::create_obj(
      ctx, {
...
template <typename T>
inline typename T::Object
Object<T>::create_bson_type(ContextType ctx, StringData type,
                            std::initializer_list<ValueType> args) {
  auto realm =
      Value<T>::validated_to_object(ctx, Object<T>::get_global(ctx, "Realm"));
  auto bson = Value<T>::validated_to_object(
      ctx, Object<T>::get_property(ctx, realm, "BSON"));
  auto ctor =
      Value<T>::to_constructor(ctx, Object<T>::get_property(ctx, bson, type));
  return Function<T>::construct(ctx, ctor, args);
}

template <typename T>
inline typename T::Value Object<T>::create_from_optional_app_error(
    ContextType ctx, const util::Optional<app::AppError> &error) {
  if (!error)
...
template <typename T>
inline typename T::Value Value<T>::from_objkey(typename T::Context ctx,
                                               const ObjKey &value) {
  throw std::runtime_error("'Mixed' type support is not implemented yet");
}

template <typename T>
inline typename T::Value Value<T>::from_objlink(typename T::Context ctx,
                                                const ObjLink &value) {
  throw std::runtime_error("'Mixed' type support is not implemented yet");
}

template <typename T>
inline typename T::Value Value<T>::from_bson(typename T::Context ctx,
                                             const bson::Bson &value) {
  ...
  case Type::Int64: {
    // int64 needs special handling. The server uses it for all intish numbers,
    // even 1.0, so we map it to a plain js number if it is in the range where
    // it can be done precisely, otherwise we map to the bson.Long type which
    // preserves the value, but is harder to use.
  ...
  case Type::Timestamp: {
    ...
    return Object<T>::create_bson_type(
        ctx, "Timestamp",
        {
            // The constructor takes the arguments "backwards" from standard
            // order.
  ...
  case Type::Binary: {
    ...
    const auto decoded = realm::util::base64_decode_to_vector(
        StringData(vec.data(), vec.size()));
    if (!decoded)
      throw std::invalid_argument("invalid base64 in binary data");
    auto Uint8Array =
        Value<T>::to_function(ctx, Object<T>::get_global(ctx, "Uint8Array"));
    auto array = Function<T>::construct(
        ctx, Uint8Array,
        {
    ...
    return Object<T>::create_bson_type(
        ctx, "Binary",
        {
            array,
            Value<T>::from_number(
                ctx, 0), // TODO get subtype from `value` once it is possible.
        });
  }
  case Type::Document:
...
template <typename T>
inline typename T::Object Value<T>::from_bson(typename T::Context ctx,
                                              const bson::BsonDocument &doc) {
...
inline bson::Bson Value<T>::to_bson(typename T::Context ctx, ValueType value) {
  // For now going through the bson.EJSON.stringify() since it will correctly
  // handle the special JS types. Consider directly converting to Bson if we
  // need more control or there are performance issues.
  auto realm =
      Value::validated_to_object(ctx, Object<T>::get_global(ctx, "Realm"));
  auto bson = Value::validated_to_object(
      ctx, Object<T>::get_property(ctx, realm, "_bson"));
  auto ejson = Value::validated_to_object(
      ctx, Object<T>::get_property(ctx, bson, "EJSON"));
  auto call_args_json = Object<T>::call_method(
      ctx, ejson, "stringify",
      {
          value,
          Object<T>::create_obj(ctx,
                                {{"relaxed", Value::from_boolean(ctx, false)}}),
      });
  return bson::parse(std::string(Value::to_string(ctx, call_args_json)));
}

template <typename T>
auto Function<T>::wrap_void_callback(ContextType ctx,
                                     const ObjectType &this_object,
                                     const FunctionType &callback) {
...
template <typename T>
template <typename Converter>
auto Function<T>::wrap_callback_error_first(ContextType ctx,
                                            const ObjectType &this_object,
                                            const FunctionType &callback,
                                            Converter &&converter) {
  return [ctx = Protected(Context<T>::get_global_context(ctx)),
    ...
    Function::callback(
        ctx, callback, this_object,
        {
            error ? Value<T>::from_undefined(ctx)
                  : converter(ctx, std::forward<decltype(result)>(result)),
            Object<T>::create_from_optional_app_error(ctx, error),
        });
  };
}
...
template <typename T>
template <typename Converter>
auto Function<T>::wrap_callback_result_first(ContextType ctx,
                                             const ObjectType &this_object,
                                             const FunctionType &callback,
                                             Converter &&converter) {
  return [callback = wrap_callback_error_first(
              ctx, this_object, callback, std::forward<Converter>(converter))](
             auto &&result,
             const util::Optional<app::AppError> &error) -> decltype(auto) {
