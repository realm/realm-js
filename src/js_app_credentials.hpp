...
public:
  ...
  static void email_password(ContextType, ObjectType, Arguments &,
                             ReturnValue &);
  ...
  static void server_api_key(ContextType, ObjectType, Arguments &,
                             ReturnValue &);
...
typename T::Function CredentialsClass<T>::create_constructor(ContextType ctx) {
  FunctionType credentials_constructor =
      ObjectWrap<T, CredentialsClass<T>>::create_constructor(ctx);
  return credentials_constructor;
}

template <typename T>
void CredentialsClass<T>::facebook(ContextType ctx, ObjectType,
                                   Arguments &arguments,
                                   ReturnValue &return_value) {
  arguments.validate_maximum(1);

  realm::app::AppCredentialsToken token =
      Value::validated_to_string(ctx, arguments[0]);

  auto credentials = realm::app::AppCredentials::facebook(token);
  return_value.set(create_object<T, CredentialsClass<T>>(
      ctx, new app::AppCredentials(credentials)));
}

template <typename T>
void CredentialsClass<T>::anonymous(ContextType ctx, ObjectType this_object,
                                    Arguments &arguments,
                                    ReturnValue &return_value) {
...
template <typename T>
void CredentialsClass<T>::apple(ContextType ctx, ObjectType this_object,
                                Arguments &arguments,
                                ReturnValue &return_value) {
  arguments.validate_maximum(1);
...
template <typename T>
void CredentialsClass<T>::google(ContextType ctx, ObjectType this_object,
                                 Arguments &arguments,
                                 ReturnValue &return_value) {
  arguments.validate_maximum(1);
  ...
  auto decode_arg = [&](ValueType arg) {
    ...
    if (Value::is_object(ctx, arguments[0])) {
      ...
      if (!Value::is_undefined(ctx, auth_code)) {
        return app::AppCredentials::google(app::AuthCode(
            std::string(Value::validated_to_string(ctx, auth_code))));
      }
      ...
      if (!Value::is_undefined(ctx, id_token)) {
        return app::AppCredentials::google(app::IdToken(
            std::string(Value::validated_to_string(ctx, id_token))));
      }
    }
    throw std::runtime_error(
        "Invalid arguments for Realm.App.Credentials.google()");
  };
...
template <typename T>
void CredentialsClass<T>::jwt(ContextType ctx, ObjectType this_object,
                              Arguments &arguments, ReturnValue &return_value) {
  arguments.validate_maximum(1);
...
template <typename T>
void CredentialsClass<T>::email_password(ContextType ctx,
  ...
  const std::string email =
      Value::validated_to_string(ctx, arguments[0], "email");
  const std::string password =
      Value::validated_to_string(ctx, arguments[1], "password");
...
template <typename T>
void CredentialsClass<T>::function(ContextType ctx, ObjectType this_object,
                                   Arguments &arguments,
                                   ReturnValue &return_value) {
  arguments.validate_count(1);
  const std::string payload_json =
      Value::validated_to_string(ctx, arguments[0], "payload");
  ...
  auto credentials = realm::app::AppCredentials::function(
      payload_bson.operator const bson::BsonDocument &());
  return_value.set(create_object<T, CredentialsClass<T>>(
      ctx, new app::AppCredentials(credentials)));
}

template <typename T>
void CredentialsClass<T>::user_api_key(ContextType ctx, ObjectType this_object,
                                       Arguments &arguments,
                                       ReturnValue &return_value) {
  arguments.validate_count(1);
  const std::string user_api_key =
      Value::validated_to_string(ctx, arguments[0], "user API key");
...
template <typename T>
void CredentialsClass<T>::server_api_key(ContextType ctx,
                                         ObjectType this_object,
                                         Arguments &arguments,
                                         ReturnValue &return_value) {
  arguments.validate_count(1);
  const std::string server_api_key =
      Value::validated_to_string(ctx, arguments[0], "server API key");
...
template <typename T>
void CredentialsClass<T>::provider(ContextType ctx, ObjectType this_object,
                                   Arguments &arguments,
                                   ReturnValue &return_value) {
...
template <typename T>
void CredentialsClass<T>::get_payload(ContextType ctx, ObjectType this_object,
                                      ReturnValue &return_value) {
  auto credentials = get_internal<T, CredentialsClass<T>>(ctx, this_object);
