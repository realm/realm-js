...
module.exports = {
  getGrantedPermissions(recipient) {
    if (['CurrentUser', 'OtherUser', 'Any'].indexOf(recipient) === -1) {
  ...
  applyPermissions(condition, realmUrl, accessLevel) {
    ...
    if (['None', 'Read', 'Write', 'Admin'].indexOf(accessLevel) === -1) {
      return Promise.reject(new Error(`'${accessLevel}' is not a valid access level. Must be 'None', 'Read', 'Write' or 'Admin'.`));
    }
  ...
  offerPermissions(realmUrl, accessLevel, expiresAt) {
    ...
    if (['Read', 'Write', 'Admin'].indexOf(accessLevel) === -1) {
      return Promise.reject(new Error(`'${accessLevel}' is not a valid access level. Must be 'Read', 'Write' or 'Admin'.`));
    }
