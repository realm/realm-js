...
const instanceMethods = {
    logout() {
        ...
        createConfiguration(config) {
            ...
            if (config && config.sync && config.sync.user) {
            ...
            // In the native code, `fullSynchronization` takes precedence over `partial`, so during the deprecation
            // period we only set `fullSynchronization` if partial has not been set.
