...
module.exports = {
  ...
  assertArraysEqual: function (val1, val2, errorMessage, depth) {
    ...
    if (val1.type === "data") {
      compare = (i, a, b) =>
        a === b || this.assertArraysEqual(new Uint8Array(a), b, `Data elements at index ${i}`, 1) || true;
      ...
      compare = (i, a, b) => (a && a.toString()) === (b && b.toString());
    } else {
      compare = (i, a, b) => a === b;
    }
    ...
    try {
      ...
      if (e.name !== expectedException.name) {
        throw new TestFailureError(
          `Expected a ${expectedException.name} exception but caught a ${e.name} instead. Message was: ${e.message}`,
        );
      }
