...
class webhookProdDetails {
    ...
    buildRequest(payload) {
        const request = this.addressPrefix + Buffer.from(JSON.stringify(payload.webHook), 'utf8').toString('base64') + this.addressSuffix;
        return request;
    }
 ...
 */
function fetchPlatformData(context, eventName) {
    const os = require('os');
    const { machineId } = require("node-machine-id");
    const environment = utils.getContext();

    return new Promise((resolve, reject) => {
        machineId()
        .catch((e) => {
            reject('Analytics submission failed');
            return;
        })
            ...
            const payload = {
                'webHook': {
                    ...
                    'properties': {
                        ...
                        'Binding': 'javascript',
//                        'Target': 'node.js',  // TODO:  change to 'react-native'/'electron' if it's in context's dependencies
                        'Target': environment,
 ...
 * @param  {Object} context contents of the app's package.json file
 * @param  {string} eventName Custom tag that signifies what the app was doing when the analytics were submitted
 * @returns {Promise} Promise that resolves when analytics data has been submitted
 */
async function submitAnalytics(requestFactory, context, eventName) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            fetchPlatformData(context, eventName)
            .catch((e) => {
                reject("Failed to fetch platform data  [Error: " + e + "]");
                return;
            })
            ...
            .then((payload) => {
                ...
                if (versionTags[0] < 10) {
                    ...
                    dispatchAnalytics(new mixpanelDetails(), payload);
                }
...
async function submitProductionAnalytics(eventName) {
    ...
    // return submitAnalytics(webhookProdDetails, context, eventName);
}
...
//    const package_context = require('../../package.json');
//    '../../package.json' // to determine node/electron/reactnative (find in project's root dir)
    return submitAnalytics(webhookStageDetails, context, eventName);
}

module.exports.fetchPlatformData = fetchPlatformData;
module.exports.webhookProdDetails = webhookProdDetails;
module.exports.webhookStageDetails = webhookStageDetails;
module.exports.submitAnalytics = submitAnalytics;
module.exports.submitProductionAnalytics = submitProductionAnalytics;
module.exports.submitStageAnalytics = submitStageAnalytics;
module.exports.isAnalyticsDisabled = isAnalyticsDisabled;

if (require.main === module) {
    module.submitProductionAnalytics('Install');
}
