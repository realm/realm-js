...
class Listener {
    ...
    add(regexStr, event, fn) {
        ...
        else if (fn instanceof Worker) {
            this.callbacks.push(new OutOfProcListener(regex, regexStr, fn));
        }
        else {
...
let listener;
function addListener(config, event, callback) {
    if (!config.listenerDirectory) {
        config.listenerDirectory = listenerDirectory;
    }
...
function localListenerRealms(regexStr) {
...
module.exports = function(Realm) {
    ...
    Realm.Sync.localListenerRealms = localListenerRealms.bind(Realm.Sync);
};
