...
module.exports = function(realmConstructor, context) {
    ...
    Object.defineProperties(realmConstructor, getOwnPropertyDescriptors({
        open: function (config) {
            ...
            // Determine how to open the synchronized Realm.
            let behavior = realmConstructor.exists(config) ? "existingRealmFileBehavior" : "newRealmFileBehavior";
            let openLocalRealmImmediately = config.sync[behavior] !== undefined && config.sync[behavior].type === this.OpenRealmBehaviorType.OpenImmediately; // Defaults to 'syncBeforeOpen'
            let throwOnTimeOut = config.sync[behavior] !== undefined && config.sync[behavior]["timeOutBehavior"] === this.OpenRealmTimeOutBehavior.ThrowException;
            let timeOut = Number.MAX_VALUE;
            if (config.sync[behavior] !== undefined) {
                let userDefinedTimeOut = config.sync[behavior]["timeOut"];
                if (typeof userDefinedTimeOut !== 'number') {
                    throw new Error("'timeOut' must be a number: '" + userDefinedTimeOut + "'");
                }
                timeOut = userDefinedTimeOut;
            }

            // Opens the synchronized Realm
            if (openLocalRealmImmediately) {
                return openLocalRealm(realmConstructor, config);
            } else {
                let asyncOpenTask;
                let openRemoteRealmPromise = new Promise((resolve, reject) => {
                    asyncOpenTask = new AsyncOpenTask(config);
                    asyncOpenTask.start((realm, error) => {
                        if (error) {
                            setTimeout(() => {
                                asyncOpenTask = null;
                                reject(error);
                ...
                let timeOutPromise = new Promise((resolve, reject) => {
                    let wait = setTimeout(() => {
                        ...
                        if (throwOnTimeOut) {
                            reject(new Error("Realm timed out while being downloaded: " + timeOut + " ms."));
                        } else {
                            return openLocalRealm(realmConstructor, config);
                        }
                    }, timeOut)
                ...
                let openPromise = Promise.race(openRemoteRealmPromise, timeOutPromise);
                openPromise.cancel = () => {
                    if (asyncOpenTask) {
                        asyncOpenTask.cancel();
                    }
